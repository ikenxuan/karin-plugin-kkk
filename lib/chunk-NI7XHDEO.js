...
import { index_default, Config, Networks, Common, Base, cleanOldDynamicCache, bilibiliDB, getBilibiliData, Render, bilibiliApiUrls, mergeFile, Version, douyinDB, getDouyinData, wbi_sign } from './chunk-YW4DRDI5.js';
...
var skipDynamic = async (PushItem) => {
  if (PushItem.Dynamic_Data.type === "DYNAMIC_TYPE_LIVE_RCMD" /* LIVE_RCMD */) {
    return false;
  }
  const tags = [];
...
var DouYinpush = class extends Base {
  ...
  async getdata(data) {
    ...
    for (const awemeId in data) {
      ...
      const skip = await skipDynamic2(pushItem);
      let img3 = [];
...
var skipDynamic2 = async (PushItem) => {
