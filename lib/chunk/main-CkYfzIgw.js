...
import { C as Client, b as bilibiliErrorCodeMap, g as getBilibiliData2, a as bilibiliApiUrls, w as wbi_sign, c as getDouyinData2, h as heicConvertExports, d as browserExports } from "./vendor-B50QAN-0.js";
...
import { g as getDouyinDB, a as getBilibiliDB, c as cleanOldDynamicCache, b as bilibiliDBInstance, d as douyinDBInstance } from "./db-B0jRqIce.js";
...
` + dynamicInfo.data.item.modules.module_dynamic.desc.text;
          ...
          case DynamicType.FORWARD: {
            ...
            switch (dynamicInfo.data.item.orig.type) {
              ...
              case DynamicType.DRAW: {
                const dynamicCARD2 = await getBilibiliData2("动态卡片数据", Config.cookies.bilibili, { dynamic_id: dynamicInfo.data.item.orig.id_str });
          ...
          case DynamicType.AV: {
            if (dynamicInfo.data.item.modules.module_dynamic.major.type === "MAJOR_TYPE_ARCHIVE") {
              ...
              const INFODATA = await getBilibiliData2("单个视频作品数据", "", { bvid });
          ...
          /** 直播动态 */
          case DynamicType.LIVE_RCMD: {
            const userINFO = await getBilibiliData2("用户主页数据", "", { host_mid: dynamicInfo.data.item.modules.module_author.mid, typeMode: "strict" });
            img$1 = await Render(
...
async function genParams(apiURL) {
  ...
  const loginInfo = await getBilibiliData2("登录基本信息", Config.cookies.bilibili);
...
` + data[dynamicId].Dynamic_Data.modules.module_dynamic.desc.text;
          ...
          case "DYNAMIC_TYPE_AV": {
            if (data[dynamicId].Dynamic_Data.modules.module_dynamic.major.type === "MAJOR_TYPE_ARCHIVE") {
              ...
              const INFODATA = await getBilibiliData2("单个视频作品数据", "", { bvid, typeMode: "strict" });
              if (INFODATA.data.redirect_url) {
                ...
                await getBilibiliData2("单个视频下载信息数据", "", { avid: aid, cid: INFODATA.data.cid, typeMode: "strict" });
              }
              img2 = await Render(
                "bilibili/dynamic/DYNAMIC_TYPE_AV",
                {
          ...
          case "DYNAMIC_TYPE_FORWARD": {
            ...
            switch (data[dynamicId].Dynamic_Data.orig.type) {
              ...
              case "DYNAMIC_TYPE_DRAW": {
                const dynamicCARD = await getBilibiliData2("动态卡片数据", Config.cookies.bilibili, { dynamic_id: data[dynamicId].Dynamic_Data.orig.id_str, typeMode: "strict" });
  ...
  for (const item of data) {
    if (item.is_At_user_id !== null && Array.isArray(item.is_At_user_id)) {
      for (const secUid of item.is_At_user_id) {
        const UserInfoData = await getDouyinData2("用户主页数据", Config.cookies.douyin, { sec_uid: secUid, typeMode: "strict" });
  ...
  for (const item of data) {
    if (item.search_text !== null && Array.isArray(item.search_text)) {
      for (const search_text2 of item.search_text) {
        const SuggestWordsData = await getDouyinData2("热点词数据", Config.cookies.douyin, { query: search_text2.search_text, typeMode: "strict" });
...
抖音号：${user_shortid}`);
          ...
          existingItem.group_id.push(`${groupId}:${botId}`);
          if (!isSubscribed) {
            await douyinDBInstance.subscribeDouyinUser(groupId, botId, sec_uid, user_shortid, UserInfoData.user.nickname);
          }
          await this.e.reply(`群：${groupInfo.groupName}(${groupId})
