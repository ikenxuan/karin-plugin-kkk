...
import { C as Client, b as bilibiliErrorCodeMap, g as getBilibiliData, a as bilibiliApiUrls, w as wbi_sign, c as browserExports, i as index$1, d as getDouyinData, h as heicConvertExports, m as markdown } from "./vendor-BnVODeEf.js";
import karin, { logger, config, segment, createNotFoundResponse, karinPathBase, copyConfigSync, filesByExt, watch, requireFileSync, ffmpeg, ffprobe, render, common, karin as karin$1, karinPathRoot, mkdirSync, handler } from "node-karin";
...
import { karinPathTemp, karinPathBase as karinPathBase$1 } from "node-karin/root";
        ...
        if (typeof method === "function") {
          return async (...args) => {
            ...
            if (prop === "getDouyinData" && result.status_code !== 0) {
              const err = result;
              const img2 = await Render("apiError/index", err);
...
class Cfg {
  ...
  async syncConfigToDatabase() {
    try {
      ...
      const { bilibiliDB: bilibiliDB2, douyinDB: douyinDB2 } = await Promise.resolve().then(() => index);
      if (pushCfg.bilibili) {
        await bilibiliDB2.syncConfigSubscriptions(pushCfg.bilibili);
      }
      if (pushCfg.douyin) {
        await douyinDB2.syncConfigSubscriptions(pushCfg.douyin);
      }
...
📝 简介: ${desc}`
        ...
        let correctList;
        if (this.islogin) {
          const simplify = playUrlData.data.dash.video.filter((item, index2, self) => {
...
正在下载中`);
        ...
        if (Config.bilibili.videoQuality === 0) {
          const simplify = playUrlData.result.dash.video.filter((item, index2, self) => {
            return self.findIndex((t) => {
              return t.id === item.id;
            }) === index2;
          });
...
` + dynamicInfo.data.item.modules.module_dynamic.desc.text;
            ...
            await this.e.reply(await Render("bilibili/dynamic/DYNAMIC_TYPE_DRAW", {
              ...
              share: Count(dynamicInfo.data.item.modules.module_stat.forward.count),
              ...
              fans: Count(userProfileData.data.follower),
              user_shortid: dynamicInfo.data.item.modules.module_author.mid,
              total_favorited: Count(userProfileData.data.like_num),
              following_count: Count(userProfileData.data.card.attention),
          ...
          case DynamicType.WORD: {
            const text = replacetext(br$4(dynamicInfo.data.item.modules.module_dynamic.desc.text), dynamicInfo.data.item.modules.module_dynamic.desc.rich_text_nodes);
            await this.e.reply(
              await Render("bilibili/dynamic/DYNAMIC_TYPE_WORD", {
                ...
                fans: Count(dynamicInfo.data.follower),
                user_shortid: dynamicInfo.data.item.modules.module_author.mid,
                total_favorited: Count(userProfileData.data.like_num),
                following_count: Count(userProfileData.data.card.attention),
                dynamicTYPE: "纯文动态"
            ...
            switch (dynamicInfo.data.item.orig.type) {
              ...
              case DynamicType.DRAW: {
                ...
                data = {
                  ...
                  create_time: Common.convertTimestampToDateTime(dynamicInfo.data.item.orig.modules.module_author.pub_ts),
                  avatar_url: dynamicInfo.data.item.orig.modules.module_author.face,
                  text: replacetext(br$4(dynamicInfo.data.item.orig.modules.module_dynamic.desc.text), dynamicInfo.data.item.orig.modules.module_dynamic.desc.rich_text_nodes),
      ...
      case "live_room_detail": {
        ...
        const img2 = await Render(
          "bilibili/dynamic/DYNAMIC_TYPE_LIVE_RCMD",
          {
            ...
            fans: Count(userProfileData.data.card.fans),
  ...
  sizes.forEach((size, index2) => {
    if (size <= (qualityOptions?.maxAutoVideoSize ?? Config.bilibili.maxAutoVideoSize)) {
      ...
      if (difference < smallestDifference) {
        ...
        closestId = Object.keys(results)[index2];
      }
    }
  });
  if (closestId !== null) {
    ...
    switch (qrcodestatusdata.data.data.code) {
      ...
      case 86090: {
        if (!executed86090) {
          ...
          executed86090 = true;
          const index2 = msg_id.indexOf(message2.messageId);
          if (index2 > -1) {
            msg_id.splice(index2, 1);
          }
        } else {
          executed86090 = true;
        }
...
UID：${host_mid}`);
        if (existingItem.group_id.length === 0) {
          const index2 = config2.bilibili.indexOf(existingItem);
          config2.bilibili.splice(index2, 1);
        }
      } else {
...
const sequelize$1 = new Sequelize({
  dialect: "sqlite",
  storage: join(`${karinPathBase$1}/${Root.pluginName}/data`, "bilibili.db"),
  logging: false,
  pool: {
    max: 5,
    // 连接池最大连接数
...
const sequelize = new Sequelize({
  dialect: "sqlite",
  storage: join(`${karinPathBase$1}/${Root.pluginName}/data`, "douyin.db"),
  logging: false,
  pool: {
    max: 5,
    // 连接池最大连接数
...
const getVideoRouter = (req, res) => {
  ...
  const htmlContent = index$1(path.join(resPath, "template", "videoView", "index.html"), {
    videoDataUrl,
    filename
  });
...
${g_video_url}`));
        ...
        this.is_mp4 && await downloadVideo(this.e, { video_url: g_video_url, title: { timestampTitle: `tmp_${Date.now()}.mp4`, originTitle: `${g_title}.mp4` } });
        return true;
      }
      case "user_dynamic": {
...
是否处于开播：${userinfo.user.live_status === 1 ? logger.green("true") : logger.red("false")}是否在一天内：${timeDifference < 864e5 ? logger.green("true") : logger.red("false")}`);
  ...
  */
  ...
  async setting(data) {
    ...
    try {
      let index2 = 0;
      while (data.data[index2].card_unique_name !== "user") {
        index2++;
      }
      const sec_uid = data.data[index2].user_list[0].user_info.sec_uid;
      ...
      if (existingItem) {
        let has = false;
        let groupIndexToRemove = -1;
        for (let index22 = 0; index22 < existingItem.group_id.length; index22++) {
          const item = existingItem.group_id[index22];
          const existingGroupId = item.split(":")[0];
          if (existingGroupId === String(groupId)) {
            has = true;
            groupIndexToRemove = index22;
            break;
          }
        }
        if (has) {
...
抖音号：${user_shortid}`);
          if (existingItem.group_id.length === 0) {
            const index22 = config2.douyin.indexOf(existingItem);
            config2.douyin.splice(index22, 1);
          }
        } else {
          existingItem.group_id.push(`${groupId}:${botId}`);
          if (!isSubscribed) {
...
class Kuaishou extends Base {
  ...
  async RESOURCES(data) {
    ...
    const transformedData = Object.entries(data.EmojiData.data.visionBaseEmoticons.iconUrls).map(([name, path2]) => {
