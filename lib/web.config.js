...
const processFrontendConfig = (frontendConfig) => {
    ...
    // 处理cookies配置
    if (frontendConfig['cookies'] && frontendConfig['cookies'].length > 0) {
        const cookiesData = frontendConfig['cookies'][0];
    ...
    // 处理app配置
    if (frontendConfig['app'] && frontendConfig['app'].length > 0) {
        const appData = frontendConfig['app'][0];
        processedConfig.app = {
    ...
    // 处理douyin配置
    if (frontendConfig['douyin'] && frontendConfig['douyin'].length > 0) {
        const douyinData = frontendConfig['douyin'][0];
    ...
    // 处理bilibili配置
    if (frontendConfig['bilibili'] && frontendConfig['bilibili'].length > 0) {
        const bilibiliData = frontendConfig['bilibili'][0];
    ...
    // 处理kuaishou配置
    if (frontendConfig['kuaishou'] && frontendConfig['kuaishou'].length > 0) {
        const kuaishouData = frontendConfig['kuaishou'][0];
    ...
    // 处理upload配置
    if (frontendConfig['upload'] && frontendConfig['upload'].length > 0) {
        const uploadData = frontendConfig['upload'][0];
