...
import { Bt as init_esm, D as formatDistanceToNow, E as fromUnixTime, Ft as require_dist, Gt as axios_default, Ht as zod_default, It as require_lib$1, Jt as Chalk, Kt as init_dist, Lt as require_lib, O as format, Rt as require_express, S as zhCN, T as init_date_fns, Ut as AxiosError$1, Vt as init_zod, Wt as init_axios, Yt as init_source, a as Window, i as init_lib, k as differenceInSeconds, n as require_heic_convert, qt as log4js, r as require_qr_code_styling, t as snapka, x as init_locale, zt as Xhshow } from "./vendor-BJx-ldZ8.js";
import { n as init_client, r as reactServerRender } from "./template-Bp2R026a.js";
import path, { resolve } from "node:path";
import fs from "node:fs";
import url, { fileURLToPath } from "node:url";
import os, { platform } from "node:os";
import crypto from "node:crypto";
import karin, { app, authMiddleware, checkPkgUpdate, checkPort, common, components, config, copyConfigSync, createBadRequestResponse, createNotFoundResponse, createServerErrorResponse, createSuccessResponse, db, defineConfig, ffmpeg, ffprobe, filesByExt, getBot, hooks, karin as karin$1, karinPathHtml, karinPathTemp, logger, logs, mkdirSync, range, render, requireFileSync, restart, segment, updatePkg, watch } from "node-karin";
...
var getPackageLogsPath, logsPath, getLogLevel, currentLogLevel, CustomLogger, logger$1, httpLogger, logMiddleware;
...
checkPort(3780).then((isOpen) => {
	if (isOpen) return server.listen(3780);
...
await init_Config();
var strategies = [];
const registerErrorStrategy = (strategy) => {
	strategies.push(strategy);
	logger.debug(`[ErrorHandler] 注册策略: ${strategy.name}`);
};
var parseLogsToStructured = (logs$1) => {
...
const renderErrorImage = async (ctx, opts = {}) => {
...
const sendErrorToMaster = async (ctx, img$2, customPrefix) => {
	...
	try {
		const prefix$1 = customPrefix || (isPushTask ? `${options.businessName} 任务执行出错！\n请即时解决以消除警告` : `群：${(await karin.getBot(botId)?.getGroupInfo(event && "groupId" in event ? event.groupId : ""))?.groupName || "未知"}(${event && "groupId" in event ? event.groupId : ""})\n${options.businessName} 任务执行出错！\n请即时解决以消除警告`);
		await karin.sendMaster(botId, master, [segment.text(prefix$1), ...img$2]);
	} catch (err) {
...
const wrapWithErrorHandler = (fn, options) => async (e, next) => {
								...
								(async () => {
									try {
										...
										while (verifyAttempts < maxAttempts && !verifySuccess) {
											...
											const ctx = await Promise.race([karin$1.ctx(e, { reply: true }), new Promise((_$1, reject) => setTimeout(() => reject(/* @__PURE__ */ new Error("2FA_TIMEOUT")), 6e4))]).catch(async (error) => {
...
const update = karin.task("kkk-更新检测", "*/5 * * * *", Handler, {
	name: "kkk-更新检测",
	log: false
});
