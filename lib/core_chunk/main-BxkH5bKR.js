...
import { A as format, Bt as dist_exports, C as init_locale, D as init_date_fns, Gt as init_axios, Ht as Chalk, Jt as init_esm, Kt as axios_default, O as fromUnixTime, Rt as require_express, Ut as init_source, Vt as init_dist, Wt as AxiosError$1, Xt as zod_default, Yt as init_zod, a as require_qr_code_styling, i as require_lib, j as differenceInSeconds, k as formatDistanceToNow, n as require_dist, o as init_lib, qt as Xhshow, r as require_lib$1, s as Window, t as require_heic_convert, w as zhCN, zt as require_protobufjs } from "./vendor-BCYIxQTn.js";
import { n as init_client, r as reactServerRender } from "./template-_4jkbta1.js";
...
var TypedEventEmitter, amagiEvents, emitLog, emitHttpResponse, emitNetworkRetry, emitNetworkError, emitApiSuccess, emitApiError, emitLogMark;
...
var init_bilibili$3 = __esmMin(() => {
	...
	BilibiliCommentReplyParamsSchema = zod_default.object({
		methodType: zod_default.literal("指定评论的回复", { error: "方法类型必须是\"指定评论的回复\"" }),
		oid: zod_default.string({ error: "OID必须是字符串" }).min(1, { error: "OID不能为空" }),
		type: smartNumber("评论类型不能为空", 1, true).refine((val) => [
			1,
			2,
	...
	BilibiliEmojiParamsSchema = zod_default.object({ methodType: zod_default.literal("Emoji数据", { error: "方法类型必须是\"Emoji数据\"" }) });
	BilibiliBangumiInfoParamsSchema = zod_default.object({
		methodType: zod_default.literal("番剧基本信息数据", { error: "方法类型必须是\"番剧基本信息数据\"" }),
	...
	BilibiliBangumiStreamParamsSchema = zod_default.object({
		methodType: zod_default.literal("番剧下载信息数据", { error: "方法类型必须是\"番剧下载信息数据\"" }),
		cid: smartNumber("CID不能为空", 1, true),
		ep_id: zod_default.string({ error: "番剧EP ID必须是字符串" }).min(1, { error: "番剧EP ID不能为空" })
	});
	BilibiliDynamicParamsSchema = zod_default.object({
		methodType: zod_default.enum(["动态详情数据", "动态卡片数据"], { error: "方法类型必须是\"动态详情数据\"或\"动态卡片数据\"" }),
		dynamic_id: zod_default.string({ error: "动态ID必须是字符串" }).min(1, { error: "动态ID不能为空" })
	});
	BilibiliLiveParamsSchema = zod_default.object({
		methodType: zod_default.enum(["直播间信息", "直播间初始化信息"], { error: "方法类型必须是\"直播间信息\"或\"直播间初始化信息\"" }),
		room_id: zod_default.string({ error: "直播间ID必须是字符串" }).min(1, { error: "直播间ID不能为空" })
	});
	BilibiliLoginParamsSchema = zod_default.object({ methodType: zod_default.literal("登录基本信息", { error: "方法类型必须是\"登录基本信息\"" }) });
	BilibiliQrcodeParamsSchema = zod_default.object({ methodType: zod_default.literal("申请二维码", { error: "方法类型必须是\"申请二维码\"" }) });
	BilibiliQrcodeStatusParamsSchema = zod_default.object({
		methodType: zod_default.literal("二维码状态", { error: "方法类型必须是\"二维码状态\"" }),
		qrcode_key: zod_default.string({ error: "二维码key必须是字符串" }).min(1, { error: "二维码key不能为空" })
	});
	BilibiliAv2BvParamsSchema = zod_default.object({
		methodType: zod_default.literal("AV转BV", { error: "方法类型必须是\"AV转BV\"" }),
	...
	BilibiliBv2AvParamsSchema = zod_default.object({
		methodType: zod_default.literal("BV转AV", { error: "方法类型必须是\"BV转AV\"" }),
		bvid: zod_default.string({ error: "BVID必须是字符串" }).min(1, { error: "BVID不能为空" })
	});
	BilibiliArticleParamsSchema = zod_default.object({
		methodType: zod_default.literal("专栏正文内容", { error: "方法类型必须是\"专栏正文内容\"" }),
		id: zod_default.string({ error: "专栏ID必须是字符串" }).min(1, { error: "专栏ID不能为空" })
	});
	BilibiliArticleCardParamsSchema = zod_default.object({
		methodType: zod_default.literal("专栏显示卡片信息", { error: "方法类型必须是\"专栏显示卡片信息\"" }),
	...
	BilibiliArticleInfoParamsSchema = zod_default.object({
		methodType: zod_default.literal("专栏文章基本信息", { error: "方法类型必须是\"专栏文章基本信息\"" }),
		id: zod_default.string({ error: "专栏ID必须是字符串" }).min(1, { error: "专栏ID不能为空" })
	});
	BilibiliColumnInfoParamsSchema = zod_default.object({
		methodType: zod_default.literal("文集基本信息", { error: "方法类型必须是\"文集基本信息\"" }),
		id: zod_default.string({ error: "文集ID必须是字符串" }).min(1, { error: "文集ID不能为空" })
	});
	BilibiliApplyCaptchaParamsSchema = zod_default.object({
		methodType: zod_default.literal("从_v_voucher_申请_captcha", { error: "方法类型必须是\"从_v_voucher_申请_captcha\"" }),
	...
	BilibiliValidateCaptchaParamsSchema = zod_default.object({
		methodType: zod_default.literal("验证验证码结果", { error: "方法类型必须是\"验证验证码结果\"" }),
		csrf: zod_default.string({ error: "CSRF Token必须是字符串" }).optional(),
	...
	BilibiliDanmakuParamsSchema = zod_default.object({
		methodType: zod_default.literal("实时弹幕", { error: "方法类型必须是\"实时弹幕\"" }),
		cid: smartNumber("CID不能为空", 1, true),
...
var init_douyin$3 = __esmMin(() => {
	...
	DouyinHotWordsParamsSchema = zod_default.object({
		methodType: zod_default.literal("热点词数据", { error: "方法类型必须是\"热点词数据\"" }),
		query: zod_default.string({ error: "搜索词必须是字符串" }).min(1, { error: "搜索词不能为空" })
	});
	DouyinSearchParamsSchema = zod_default.object({
		methodType: zod_default.literal("搜索数据", { error: "方法类型必须是\"搜索数据\"" }),
		query: zod_default.string({ error: "搜索词必须是字符串" }).min(1, { error: "搜索词不能为空" }),
	...
	DouyinCommentReplyParamsSchema = zod_default.object({
		methodType: zod_default.literal("指定评论回复数据", { error: "方法类型必须是\"指定评论回复数据\"" }),
		aweme_id: zod_default.string({ error: "视频ID必须是字符串" }).min(1, { error: "视频ID不能为空" }),
		comment_id: zod_default.string({ error: "评论ID必须z串" }).min(1, { error: "评论ID不能为空" }),
		number: smartPositiveInteger("评论数量必须是正整数").optional().default(5),
		cursor: zod_default.coerce.number({ error: "游标必须是数字" }).int({ error: "游标必须是整数" }).min(0, { error: "游标不能小于0" }).default(0).optional()
	});
	DouyinUserParamsSchema = zod_default.object({
		methodType: zod_default.enum(["用户主页数据", "用户主页视频列表数据"], { error: "方法类型必须是指定的枚举值之一" }),
		sec_uid: zod_default.string({ error: "用户ID必须是字符串" }).min(1, { error: "用户ID不能为空" })
	});
	DouyinMusicParamsSchema = zod_default.object({
		methodType: zod_default.literal("音乐数据", { error: "方法类型必须是\"音乐数据\"" }),
		music_id: zod_default.string({ error: "音乐ID必须是字符串" }).min(1, { error: "音乐ID不能为空" })
	});
	DouyinLiveRoomParamsSchema = zod_default.object({
		methodType: zod_default.literal("直播间信息数据", { error: "方法类型必须是\"直播间信息数据\"" }),
		web_rid: zod_default.string({ error: "直播间ID必须是字符串" }).min(1, { error: "直播间ID不能为空" }),
		room_id: zod_default.string({ error: "直播间ID必须是字符串" }).min(1, { error: "直播间ID不能为空" })
	});
	DouyinQrcodeParamsSchema = zod_default.object({
		methodType: zod_default.literal("申请二维码数据", { error: "方法类型必须是\"申请二维码数据\"" }),
		verify_fp: zod_default.string({ error: "fp指纹必须是字符串" }).min(1, { error: "fp指纹不能为空" })
	});
	DouyinEmojiListParamsSchema = zod_default.object({ methodType: zod_default.literal("Emoji数据", { error: "方法类型必须是\"Emoji数据\"" }) });
	DouyinEmojiProParamsSchema = zod_default.object({ methodType: zod_default.literal("动态表情数据", { error: "方法类型必须是\"动态表情数据\"" }) });
	DouyinDanmakuParamsSchema = zod_default.object({
		methodType: zod_default.literal("弹幕数据", { error: "方法类型必须是\"弹幕数据\"" }),
		aweme_id: zod_default.string({ error: "视频ID必须是字符串" }).min(1, { error: "视频ID不能为空" }),
...
var init_kuaishou$2 = __esmMin(() => {
	...
	KuaishouVideoParamsSchema = zod_default.object({
		methodType: zod_default.literal("单个视频作品数据", { error: "方法类型必须是\"单个视频作品数据\"" }),
		photoId: zod_default.string({ error: "视频ID必须是字符串" }).min(1, { error: "视频ID不能为空" })
	});
	KuaishouCommentParamsSchema = zod_default.object({
		methodType: zod_default.literal("评论数据", { error: "方法类型必须是\"评论数据\"" }),
		photoId: zod_default.string({ error: "视频ID必须是字符串" }).min(1, { error: "视频ID不能为空" })
	});
	KuaishouEmojiParamsSchema = zod_default.object({ methodType: zod_default.literal("Emoji数据", { error: "方法类型必须是\"Emoji数据\"" }) });
...
var init_xiaohongshu$2 = __esmMin(() => {
	...
	HomeFeedParamsSchema = zod_default.object({
		methodType: zod_default.literal("首页推荐数据", { error: "方法类型必须是\"首页推荐数据\"" }),
		cursor_score: zod_default.string({ error: "cursor_score必须是字符串" }).optional(),
		num: zod_default.coerce.number({ error: "数量必须是数字" }).int({ error: "数量必须是整数" }).min(1, { error: "数量不能小于1" }).max(100, { error: "数量不能大于100" }).optional(),
		refresh_type: zod_default.coerce.number({ error: "refresh_type必须是数字" }).int({ error: "refresh_type必须是整数" }).optional(),
		note_index: zod_default.coerce.number({ error: "note_index必须是数字" }).int({ error: "note_index必须是整数" }).optional(),
		category: zod_default.string({ error: "category必须是字符串" }).optional(),
		search_key: zod_default.string({ error: "search_key必须是字符串" }).optional()
	});
	NoteParamsSchema = zod_default.object({
		methodType: zod_default.literal("单个笔记数据", { error: "方法类型必须是\"单个笔记数据\"" }),
		note_id: zod_default.string({ error: "note_id必须是字符串" }),
		xsec_token: zod_default.string({ error: "xsec_token必须是字符串" })
	});
	CommentParamsSchema = zod_default.object({
		methodType: zod_default.literal("评论数据", { error: "方法类型必须是\"评论数据\"" }),
		note_id: zod_default.string({ error: "note_id必须是字符串" }),
		cursor: zod_default.string({ error: "cursor必须是字符串" }).optional(),
		xsec_token: zod_default.string({ error: "xsec_token必须是字符串" })
	});
	UserParamsSchema = zod_default.object({
		methodType: zod_default.literal("用户数据", { error: "方法类型必须是\"用户数据\"" }),
		user_id: zod_default.string({ error: "user_id必须是字符串" })
	});
	UserNoteParamsSchema = zod_default.object({
		methodType: zod_default.literal("用户笔记数据", { error: "方法类型必须是\"用户笔记数据\"" }),
		user_id: zod_default.string({ error: "user_id必须是字符串" }),
		cursor: zod_default.string({ error: "cursor必须是字符串" }).optional(),
		num: zod_default.coerce.number({ error: "数量必须是数字" }).int({ error: "数量必须是整数" }).min(1, { error: "数量不能小于1" }).max(100, { error: "数量不能大于100" }).optional()
	});
	EmojiListParamsSchema = zod_default.object({ methodType: zod_default.literal("表情列表", { error: "方法类型必须是\"表情列表\"" }) });
	SearchNoteParamsSchema = zod_default.object({
		methodType: zod_default.literal("搜索笔记", { error: "方法类型必须是\"搜索笔记\"" }),
		keyword: zod_default.string({ error: "keyword必须是字符串" }),
		page: zod_default.coerce.number({ error: "page必须是数字" }).int({ error: "page必须是整数" }).min(1, { error: "page不能小于1" }).optional(),
		page_size: zod_default.coerce.number({ error: "page_size必须是数字" }).int({ error: "page_size必须是整数" }).min(1, { error: "page_size不能小于1" }).max(100, { error: "page_size不能大于100" }).optional(),
		sort: zod_default.enum(SearchSortTypeValues, { error: "排序类型不合法" }).optional(),
		note_type: zod_default.coerce.number({ error: "笔记类型必须是数字" }).int({ error: "笔记类型必须是整数" }).refine((val) => SearchNoteTypeValues.includes(val), { message: "笔记类型不合法" }).optional()
	});
...
async function fetchBangumiInfo(options, cookie, requestConfig) {
	return fetchBilibiliInternal("番剧基本信息数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchBangumiStreamUrl(options, cookie, requestConfig) {
	return fetchBilibiliInternal("番剧下载信息数据", options, {
		cookie,
		requestConfig
	});
}
...
async function fetchComments(options, cookie, requestConfig) {
	return fetchBilibiliInternal("评论数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchCommentReplies$1(options, cookie, requestConfig) {
	return fetchBilibiliInternal("指定评论的回复", options, {
		cookie,
		requestConfig
	});
}
...
async function fetchDynamicDetail(options, cookie, requestConfig) {
	return fetchBilibiliInternal("动态详情数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchDynamicCard(options, cookie, requestConfig) {
	return fetchBilibiliInternal("动态卡片数据", options, {
		cookie,
		requestConfig
	});
}
...
async function fetchLiveRoomInfo$1(options, cookie, requestConfig) {
	return fetchBilibiliInternal("直播间信息", options, {
		cookie,
		requestConfig
	});
}
async function fetchLiveRoomInitInfo(options, cookie, requestConfig) {
	return fetchBilibiliInternal("直播间初始化信息", options, {
		cookie,
		requestConfig
	});
}
...
async function convertAvToBv(options, cookie, requestConfig) {
	return fetchBilibiliInternal("AV转BV", options, {
		cookie,
		requestConfig
	});
}
async function convertBvToAv(options, cookie, requestConfig) {
	return fetchBilibiliInternal("BV转AV", options, {
		cookie,
		requestConfig
	});
}
async function fetchEmojiList$3(options, cookie, requestConfig) {
	return fetchBilibiliInternal("Emoji数据", {}, {
		cookie,
		requestConfig
	});
}
...
async function fetchVideoInfo(options, cookie, requestConfig) {
	return fetchBilibiliInternal("单个视频作品数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchVideoStreamUrl(options, cookie, requestConfig) {
	return fetchBilibiliInternal("单个视频下载信息数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchVideoDanmaku(options, cookie, requestConfig) {
	return fetchBilibiliInternal("实时弹幕", options, {
		cookie,
		requestConfig
	});
}
...
var init_a_bogus = __esmMin(() => {
	...
	a_bogus_default = (url$1, user_agent) => {
...
var init_getdata$3 = __esmMin(() => {
	...
	init_API$2();
	init_sign();
	getSignature = (url$1, signType = "a_bogus", userAgent) => {
		switch (signType) {
	...
	buildSignedUrl = (url$1, signType = "a_bogus", userAgent) => {
		...
		switch (data$1.methodType) {
			...
			case "弹幕数据": {
				...
				const segmentPromises = segments.map(async (segment$1, index) => {
					...
					try {
						const segmentData = await GlobalGetData$3(`${data$1.methodType}-段${index + 1}`, {
							...baseRequestConfig,
							url: buildSignedUrl(url$1, signType, userAgent)
						});
			...
			default: {
				...
				if (typeof customUrl === "string" && customUrl.length > 0) {
					...
					return await GlobalGetData$3(data$1.methodType ?? "自定义请求", {
						...baseRequestConfig,
						url: url$1
					});
				}
...
async function fetchWorkComments$1(options, cookie, requestConfig) {
	return fetchDouyinInternal("评论数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchCommentReplies(options, cookie, requestConfig) {
	return fetchDouyinInternal("指定评论回复数据", options, {
		cookie,
		requestConfig
	});
}
...
async function searchContent(options, cookie, requestConfig) {
	...
	return fetchDouyinInternal("搜索数据", {
		...rest,
		type: mappedType
	}, {
		cookie,
...
async function fetchSuggestWords(options, cookie, requestConfig) {
	return fetchDouyinInternal("热点词数据", options, {
		cookie,
		requestConfig
	});
}
...
async function fetchUserProfile$1(options, cookie, requestConfig) {
	return fetchDouyinInternal("用户主页数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchUserVideoList(options, cookie, requestConfig) {
	return fetchDouyinInternal("用户主页视频列表数据", options, {
		cookie,
		requestConfig
	});
}
...
function createBoundDouyinFetcher(cookie, requestConfig) {
	return {
		fetchVideoWork: (options) => fetchVideoWork$1(options, cookie, requestConfig),
		...
		parseWork: (options) => parseWork$1(options, cookie, requestConfig),
		fetchDanmakuList: (options) => fetchDanmakuList(options, cookie, requestConfig),
		fetchWorkComments: (options) => fetchWorkComments$1(options, cookie, requestConfig),
		fetchCommentReplies: (options) => fetchCommentReplies(options, cookie, requestConfig),
		fetchUserProfile: (options) => fetchUserProfile$1(options, cookie, requestConfig),
		...
		fetchEmojiList: (options) => fetchEmojiList$2(options, cookie, requestConfig),
...
var init_getdata$2 = __esmMin(() => {
	...
	KuaishouData = async (data$1, cookie, requestConfig) => {
		...
		switch (data$1.methodType) {
			...
			default:
				logger$1.warn(`未知的快手数据接口：「${logger$1.red(data$1.methodType)}」`);
				return null;
		}
	};
...
async function fetchVideoWork(options, cookie, requestConfig) {
	return fetchKuaishouInternal("单个视频作品数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchWorkComments(options, cookie, requestConfig) {
	return fetchKuaishouInternal("评论数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchEmojiList$1(options, cookie, requestConfig) {
	return fetchKuaishouInternal("Emoji数据", {}, {
		cookie,
		requestConfig
	});
}
...
function createBoundKuaishouFetcher(cookie, requestConfig) {
	return {
		...
		fetchEmojiList: (options) => fetchEmojiList$1(options, cookie, requestConfig)
	};
}
...
var init_getdata$1 = __esmMin(() => {
	...
	XiaohongshuData = async (data$1, cookie, requestConfig) => {
		...
		switch (data$1.methodType) {
			case "首页推荐数据": return await GlobalGetData$1(data$1.methodType, {
				...baseRequestConfig,
			...
			case "单个笔记数据": return await GlobalGetData$1(data$1.methodType, {
				...baseRequestConfig,
			...
			case "用户笔记数据": return await GlobalGetData$1(data$1.methodType, {
				...baseRequestConfig,
				method: "GET",
			...
			case "搜索笔记": return await GlobalGetData$1(data$1.methodType, {
				...baseRequestConfig,
			...
			default: throw new Error(`未知的小红书数据接口: 「${logger$1.red(data$1.methodType)}」`);
		}
	};
...
async function fetchEmojiList(options, cookie, requestConfig) {
	return fetchXiaohongshuInternal("表情列表", {}, {
		cookie,
		requestConfig
	});
}
...
async function fetchHomeFeed(options = {}, cookie, requestConfig) {
	return fetchXiaohongshuInternal("首页推荐数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchNoteDetail(options, cookie, requestConfig) {
	return fetchXiaohongshuInternal("单个笔记数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchNoteComments(options, cookie, requestConfig) {
	return fetchXiaohongshuInternal("评论数据", options, {
		cookie,
		requestConfig
	});
}
...
async function searchNotes(options, cookie, requestConfig) {
	...
	return fetchXiaohongshuInternal("搜索笔记", {
		...rest,
...
async function fetchUserProfile(options, cookie, requestConfig) {
	return fetchXiaohongshuInternal("用户数据", options, {
		cookie,
		requestConfig
	});
}
async function fetchUserNoteList(options, cookie, requestConfig) {
	return fetchXiaohongshuInternal("用户笔记数据", options, {
		cookie,
		requestConfig
	});
}
...
function createBoundXiaohongshuFetcher(cookie, requestConfig) {
	return {
		fetchHomeFeed: (options = {}) => fetchHomeFeed(options, cookie, requestConfig),
		fetchNoteDetail: (options) => fetchNoteDetail(options, cookie, requestConfig),
		fetchNoteComments: (options) => fetchNoteComments(options, cookie, requestConfig),
		fetchUserProfile: (options) => fetchUserProfile(options, cookie, requestConfig),
		fetchUserNoteList: (options) => fetchUserNoteList(options, cookie, requestConfig),
		searchNotes: (options) => searchNotes(options, cookie, requestConfig),
...
var RECOVERABLE_ERROR_CODES, DEFAULT_MAX_RETRIES, RETRY_DELAY_BASE, isRecoverableError, delay, createNetworkErrorResult, cleanUserAgent, fetchData, normalizeHeaders, fetchResponse, isNetworkErrorResult, getHeadersAndData;
var init_networks = __esmMin(() => {
	...
	isRecoverableError = (error) => RECOVERABLE_ERROR_CODES.includes(error.code);
...
}), getLog4js, log4jsPromise, getPackageLogsPath, getLogLevel, CustomLogger, logger$1, httpLogger, logMiddleware;
var init_logger = __esmMin(() => {
	...
	getLog4js = async () => {
		try {
			...
			const lib = await Promise.resolve().then(() => (init_dist(), dist_exports));
			return lib.default || lib;
		}
	};
	log4jsPromise = getLog4js();
	getPackageLogsPath = () => {
	...
	logger$1 = new CustomLogger("default");
	httpLogger = new CustomLogger("http");
	logMiddleware = (pathsToLog) => (req, res, next) => {
...
var createBilibiliApiMethod, createBoundBilibiliApiMethod, bilibili$1, createBoundBilibiliApi;
var init_BilibiliApi = __esmMin(() => {
	init_DataFetchers();
	createBilibiliApiMethod = (methodType) => async (options, cookie) => await getBilibiliData(methodType, options, cookie);
	createBoundBilibiliApiMethod = (methodType, cookie) => async (options) => await getBilibiliData(methodType, options, cookie);
	bilibili$1 = {
	...
	createBoundBilibiliApi = (cookie, requestConfig) => ({
...
var init_routes$3 = __esmMin(() => {
	...
	createBilibiliRouteHandler = (dataFetcher, methodType, cookie, requestConfig = getBilibiliDefaultConfig(cookie)) => async (req, res) => {
		try {
	...
	createBilibiliRoutes = (cookie, requestConfig = getBilibiliDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(BilibiliMethodRoutes)) router$2.get(path$1, createBilibiliValidationMiddleware(method), createBilibiliRouteHandler(getBilibiliData, method, cookie, requestConfig));
		return router$2;
	};
});
var bilibiliUtils;
...
var init_getdata = __esmMin(() => {
	...
	fetchBilibili = async (data$1, cookie, requestConfig) => {
		...
		switch (data$1.methodType) {
			case "单个视频作品数据": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
			...
			case "指定评论的回复": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.指定评论的回复(data$1)
			});
			case "Emoji数据": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
			...
			case "直播间信息": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.直播间信息({ room_id: data$1.room_id })
			});
			case "直播间初始化信息": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.直播间初始化信息({ room_id: data$1.room_id })
			});
			case "申请二维码": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
			...
			case "登录基本信息": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.登录基本信息()
			});
			case "获取UP主总播放量": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
			...
			case "专栏正文内容": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.专栏正文内容({ id: data$1.id })
			});
			case "专栏显示卡片信息": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.专栏显示卡片信息({ ids: data$1.ids })
			});
			case "专栏文章基本信息": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.专栏文章基本信息({ id: data$1.id })
			});
			case "文集基本信息": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				url: bilibiliApiUrls.文集基本信息({ id: data$1.id })
			});
			case "从_v_voucher_申请_captcha": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				method: "POST",
				url: bilibiliApiUrls.从_v_voucher_申请_captcha(data$1).Url,
				data: bilibiliApiUrls.从_v_voucher_申请_captcha(data$1).Body
			});
			case "验证验证码结果": return await GlobalGetData(data$1.methodType, {
				...baseRequestConfig,
				method: "POST",
...
function getChineseMethodName(platform$1, englishMethod) {
	return {
	...
	}[platform$1][englishMethod];
}
function isEnglishMethodName(platform$1, method) {
	return method in {
		douyin: DouyinMethodReverseMapping,
		bilibili: BilibiliMethodReverseMapping,
		kuaishou: KuaishouMethodReverseMapping,
...
var DouyinMethodMapping, DouyinMethodReverseMapping, BilibiliMethodMapping, BilibiliMethodReverseMapping, KuaishouMethodMapping, KuaishouMethodReverseMapping, XiaohongshuMethodMapping, XiaohongshuMethodReverseMapping;
var init_api_spec = __esmMin(() => {
	...
	DouyinMethodReverseMapping = Object.fromEntries(Object.entries(DouyinMethodMapping).map(([k, v]) => [v, k]));
	...
	BilibiliMethodReverseMapping = Object.fromEntries(Object.entries(BilibiliMethodMapping).map(([k, v]) => [v, k]));
	KuaishouMethodMapping = {
		"单个视频作品数据": "fetchVideoWork",
		"评论数据": "fetchWorkComments",
		"Emoji数据": "fetchEmojiList"
	};
	KuaishouMethodReverseMapping = Object.fromEntries(Object.entries(KuaishouMethodMapping).map(([k, v]) => [v, k]));
	...
	XiaohongshuMethodReverseMapping = Object.fromEntries(Object.entries(XiaohongshuMethodMapping).map(([k, v]) => [v, k]));
});
function printMethodMigrationWarning(platform$1, oldMethod, newMethod) {
	const key = `method:${platform$1}:${oldMethod}`;
	...
	const border = chalk$1.yellow("━".repeat(70));
	const warning = chalk$1.bold.yellow("⚠️  方法名迁移提示");
	const oldApiExample = `get${platform$1.charAt(0).toUpperCase() + platform$1.slice(1)}Data('${oldMethod}', options)`;
	const newApiExample = `${platform$1}Fetcher.${newMethod}(options)`;
	console.warn(`\n${border}\n${warning}\n\n${chalk$1.bold("平台:")} ${platformNames[platform$1]} (${platform$1})\n${chalk$1.bold("旧方法名:")} ${chalk$1.red(`'${oldMethod}'`)} ${chalk$1.gray("(中文，已废弃)")}\n${chalk$1.bold("新方法名:")} ${chalk$1.green(`'${newMethod}'`)} ${chalk$1.gray("(英文，推荐)")}\n\n${chalk$1.bold("旧用法:")}\n  ${chalk$1.red.dim(oldApiExample)}\n\n${chalk$1.bold("新用法:")}\n  ${chalk$1.green(`import { ${platform$1}Fetcher } from '@ikenxuan/amagi'`)}\n  ${chalk$1.green(newApiExample)}\n\n${chalk$1.gray("v6 版本仍支持中文方法名，但建议迁移到英文方法名以获得更好的类型提示。")}\n${border}\n`);
}
function printEnglishMethodNotSupportedWarning(platform$1, englishMethod, chineseMethod) {
	const key = `english:${platform$1}:${englishMethod}`;
	if (shownWarnings.has(key)) return;
	shownWarnings.add(key);
	const border = chalk$1.yellow("━".repeat(70));
	const warning = chalk$1.bold.yellow("⚠️  方法名错误");
	console.warn(`\n${border}\n${warning}\n\n${chalk$1.bold("您使用了英文方法名:")} ${chalk$1.red(`'${englishMethod}'`)}\n${chalk$1.bold("旧 API 仅支持中文方法名:")} ${chalk$1.green(`'${chineseMethod}'`)}\n\n${chalk$1.bold("方案 1 - 使用中文方法名 (兼容旧版):")}\n  ${chalk$1.cyan(`get${platform$1.charAt(0).toUpperCase() + platform$1.slice(1)}Data('${chineseMethod}', options)`)}\n\n${chalk$1.bold("方案 2 - 使用新 API (推荐):")}\n  ${chalk$1.green(`import { ${platform$1}Fetcher } from '@ikenxuan/amagi'`)}\n  ${chalk$1.green(`${platform$1}Fetcher.${englishMethod}(options)`)}\n\n${border}\n`);
}
var chalk$1, shownWarnings;
var init_deprecation = __esmMin(() => {
	init_source();
	chalk$1 = new Chalk();
	shownWarnings = /* @__PURE__ */ new Set();
});
async function getDouyinData(methodType, optionsOrCookie, cookieOrOptions, requestConfig) {
	try {
		let actualMethodType = methodType;
		if (isEnglishMethodName("douyin", actualMethodType)) {
			...
			const eng = DouyinMethodMapping[actualMethodType];
			if (eng) printMethodMigrationWarning("douyin", actualMethodType, eng);
		}
		let options;
		let cookie;
		let config$1;
		if (typeof optionsOrCookie === "string") {
			...
			config$1 = requestConfig;
		} else {
			options = optionsOrCookie;
			cookie = cookieOrOptions;
			config$1 = requestConfig;
		}
		const { typeMode: _$1, ...validationOptions } = options ?? {};
		const rawData = await DouyinData({ ...validateDouyinParams(actualMethodType, validationOptions) }, cookie, config$1);
		if (rawData.data === "" || rawData.status_code !== 0) return createErrorResponse(rawData.amagiError, rawData.status_msg ?? "抖音数据获取失败");
		return createSuccessResponse$1(rawData, "获取成功", 200);
	} catch (error) {
		const errorMessage = error instanceof Error ? error.message : "未知错误";
		throw new Error(`抖音数据获取失败: ${errorMessage}`);
	}
...
async function getBilibiliData(methodType, optionsOrCookie, cookieOrOptions, requestConfig) {
	try {
		...
		if (isEnglishMethodName("bilibili", actualMethodType)) {
			...
			const eng = BilibiliMethodMapping[actualMethodType];
			if (eng) printMethodMigrationWarning("bilibili", actualMethodType, eng);
		}
		let options;
		let cookie;
		let config$1;
		...
		const { typeMode: _$1, ...validationOptions } = options ?? {};
		const rawData = await fetchBilibili({ ...validateBilibiliParams(actualMethodType, validationOptions) }, cookie, config$1);
...
async function getKuaishouData(methodType, optionsOrCookie, cookieOrOptions, _requestConfig) {
	try {
		let actualMethodType = methodType;
		if (isEnglishMethodName("kuaishou", actualMethodType)) {
			...
			const eng = KuaishouMethodMapping[actualMethodType];
			if (eng) printMethodMigrationWarning("kuaishou", actualMethodType, eng);
		}
		let options;
		let cookie;
		if (typeof optionsOrCookie === "string") {
		...
		const { typeMode: _$1, ...validationOptions } = options ?? {};
		const rawData = await KuaishouData({ ...validateKuaishouParams(actualMethodType, validationOptions) }, cookie);
...
async function getXiaohongshuData(methodType, optionsOrCookie, cookieOrOptions, _requestConfig) {
	try {
		let actualMethodType = methodType;
		if (isEnglishMethodName("xiaohongshu", actualMethodType)) {
			...
			const eng = XiaohongshuMethodMapping[actualMethodType];
			if (eng) printMethodMigrationWarning("xiaohongshu", actualMethodType, eng);
		}
		let options;
		let cookie;
		if (typeof optionsOrCookie === "string") {
		...
		const { typeMode: _$1, ...validationOptions } = options ?? {};
		const rawData = await XiaohongshuData({ ...validateXiaohongshuParams(actualMethodType, validationOptions) }, cookie);
...
var createDouyinApiMethod, createBoundDouyinApiMethod, douyin$1, createBoundDouyinApi;
var init_DouyinApi = __esmMin(() => {
	init_DataFetchers();
	createDouyinApiMethod = (methodType) => async (options, cookie, requestConfig) => await getDouyinData(methodType, options, cookie, requestConfig);
	createBoundDouyinApiMethod = (methodType, cookie, requestConfig) => async (options) => await getDouyinData(methodType, options, cookie, requestConfig);
	douyin$1 = {
		...
		invoke: async (methodType, options, cookie, requestConfig) => await getDouyinData(methodType, options, cookie, requestConfig)
	};
	createBoundDouyinApi = (cookie, requestConfig) => ({
		...
		invoke: async (methodType, options) => await getDouyinData(methodType, options, cookie, requestConfig)
	});
});
...
var init_routes$2 = __esmMin(() => {
	...
	createDouyinRouteHandler = (dataFetcher, methodType, cookie, requestConfig = getDouyinDefaultConfig(cookie)) => async (req, res) => {
		try {
			const result = await dataFetcher(methodType, req.validatedParams, cookie, requestConfig);
			res.json({
				...result,
	...
	createDouyinRoutes = (cookie, requestConfig = getDouyinDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(DouyinMethodRoutes)) router$2.get(path$1, createDouyinValidationMiddleware(method), createDouyinRouteHandler(getDouyinData, method, cookie, requestConfig));
		return router$2;
	};
});
var douyinUtils;
...
var createKuaishouApiMethod, createBoundKuaishouApiMethod, kuaishou$1, createBoundKuaishouApi;
var init_KuaishouApi = __esmMin(() => {
	init_DataFetchers();
	createKuaishouApiMethod = (methodType) => async (options, cookie) => await getKuaishouData(methodType, options, cookie);
	createBoundKuaishouApiMethod = (methodType, cookie) => async (options) => await getKuaishouData(methodType, options, cookie);
	kuaishou$1 = {
	...
	createBoundKuaishouApi = (cookie, requestConfig) => ({
...
var init_routes$1 = __esmMin(() => {
	...
	createKuaishouRouteHandler = (dataFetcher, methodType, cookie, requestConfig = getKuaishouDefaultConfig(cookie)) => async (req, res) => {
		try {
			const result = await dataFetcher(methodType, req.validatedParams, cookie, requestConfig);
			res.json({
				...result,
	...
	createKuaishouRoutes = (cookie, requestConfig = getKuaishouDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(KuaishouMethodRoutes)) router$2.get(path$1, createKuaishouValidationMiddleware(method), createKuaishouRouteHandler(getKuaishouData, method, cookie, requestConfig));
		return router$2;
	};
});
var kuaishouUtils;
...
var createXiaohongshuApiMethod, createBoundXiaohongshuApiMethod, xiaohongshu$1, createBoundXiaohongshuApi;
var init_XiaohongshuApi = __esmMin(() => {
	init_DataFetchers();
	createXiaohongshuApiMethod = (methodType) => async (options, cookie, requestConfig) => await getXiaohongshuData(methodType, options, cookie, requestConfig);
	createBoundXiaohongshuApiMethod = (methodType, cookie, requestConfig) => async (options) => await getXiaohongshuData(methodType, options, cookie, requestConfig);
	xiaohongshu$1 = {
	...
	createBoundXiaohongshuApi = (cookie, requestConfig) => ({
...
var init_routes = __esmMin(() => {
	...
	createXiaohongshuRouteHandler = (dataFetcher, methodType, cookie, requestConfig = getXiaohongshuDefaultConfig(cookie)) => async (req, res) => {
		try {
			const result = await dataFetcher(methodType, req.validatedParams, cookie, requestConfig);
			res.json({
				...result,
	...
	createXiaohongshuRoutes = (cookie, requestConfig = getXiaohongshuDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(XiaohongshuMethodRoutes)) router$2.get(path$1, createXiaohongshuValidationMiddleware(method), createXiaohongshuRouteHandler(getXiaohongshuData, method, cookie, requestConfig));
		return router$2;
	};
});
...
var init_server = __esmMin(() => {
	...
	createAmagiClient = (options) => {
		...
		const getDouyinDataWithCookie = async (methodType, options$1) => await getDouyinData(methodType, options$1, douyinCookie, requestConfig);
		const getBilibiliDataWithCookie = async (methodType, options$1) => await getBilibiliData(methodType, options$1, bilibiliCookie, requestConfig);
		const getKuaishouDataWithCookie = async (methodType, options$1) => await getKuaishouData(methodType, options$1, kuaishouCookie, requestConfig);
		const getXiaohongshuDataWithCookie = async (methodType, options$1) => await getXiaohongshuData(methodType, options$1, xiaohongshuCookie, requestConfig);
		return {
			startServer,
			events: amagiEvents,
...
var init_amagiClient = __esmMin(() => {
	...
	AmagiBase = class {
		...
		wrapAmagiClient = (client) => new Proxy(client, { get(target, prop) {
...
async function getResolution$1(path$1) {
	try {
...
async function getFrameRate$1(path$1) {
	try {
...
async function burnBiliDanmaku(videoPath, danmakuList, outputPath, options = {}) {
	...
	const resolution = await getResolution$1(videoPath);
	const frameRate = await getFrameRate$1(videoPath);
...
async function getResolution(path$1) {
	try {
		const { stdout } = await ffprobe(`-v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 "${path$1}"`);
		const [w, h] = stdout.trim().split("x").map(Number);
		if (w && h) return {
...
async function getFrameRate(path$1) {
	try {
		const { stdout } = await ffprobe(`-v error -select_streams v:0 -show_entries stream=r_frame_rate -of default=noprint_wrappers=1:nokey=1 "${path$1}"`);
		const [num, den] = stdout.trim().split("/").map(Number);
		if (den > 0) return num / den;
...
var ERROR_CODE_MAP, getErrorDescription, sanitizeIP, sanitizeHeaders, baseHeaders, Networks;
var init_Networks = __esmMin(() => {
	ERROR_CODE_MAP = {
		ECONNRESET: "连接被重置",
		ECONNREFUSED: "连接被拒绝",
	...
	sanitizeHeaders = (headers) => {
		...
		for (const [key, value] of Object.entries(headers)) {
			...
			if (sensitiveKeys.some((sk) => lowerKey.includes(sk))) sanitized[key] = "[敏感信息......]";
			else if (ipSensitiveKeys.some((sk) => lowerKey.includes(sk))) sanitized[key] = sanitizeIP(String(value));
	...
	Networks = class {
		...
		constructor(data$1) {
			...
			this.axiosInstance.interceptors.response.use(void 0, async (error) => {
				...
				if (!this.isRecoverableNetworkError(error)) return Promise.reject(error);
		...
		isRecoverableNetworkError(error) {
			...
			if (error?.code && recoverableErrorCodes.includes(error.code)) return true;
			if (error instanceof AxiosError) {
				...
				if (recoverableKeywords.some((keyword) => errorMessage.includes(keyword.toLowerCase()))) return true;
			}
			if (error instanceof Error) {
				...
				if ([
					...
					"ECONNRESET",
					"socket hang up"
				].some((keyword) => errorMessage.includes(keyword.toLowerCase()))) return true;
			}
			return false;
		}
		async downloadStream(progressCallback, retryCount = 0) {
			...
			try {
				...
				clearTimeout(timeoutId);
				if (intervalId) clearInterval(intervalId);
				const isRecoverableError$1 = this.isRecoverableNetworkError(error);
				const errorDesc = getErrorDescription(error);
				if (error instanceof AxiosError) {
					const sanitized = sanitizeHeaders(this.headers);
					logger.error(`请求在 ${this.timeout / 1e3} 秒后超时或失败: ${errorDesc}, URL: ${this.url}, Headers: ${JSON.stringify(sanitized)}`);
				} else logger.error(`下载失败: ${errorDesc}`);
				const nextDelay = Math.max(1e3, Math.min(2 ** retryCount * 1e3, 8e3));
				if (retryCount < this.maxRetries) {
					if (isRecoverableError$1 && fs.existsSync(this.filepath)) {
					...
					if (fs.existsSync(this.filepath)) {
						const stats = fs.statSync(this.filepath);
						if (isRecoverableError$1 && stats.size > 0) {
					...
					const sanitized = sanitizeHeaders(this.headers);
					const errorDesc$1 = getErrorDescription(error);
					throw new Error(`在 ${this.maxRetries} 次尝试后下载失败: ${errorDesc$1}, URL: ${this.url}, Headers: ${JSON.stringify(sanitized)}`);
				}
			}
		}
		async getfetch() {
		...
		async getLongLink(url$1 = "") {
			const targetUrl = this.url || url$1;
			try {
				new URL(targetUrl);
			} catch {
				const sanitized = sanitizeHeaders(this.headers);
			...
			try {
				...
				const sanitized = sanitizeHeaders(this.headers);
				const msg = `获取链接重定向失败: ${targetUrl} - ${getErrorDescription(axiosError)}, Headers: ${JSON.stringify(sanitized)}`;
				logger.error(msg);
				throw new Error(msg);
			}
		}
		...
		async getData() {
			try {
				...
				if (error instanceof AxiosError) {
					const sanitized = sanitizeHeaders(this.headers);
					const errorMsg = `${getErrorDescription(error)}, URL: ${this.url}, Headers: ${JSON.stringify(sanitized)}`;
...
var init_parse$1 = __esmMin(() => {
	...
	parseDrawDynamic = (data$1) => {
		...
		const images = (moduleDynamic.major?.opus?.pics || []).map((pic) => pic.url).filter((url$1) => typeof url$1 === "string");
		return {
			title: "图文动态",
			description,
			thumbnail: images[0] || "",
...
var init_link = __esmMin(() => {
	detectPlatform = (url$1) => {
		if (url$1.includes("douyin.com") || url$1.includes("iesdouyin.com") || url$1.includes("webcast.amemv.com") || url$1.includes("live.douyin.com")) return "douyin";
		if (url$1.includes("bilibili.com") || url$1.includes("b23.tv")) return "bilibili";
		return "unknown";
	};
	extractWorkId = (url$1, platform$1) => {
		if (platform$1 === "douyin") {
			const videoMatch = /video\/(\d+)/.exec(url$1);
			if (videoMatch) return {
				type: "video",
				id: videoMatch[1]
			};
			const noteMatch = /note\/(\d+)/.exec(url$1);
			if (noteMatch) return {
				type: "note",
				id: noteMatch[1]
			};
			const modalMatch = /modal_id=(\d+)/.exec(url$1);
		...
		if (platform$1 === "bilibili") {
			const bvidMatch = /\/video\/(BV[a-zA-Z0-9]+)/.exec(url$1);
			if (bvidMatch) return {
				type: "video",
				id: bvidMatch[1]
			};
			const aidMatch = /\/video\/av(\d+)/.exec(url$1);
			if (aidMatch) return {
				type: "video",
				id: aidMatch[1]
			};
			const tMatch = /^https:\/\/t\.bilibili\.com\/(\d+)/.exec(url$1);
			if (tMatch) return {
				type: "dynamic",
				id: tMatch[1]
			};
			const opusMatch = /\/opus\/(\d+)/.exec(url$1);
			if (opusMatch) return {
				type: "dynamic",
				id: opusMatch[1]
			};
...
var $var, $literal, $compare, $not, $or, $includes, $eq, $ne;
var init_schema$1 = __esmMin(() => {
	...
	$eq = (field, value) => $compare(field, "===", value);
	$ne = (field, value) => $compare(field, "!==", value);
});
var appConfigSchema;
...
var import_lib, import_lib$1, import_dist, server, proxyOptions, app$1, staticRouter;
var init_Register = __esmMin(async () => {
	await init_src();
	import_lib = __toESM(require_lib(), 1);
	import_lib$1 = __toESM(require_lib$1(), 1);
	...
	if (process.env.NODE_ENV !== "test") checkPort(3780).then((isOpen) => {
		if (isOpen) {
			...
			s.on("error", (err) => {
				logger$1.error(`[karin-plugin-kkk] 字体代理服务器启动失败: ${err.message}`);
			});
			return s;
		}
		return logger$1.error("端口 3780 被占用，字体代理服务器将不会启动。");
	});
	...
	if (Config.app.APIServer && Config.app.APIServerMount) {
		app$1.use(logMiddleware(["/api/bilibili", "/api/douyin"]));
...
var strategies, registerErrorStrategy, parseLogsToStructured, statBotId, renderErrorImage, sendErrorToMaster, handleBusinessError, wrapWithErrorHandler;
var init_ErrorHandler = __esmMin(async () => {
	...
	sendErrorToMaster = async (ctx, img$2, customPrefix) => {
		...
		const botId = isPushTask ? statBotId(Config.pushlist).douyin.botId || statBotId(Config.pushlist).bilibili.botId : event?.bot?.selfId;
		if (!botId || !master) return;
		try {
			const prefix$1 = customPrefix || (isPushTask ? `${options.businessName} 任务执行出错！\n请即时解决以消除警告` : `群：${(await karin$1.getBot(botId)?.getGroupInfo(event && "groupId" in event ? event.groupId : ""))?.groupName || "未知"}(${event && "groupId" in event ? event.groupId : ""})\n${options.businessName} 任务执行出错！\n请即时解决以消除警告`);
	...
	handleBusinessError = async (error, options, logs$1, event) => {
		try {
			...
			for (const strategy of strategies) if (strategy.match(ctx)) {
			...
			if (Config.app.errorLogSendTo.some((item) => item === "master")) {
				...
				const botId = isPushTask ? statBotId(Config.pushlist).douyin.botId || statBotId(Config.pushlist).bilibili.botId : event?.bot?.selfId;
				if (botId && master) {
					const prefix$1 = isPushTask ? `${options.businessName} 任务执行出错！\n请即时解决以消除警告` : `群：${(await karin$1.getBot(botId)?.getGroupInfo(event && "groupId" in event ? event.groupId : ""))?.groupName || "未知"}(${event && "groupId" in event ? event.groupId : ""})\n${options.businessName} 任务执行出错！\n请即时解决以消除警告`;
					await karin$1.sendMaster(botId, master, [segment.text(prefix$1), ...img$2]);
				}
			}
...
const BilibiliWeb = (all) => [components.accordion.create("bilibili", {
	label: "B站相关",
...
const DouyinWeb = (all) => [components.accordion.create("douyin", {
	label: "抖音相关",
...
const KuaishouWeb = (all) => [components.accordion.create("kuaishou", {
	label: "快手相关",
...
const XiaohongshuWeb = (all) => [components.accordion.create("xiaohongshu", {
	label: "小红书相关",
...
const extractArticleImages = (content) => {
	...
	if (content.opus?.content?.paragraphs) {
		for (const paragraph of content.opus.content.paragraphs) if (paragraph.para_type === 2 && paragraph.pic?.pics) {
			for (const pic of paragraph.pic.pics) if (pic.url) {
				const url$1 = pic.url.startsWith("//") ? `https:${pic.url}` : pic.url;
				images.push(url$1);
			}
		}
	}
	if (content.content && typeof content.content === "string") {
		...
		while ((match = imgRegex.exec(content.content)) !== null) {
			let url$1 = match[1];
			if (url$1.startsWith("//")) url$1 = `https:${url$1}`;
			images.push(url$1);
		}
	}
	return images;
...
init_src();
const getBilibiliID = async (url$1) => {
	...
	switch (true) {
		...
		case /(video\/|video-)([A-Za-z0-9]+)/.test(longLink): {
			...
			if (bvid && bvid.toLowerCase().startsWith("av")) {
				...
				bvid = (await Client.bilibili.api.convertAvToBv({
					avid,
					typeMode: "strict"
				})).data.data.bvid;
			}
					...
					if ((this.forceBurnDanmaku || Config.douyin.burnDanmaku) && danmakuList.length > 0) {
						...
						if (videoFile.filepath) {
							...
							if (await burnDouyinDanmaku(videoFile.filepath, danmakuList, resultPath, {
								...
								const stats = fs.statSync(filePath);
								const fileSizeInMB = Number((stats.size / (1024 * 1024)).toFixed(2));
								const { uploadFile: uploadFile$1 } = await init_utils().then(() => utils_exports);
...
const getDouyinID = async (event, url$1, log = true) => {
	const longLink = (await axios.get(url$1, { headers: { "User-Agent": "Apifox/1.0.0 (https://apifox.com)" } })).request.res.responseUrl;
	let result = {};
	switch (true) {
...
await init_module();
const getKuaishouID = async (url$1, log = true) => {
	const longLink = await new Networks({ url: url$1 }).getLongLink();
	let result = {};
	switch (true) {
...
const douyinLogin = async (e) => {
	...
	try {
		...
		page.on("request", async (request) => {
			...
			if (url$1.includes("passport") || url$1.includes("login") || url$1.includes("qrconnect") || url$1.includes("qrcode")) logger.debug(`[请求] ${resourceType}: ${url$1}`);
			if (resourceType === "media" || resourceType === "font" || resourceType === "stylesheet" || /\.(mp4|webm|m3u8|flv|avi|mov|wmv|mkv)(\?|$)/i.test(url$1) || url$1.includes("/aweme/") || url$1.includes("/video/") || url$1.includes("v.douyin.com") || resourceType === "image" && !url$1.includes("qrcode") && !url$1.includes("data:image") && (url$1.includes(".jpg") || url$1.includes(".jpeg") || url$1.includes(".webp"))) {
				if (url$1.includes("passport") || url$1.includes("login") || url$1.includes("qrconnect")) logger.warn(`[拦截] 登录相关请求被拦截: ${url$1}`);
				request.abort();
		...
		try {
			...
			qrCodeBase64 = await Promise.race([waitQrcode(page), new Promise((_$1, reject) => setTimeout(() => reject(/* @__PURE__ */ new Error("QR_CODE_TIMEOUT")), 6e4))]);
			logger.mark("二维码获取成功");
		} catch (error) {
		...
		try {
			...
			const loginResult = await Promise.race([new Promise((resolve$1) => {
				...
				page.on("response", async (response) => {
					responseCount++;
					const url$1 = response.url();
					if (responseCount % 10 === 0) logger.debug(`[心跳] 已收到 ${responseCount} 个响应`);
					if (url$1.includes("passport") || url$1.includes("login") || url$1.includes("qrconnect") || url$1.includes("qrcode")) logger.debug(`[响应] 登录相关请求: ${url$1}, status: ${response.status()}`);
				});
...
var handleBilibili = wrapWithErrorHandler(async (e) => {
	...
	if (urlMatch) url$1 = urlMatch[0];
	else if (bvRegex.test(e.msg)) url$1 = `https://www.bilibili.com/video/${e.msg}`;
	else if (avRegex.test(e.msg)) url$1 = `https://www.bilibili.com/video/${e.msg}`;
	if (!url$1) {
...
var handleXiaohongshu = wrapWithErrorHandler(async (e) => {
	const url$1 = e.msg.replaceAll("\\", "").match(/https?:\/\/[^\s"'<>]+/)?.[0];
	if (!url$1) {
		logger.warn(`未能在消息中找到有效链接: ${e.msg}`);
		return true;
	}
	const iddata = await getXiaohongshuID(url$1);
