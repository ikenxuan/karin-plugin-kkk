...
var isPortOccupied = (port) => new Promise((resolve$1) => {
	const server$1 = net.createServer();
	server$1.once("error", (err) => {
		if (err.code === "EADDRINUSE") resolve$1(true);
		else resolve$1(false);
	});
	server$1.once("listening", () => {
		server$1.close();
		resolve$1(false);
	});
	server$1.listen(port);
});
isPortOccupied(3780).then((isOccupied) => {
	if (isOccupied) {
		logger$1.error("端口 3780 被占用，代理服务器将不会启动。");
		return;
	}
	server.listen(3780);
});
