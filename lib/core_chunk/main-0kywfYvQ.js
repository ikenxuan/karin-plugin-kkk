...
import { C as Client, D as DynamicType, g as getBilibiliData, b as bilibiliApiUrls, w as wbi_sign, M as MajorType, a as getDouyinData, h as heicConvertExports, l as logger$1 } from "./vendor-IyMLJerg.js";
import karin, { copyConfigSync, filesByExt, watch, logger, requireFileSync, segment, createNotFoundResponse, ffmpeg, ffprobe, karinPathHtml, render, common, createBadRequestResponse, createServerErrorResponse, createSuccessResponse, getBot, defineConfig, components, mkdirSync, config, range, karinPathTemp as karinPathTemp$1 } from "node-karin";
import axios, { AxiosError } from "node-karin/axios";
import { pipeline } from "stream/promises";
import path, { join } from "node:path";
import reactServerRender from "./template.js";
import { getDouyinDB, getBilibiliDB, cleanOldDynamicCache, bilibiliDBInstance, douyinDBInstance } from "./db-Bl5NdrPS.js";
...
` + dynamicInfo.data.data.item.modules.module_dynamic.major.opus.summary.text;
            ...
            this.e.reply(await Render("bilibili/dynamic/DYNAMIC_TYPE_DRAW", {
              ...
              text: dynamicInfo.data.data.item.modules.module_dynamic.major ? replacetext(
                br$3(dynamicInfo.data.data.item.modules.module_dynamic.major.opus?.summary?.text ?? ""),
          ...
          case DynamicType.FORWARD: {
            const text = replacetext(
              br$3(dynamicInfo.data.data.item.modules.module_dynamic.desc.text),
              ...
              commentsData && Config.bilibili.sendContent.some((item) => item === "comment") && this.e.reply(
                await Render("bilibili/comment", {
                  ...
                  CommentLength: String(
                    bilibiliComments(commentsData.data, dynamicInfo.data.data.item.modules.module_author.mid.toString())?.length ? bilibiliComments(commentsData.data, dynamicInfo.data.data.item.modules.module_author.mid.toString()).length : 0
                  ),
...
const formatVideoStats$1 = (view, danmaku, like, coin, share, favorite) => {
  ...
  const line1 = alignTwoColumns$1(viewText, danmakuText, maxFirstColLength);
  const line2 = alignTwoColumns$1(likeText, coinText, maxFirstColLength);
  const line3 = alignTwoColumns$1(shareText, favoriteText, maxFirstColLength);
  return `${line1}
${line2}
${line3}`;
};
const alignTwoColumns$1 = (col1, col2, targetLength) => {
  const col1Width = getStringDisplayWidth$1(col1);
  const spacesNeeded = targetLength - col1Width + 5;
  return col1 + " ".repeat(spacesNeeded) + col2;
};
const getStringDisplayWidth$1 = (str) => {
  let width = 0;
...
const videoStreamRouter = (req, res) => {
  ...
  try {
    ...
    if (range2) {
      const parts = range2.replace(/bytes=/, "").split("-");
          ...
          components.accordion.createItem("cfg:app", {
            ...
            children: [
              ...
              components.input.number("multiPageHeight", {
                ...
                rules: [
                  { min: 1e3, max: 2e4, error: "请输入一个范围在 1000 到 20000 之间的数字" }
                ]
              })
            ]
          })
...
const processCommentEmojis$1 = (text, emojiData) => {
...
const processAtUsers$1 = async (text, userIds) => {
...
async function douyinComments(data, emojidata) {
  ...
  for (const comment of data.data.comments) {
    ...
    if (digg_count > 1e4) {
      digg_count = (digg_count / 1e4).toFixed(1) + "w";
    }
  ...
  jsonArray.sort((a, b) => {
    ...
    const bCount = typeof b.digg_count === "string" && b.digg_count.includes("w") ? parseFloat(b.digg_count) * 1e4 : typeof b.digg_count === "number" ? b.digg_count : 0;
...
`),
            ...
            const fixedOrder = ["cover", "title", "author", "stats"];
            fixedOrder.forEach((item) => {
...
function douyinProcessVideos(videos, videoQuality, maxAutoVideoSize) {
  ...
  videosByQuality.forEach((videos2) => {
    videos2.sort((a, b) => b.play_addr.data_size - a.play_addr.data_size);
  });
  if (videoQuality === "adapt") {
    const sizeLimitBytes = (maxAutoVideoSize || Config.upload.filelimit) * 1024 * 1024;
    const qualityPriority2 = ["4k", "2k", "1080p", "720p", "540p"];
    for (const quality of qualityPriority2) {
...
const formatVideoStats = (digg_count, share_count, collect_count, comment_count, recommend_count) => {
...
async function xiaohongshuComments(data, emojiData, useDarkTheme = false) {
  ...
  for (const comment of data.data.comments) {
    ...
    if (comment.sub_comments && Array.isArray(comment.sub_comments)) {
      processedComment.sub_comments = comment.sub_comments.map((subComment) => ({
        id: subComment.id,
  ...
  for (const comment of comments) {
    ...
    if (parseInt(comment.like_count) > 1e4) {
      comment.like_count = (parseInt(comment.like_count) / 1e4).toFixed(1) + "w";
    }
    if (comment.sub_comments && Array.isArray(comment.sub_comments)) {
      for (const subComment of comment.sub_comments) {
        ...
        if (parseInt(subComment.like_count) > 1e4) {
          subComment.like_count = (parseInt(subComment.like_count) / 1e4).toFixed(1) + "w";
        }
      }
    }
  }
...
class Xiaohongshu extends Base {
  e;
  type;
  constructor(e, iddata) {
    super(e);
  ...
  async RESOURCES(data) {
    ...
    if (Config.xiaohongshu.sendContent.some((item) => item === "info")) {
      const noteInfoImg = await Render(
    ...
    if (Config.xiaohongshu.sendContent.some((item) => item === "comment")) {
      ...
      if (!CommentData.data.data.comments || CommentData.data.data.comments.length === 0) {
        ...
        const commentListImg = await Render(
          "xiaohongshu/comment",
          {
...
const getChangelogImage = async (props) => {
  ...
  const htmlPath = `${karinPathTemp$1}/html/${Root.pluginName}/version/CHANGELOG.html`;
