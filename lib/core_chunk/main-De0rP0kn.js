...
import { At as DynamicType, Bt as app$1, Dt as require_lib$1, Et as require_dist, Ft as createKuaishouRoutes, It as init_default, Lt as logMiddleware, Mt as bilibiliApiUrls, Nt as createBilibiliRoutes, Ot as require_lib, Pt as createDouyinRoutes, Rt as logger$1, Vt as init_dist, jt as MajorType, kt as Client, n as require_browser, t as require_heic_convert, zt as wbi_sign } from "./vendor-BHut6Oqc.js";
import { n as init_client, r as reactServerRender } from "./template-C5OPO-g7.js";
	...
	createQrCodePlugin = () => ({
		...
		async beforeRender(ctx) {
			...
			const toDataUrl = async (url$1) => {
				const svg = await import_browser.toString(url$1, {
					type: "svg",
					width: 600,
		...
		switch (this.type) {
			...
			case "user_dynamic": {
				...
				const videos = rawData.data.aweme_list.map((aweme) => {
					...
					let cover$1 = "";
					if (isVideo && aweme.video?.cover) cover$1 = aweme.video.cover.url_list?.[0] || "";
					else if (isVideo && aweme.video?.dynamic_cover) cover$1 = aweme.video.dynamic_cover.url_list?.[0] || "";
					else if (!isVideo && aweme.images && aweme.images.length > 0) cover$1 = aweme.images[0].url_list?.[0] || "";
					else cover$1 = aweme.share_info?.share_url || "";
					return {
