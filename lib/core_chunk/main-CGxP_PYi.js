				...
				switch (dynamicInfo.data.data.item.type) {
					case DynamicType.DRAW: {
						...
						this.e.reply(await Render("bilibili/dynamic/DYNAMIC_TYPE_DRAW", {
							...
							render_time: format$1(/* @__PURE__ */ new Date(), "yyyy-MM-dd HH:mm:ss"),
							dynamicTYPE: "图文动态",
						...
						switch (dynamicInfo.data.data.item.orig.type) {
							case DynamicType.AV:
								data = {
									...
									create_time: format$1(fromUnixTime(dynamicInfo.data.data.item.orig.modules.module_author.pub_ts), "yyyy-MM-dd HH:mm"),
					...
					case DynamicType.AV:
						if (dynamicInfo.data.data.item.modules.module_dynamic.major.type === "MAJOR_TYPE_ARCHIVE") {
							...
							img$1 = await Render("bilibili/dynamic/DYNAMIC_TYPE_AV", {
								...
								create_time: format$1(fromUnixTime(INFODATA.data.data.ctime), "yyyy-MM-dd HH:mm"),
					...
					case DynamicType.LIVE_RCMD: {
						...
						img$1 = await Render("bilibili/dynamic/DYNAMIC_TYPE_LIVE_RCMD", {
							...
							create_time: format$1(fromUnixTime(dynamicInfo.data.data.item.modules.module_author.pub_ts), "yyyy-MM-dd HH:mm"),
							now_time: format$1(/* @__PURE__ */ new Date(), "yyyy-MM-dd HH:mm:ss"),
...
var br$3 = (data) => data = data.replace(/\n/g, "<br>");
...
var br$2 = (data) => {
	for (const i in data) {
		...
		if (data[i].replies && Array.isArray(data[i].replies)) {
			for (const subReply of data[i].replies) if (subReply.message) subReply.message = subReply.message.replace(/\n/g, "<br>");
		}
	}
	return data;
};
...
var getRelativeTimeFromTimestamp$3 = (timestamp) => {
	...
	return format$1(commentDate, "yyyy-MM-dd");
};
				...
				switch (data[dynamicId].dynamic_type) {
					case DynamicType.DRAW:
						...
						img$2 = await Render("bilibili/dynamic/DYNAMIC_TYPE_DRAW", {
							...
							create_time: format$1(fromUnixTime(data[dynamicId].Dynamic_Data.modules.module_author.pub_ts), "yyyy-MM-dd HH:mm"),
						...
						switch (data[dynamicId].Dynamic_Data.orig.type) {
							case DynamicType.AV:
								param = {
									...
									create_time: format$1(fromUnixTime(data[dynamicId].Dynamic_Data.orig.modules.module_author.pub_ts), "yyyy-MM-dd HH:mm"),
...
var br$1 = (data) => data = data.replace(/\n/g, "<br>");
...
const douyinComments = async (data, emojidata) => {
	let jsonArray = [];
	let imageUrls = [];
	...
	for (const comment of data.data.comments) {
		...
		if (replyComment.data.comments.length > 0) {
			const firstReplyComment$1 = replyComment.data.comments[0];
			const replyUserintextlongid = firstReplyComment$1.text_extra && firstReplyComment$1.text_extra[0] && firstReplyComment$1.text_extra[0].sec_uid ? firstReplyComment$1.text_extra.map((extra) => extra.sec_uid) : null;
			replyText = await processAtUsers$1(firstReplyComment$1.text, replyUserintextlongid);
			const replyImageUrl = firstReplyComment$1.image_list?.[0]?.origin_url?.url_list?.[0];
			const replyStickerUrl = firstReplyComment$1.sticker?.animate_url?.url_list?.[0];
			if (replyImageUrl) replyImageList = [replyImageUrl];
			else if (replyStickerUrl) replyImageList = [replyStickerUrl];
		}
		const firstReplyComment = replyComment.data.comments.length > 0 ? replyComment.data.comments[0] : null;
		const commentObj = {
			id: id++,
			replyComment: firstReplyComment ? {
				create_time: getRelativeTimeFromTimestamp$2(firstReplyComment.create_time),
				nickname: firstReplyComment.user.nickname,
				userimageurl: firstReplyComment.user.avatar_thumb.url_list[0],
				text: processCommentEmojis$1(replyText, emojidata),
				digg_count: firstReplyComment.digg_count > 1e4 ? (firstReplyComment.digg_count / 1e4).toFixed(1) + "w" : firstReplyComment.digg_count,
...
var DouYinpush = class extends Base {
		...
		for (const awemeId in data) {
			...
			if (!skip) if (pushItem.living && "room_data" in pushItem.Detail_Data && Detail_Data.live_data) img$2 = await Render("douyin/live", {
				...
				img$2 = await Render("douyin/dynamic", {
					...
					create_time: format$1(fromUnixTime(pushItem.create_time), "yyyy-MM-dd HH:mm"),
					avater_url: "https://p3-pc.douyinpic.com/aweme/1080x1080/" + Detail_Data.user_info.data.user.avatar_larger.uri,
	...
	async setting(data) {
		...
		try {
			...
			for (const userItem of data.user_list) if ((userItem.user_info.unique_id === "" ? userItem.user_info.short_id : userItem.user_info.unique_id) === inputDouyinId) {
...
init_locale();
init_Config();
const kuaishouComments = async (data, emojidata) => {
	let jsonArray = [];
...
var handleTestDouyinPush = wrapWithErrorHandler(async (e) => {
	...
	const img$2 = await Render("douyin/dynamic", {
		...
		create_time: format$1(fromUnixTime(workInfo.data.aweme_detail.create_time), "yyyy-MM-dd HH:mm"),
...
const xiaohongshuComments = async (data, emojiData, useDarkTheme = false) => {
...
export { Root as A, dylogin as C, task as D, setdyck as E, webConfig as O, biLogin as S, setbilick as T, setbiliPush as _, bilibiliAPP as a, help as b, prefix as c, bilibiliPush as d, bilibiliPushList as f, forcePush as g, douyinPushList as h, update as i, init_root as j, web_config_default as k, xiaohongshuAPP as l, douyinPush as m, kkkUpdateCommand as n, douyinAPP as o, changeBotID as p, kkkUpdateTest as r, kuaishouAPP as s, kkkUpdate as t, testVerification as u, setdyPush as v, removeAllFiles as w, version as x, testDouyinPush as y };
