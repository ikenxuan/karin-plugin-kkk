...
async function getVideoBitrate(path$1) {
	...
	try {
		...
		if (bitrate > 0) {
			const kbps = Math.round(bitrate / 1e3);
...
async function mergeAndBurn(videoPath, audioPath, danmakuList, outputPath, options = {}) {
	...
	const result = await ffmpeg(`-y -i "${videoPath}" -i "${audioPath}" -f mp4 -vf "${buildFilter(canvas, assPath)}" -r ${frameRate} ${getEncoderParams(await detectEncoder(videoCodec), sourceBitrate)} -c:a aac -b:a 192k "${outputPath}"`);
...
var ENCODER_PRIORITY, SOFTWARE_FALLBACK, cachedEncoders, toASSColor, toASSTime, estimateWidth, escapeASS, escapeWinPath, isLandscape, MAX_OUTPUT_WIDTH;
