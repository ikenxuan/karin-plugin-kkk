...
import { n as init_client, r as reactServerRender } from "./template-C66o1BRo.js";
	...
	for (const key of configKeys) {
		...
		for (const prop of nestedProps) {
			...
			if (propValue !== void 0 && propValue !== null) {
				setNestedProperty(configObj, (prop.startsWith(`${key}:`) ? prop.slice(key.length + 1) : prop).split(":"), propValue);
				hasValidData = true;
			}
		}
...
const bilibiliComments = (commentsData, host_mid) => {
	...
	if (commentsData.data.top && commentsData.data.top.upper) {
		...
		if (topReply.content && topReply.content.pictures && topReply.content.pictures.length > 0) {
			for (const picture of topReply.content.pictures) if (picture.img_src) image_urls.push(picture.img_src);
		}
	...
	for (const [i, reply] of commentsData.data.replies.entries()) {
		...
		if (reply.content && reply.content.pictures && reply.content.pictures.length > 0) {
			for (const picture of reply.content.pictures) if (picture.img_src) image_urls.push(picture.img_src);
		}
		...
		if (reply.replies && Array.isArray(reply.replies)) for (const subReply of reply.replies) {
			...
			if (subReply.content && subReply.content.pictures && subReply.content.pictures.length > 0) {
				for (const picture of subReply.content.pictures) if (picture.img_src) image_urls.push(picture.img_src);
			}
