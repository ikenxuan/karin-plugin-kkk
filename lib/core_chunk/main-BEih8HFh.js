...
import { At as bilibiliApiUrls, Dt as Client, Et as require_lib, Ft as logMiddleware, It as logger$1, Lt as wbi_sign, Mt as createDouyinRoutes, Nt as createKuaishouRoutes, Ot as DynamicType, Pt as init_default, Rt as app$1, Tt as require_lib$1, jt as createBilibiliRoutes, kt as MajorType, n as require_browser, t as require_heic_convert, wt as require_dist, zt as init_dist } from "./vendor-Diws7lSx.js";
import { n as init_client, r as reactServerRender } from "./template-DQ5cwa9l.js";
...
var AmagiBase, getAmagiClient, getBilibiliData, getDouyinData, getKuaishouData;
var init_amagiClient = __esmMin(() => {
	...
	AmagiBase = class {
		...
		wrapAmagiClient = (client) => new Proxy(client, { get(target, prop) {
			...
			if (typeof method === "function") return async (...args) => {
				...
				const isResultType = (val) => val && typeof val === "object" && "success" in val && typeof val.success === "boolean" && "code" in val && "message" in val;
	...
	getAmagiClient = () => new AmagiBase().amagi;
	getBilibiliData = (...args) => getAmagiClient().getBilibiliData(...args);
	getDouyinData = (...args) => getAmagiClient().getDouyinData(...args);
	getKuaishouData = (...args) => getAmagiClient().getKuaishouData(...args);
});
...
const wrapWithErrorHandler = (fn, options) => async (...args) => {
	...
	try {
		...
		if (Config.app.errorLogSendTo && Config.app.errorLogSendTo.length > 0) await handleBusinessError(error, options, collectedLogs, args[0]);
		throw error;
	} finally {
