...
import { Bt as createKuaishouRoutes, C as fromUnixTime, E as differenceInSeconds, Ft as DynamicType, Gt as app$1, Ht as logMiddleware, It as MajorType, Kt as init_dist, Lt as bilibiliApiUrls, Mt as require_lib$1, Nt as require_lib, Pt as Client, Rt as createBilibiliRoutes, S as init_date_fns, T as format, Ut as logger$1, Vt as init_default, Wt as wbi_sign, b as init_locale, jt as require_dist, n as require_qr_code_styling, r as require_browser, t as require_heic_convert, w as formatDistanceToNow, x as zhCN, zt as createDouyinRoutes } from "./vendor-Db79gRLX.js";
import { n as init_client, r as reactServerRender } from "./template-BnnBgBDf.js";
...
var import_browser, import_qr_code_styling, createQrCodePlugin;
		...
		async beforeRender(ctx) {
			...
			const toDataUrl = async (url$1) => {
				const buffer = await new import_qr_code_styling.default({
					jsdom: JSDOM,
					nodeCanvas: import_browser.default,
					type: "svg",
					shape: "square",
					width: 1200,
					height: 1200,
					...
					backgroundOptions: { color: useDarkTheme ? "#18181B" : "#FAFAFA" },
...
const douyinComments = async (data, emojidata) => {
	...
	for (const comment of data.data.comments) {
		...
		if (replyComment.data.comments.length > 0) {
			const replyUserintextlongid = replyComment.data.comments[0].text_extra && replyComment.data.comments[0].text_extra[0] && replyComment.data.comments[0].text_extra[0].sec_uid ? replyComment.data.comments[0].text_extra.map((extra) => extra.sec_uid) : null;
			replyText = await processAtUsers$1(replyComment.data.comments[0].text, replyUserintextlongid);
		}
		const commentObj = {
			id: id++,
