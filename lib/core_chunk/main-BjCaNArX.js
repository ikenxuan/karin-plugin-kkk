...
}), AmagiBase, amagiClient, getBilibiliData, getDouyinData, getKuaishouData;
...
const wrapWithErrorHandler = (fn, options) => async (e, next) => {
	const ctx = logger.runContext(async () => await fn(e, next));
	try {
		return await ctx.run();
	} catch (error) {
		await new Promise((resolve$1) => setTimeout(resolve$1, 100));
		await handleBusinessError(error, options, parseLogsToStructured(ctx.logs()), e);
		throw error;
	}
};
...
var handleBusinessError = async (error, options, logs$1, event) => {
	try {
		...
		if (Config.app.errorLogSendTo.some((item) => item === "master")) try {
			...
			} else selectedBotId = event?.bot?.selfId || "";
			if (selectedBotId && master) {
				if (isPushTask) await karin.sendMaster(selectedBotId, master, [segment.text(`${options.businessName} 任务执行出错！\n请即时解决以消除警告`), ...img$2]);
				else {
			...
			} else logger.warn(`[ErrorHandler] 无法发送给主人: selectedBotId=${selectedBotId}, master=${master}`);
		} catch (masterError) {
...
export { init_root as A, removeAllFiles as C, webConfig as D, task as E, web_config_default as O, dylogin as S, setdyck as T, setdyPush as _, bilibiliAPP as a, version as b, prefix as c, bilibiliPushList as d, changeBotID as f, setbiliPush as g, forcePush as h, update as i, Root as k, xiaohongshuAPP as l, douyinPushList as m, kkkUpdateCommand as n, douyinAPP as o, douyinPush as p, kkkUpdateTest as r, kuaishouAPP as s, kkkUpdate as t, bilibiliPush as u, testDouyinPush as v, setbilick as w, biLogin as x, help as y };
