...
var getGroupsRouter, getAuthorsRouter, getDouyinContentRouter, getBilibiliContentRouter, addDouyinContentRouter, addBilibiliContentRouter, deleteContentRouter, verifyBilibiliCaptchaRouter;
var init_content_router = __esmMin(async () => {
		...
		try {
			...
			return createSuccessResponse(res, await Promise.all(caches.map(async (cache) => {
				...
				if (cache.douyinUser) {
					...
					try {
						avatarUrl = (await getDouyinData("用户主页数据", {
							sec_uid: cache.douyinUser.sec_uid,
							typeMode: "strict"
						})).data?.user?.avatar_larger?.url_list[0] || "";
					} catch {}
				}
				return {
					id: cache.aweme_id,
	...
	addBilibiliContentRouter = async (req, res) => {
		try {
			...
			if (!await bilibiliDB$1.getBilibiliUser(host_mid)) return createBadRequestResponse(res, "该UP主未在订阅列表中，请先添加订阅");
	...
	deleteContentRouter = async (req, res) => {
		try {
			...
			let affected = 0;
			if (platform$1 === "douyin") affected = (await (await getDouyinDB()).awemeCacheRepository.delete({
				aweme_id: id,
				groupId
			})).affected;
			else if (platform$1 === "bilibili") affected = (await (await getBilibiliDB()).dynamicCacheRepository.delete({
				dynamic_id: id,
				groupId
			})).affected;
			if (affected === 0) return createBadRequestResponse(res, "未找到要删除的内容");
	...
	verifyBilibiliCaptchaRouter = async (req, res) => {
		try {
...
var init_Register = __esmMin(async () => {
	...
	app$1.post("/bilibili/verify", authMiddleware, signatureVerificationMiddleware, verifyBilibiliCaptchaRouter);
