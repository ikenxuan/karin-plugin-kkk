...
import { getDouyinDB, getBilibiliDB, cleanOldDynamicCache, bilibiliDBInstance, douyinDBInstance } from "./db-DcrhyV4F.js";
...
${infoData.data.data.desc || "暂无简介"}`));
            ...
            infoTexts.push(segment.text(`发布时间：${new Date(infoData.data.data.ctime * 1e3).toLocaleString("zh-CN")}`));
...
${VideoData.data.aweme_detail.desc}`));
            ...
            infoTexts.push(segment.text(`发布时间：${new Date(VideoData.data.aweme_detail.create_time * 1e3).toLocaleString("zh-CN")}`));
            if (this.is_mp4) {
