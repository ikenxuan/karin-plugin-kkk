...
import { A as format, Bt as Xhshow, C as init_locale, D as init_date_fns, Gt as init_axios, Ht as init_zod, Jt as init_dist, Kt as axios_default, O as fromUnixTime, Rt as require_express, Ut as zod_default, Vt as init_esm, Wt as AxiosError$1, Xt as init_source, Yt as Chalk, a as require_qr_code_styling, i as require_lib, j as differenceInSeconds, k as formatDistanceToNow, n as require_dist, o as init_lib, qt as dist_exports, r as require_lib$1, s as Window, t as require_heic_convert, w as zhCN, zt as require_protobufjs } from "./vendor-D0RdNWnU.js";
import { n as init_client, r as reactServerRender } from "./template-CXQb5rL4.js";
		...
		for (let attempt = 0; attempt <= maxRetries; attempt++) try {
			...
			if (error instanceof AxiosError$1) {
				...
				if (isRecoverableError(error) && attempt < maxRetries) {
					...
					logger$1.warn(`网络请求失败 [${error.code}]，${delayMs}ms 后进行第 ${attempt + 1} 次重试...`);
					await delay(delayMs);
					continue;
				}
				logger$1.error("网络请求失败:", error.message);
...
var init_BilibiliApi = __esmMin(() => {
	init_DataFetchers();
	createBilibiliApiMethod = (methodType) => async (options, cookie) => await getBilibiliData$1(methodType, options, cookie);
	createBoundBilibiliApiMethod = (methodType, cookie) => async (options) => await getBilibiliData$1(methodType, options, cookie);
	bilibili$1 = {
...
var init_routes$3 = __esmMin(() => {
	...
	createBilibiliRoutes = (cookie, requestConfig = getBilibiliDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(BilibiliMethodRoutes)) router$2.get(path$1, createBilibiliValidationMiddleware(method), createBilibiliRouteHandler(getBilibiliData$1, method, cookie, requestConfig));
		return router$2;
	};
});
var bilibiliUtils;
...
var fetchBilibili, GlobalGetData$3, bilibiliErrorCodeMap;
var init_getdata$3 = __esmMin(() => {
	...
	GlobalGetData$3 = async (type, options, retryCount = 0) => {
		const MAX_RETRIES = 3;
		let warningMessage = "";
		try {
...
var getSignature, getSignParamName, buildSignedUrl, DouyinData, fetchPaginatedData, GlobalGetData$2, parseDouyinMultiJson, filterSearchResponses;
var init_getdata$2 = __esmMin(() => {
	...
	GlobalGetData$2 = async (type, config$1) => {
		let warningMessage = "";
		try {
...
var KuaishouData, GlobalGetData$1;
var init_getdata$1 = __esmMin(() => {
	...
	GlobalGetData$1 = async (type, options) => {
		let warningMessage = "";
		try {
			const result = await fetchData(options);
			if (isNetworkErrorResult(result)) {
...
var XiaohongshuData, GlobalGetData;
var init_getdata = __esmMin(() => {
	...
	GlobalGetData = async (methodType, config$1) => {
		try {
...
async function getDouyinData$1(methodType, optionsOrCookie, cookieOrOptions, requestConfig) {
	try {
		let options;
		let cookie;
		let config$1;
		...
		const rawData = await DouyinData({ ...validateDouyinParams(methodType, validationOptions) }, cookie, config$1);
...
async function getBilibiliData$1(methodType, optionsOrCookie, cookieOrOptions, requestConfig) {
	try {
		let options;
		let cookie;
		let config$1;
		...
		const { typeMode: _$1, ...validationOptions } = options ?? {};
		const rawData = await fetchBilibili({ ...validateBilibiliParams(methodType, validationOptions) }, cookie, config$1);
...
async function getKuaishouData$1(methodType, optionsOrCookie, cookieOrOptions, requestConfig) {
	try {
		let options;
		let cookie;
		if (typeof optionsOrCookie === "string") {
...
async function getXiaohongshuData$1(methodType, optionsOrCookie, cookieOrOptions, requestConfig) {
	try {
		let options;
		let cookie;
		if (typeof optionsOrCookie === "string") {
...
var init_DouyinApi = __esmMin(() => {
	init_DataFetchers();
	createDouyinApiMethod = (methodType) => async (options, cookie, requestConfig) => await getDouyinData$1(methodType, options, cookie, requestConfig);
	createBoundDouyinApiMethod = (methodType, cookie, requestConfig) => async (options) => await getDouyinData$1(methodType, options, cookie, requestConfig);
	douyin$1 = {
		...
		invoke: async (methodType, options, cookie, requestConfig) => await getDouyinData$1(methodType, options, cookie, requestConfig)
	};
	createBoundDouyinApi = (cookie, requestConfig) => ({
		...
		invoke: async (methodType, options) => await getDouyinData$1(methodType, options, cookie, requestConfig)
	});
});
...
var init_routes$2 = __esmMin(() => {
	...
	createDouyinRoutes = (cookie, requestConfig = getDouyinDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(DouyinMethodRoutes)) router$2.get(path$1, createDouyinValidationMiddleware(method), createDouyinRouteHandler(getDouyinData$1, method, cookie, requestConfig));
		return router$2;
	};
});
var douyinUtils;
...
var init_KuaishouApi = __esmMin(() => {
	init_DataFetchers();
	createKuaishouApiMethod = (methodType) => async (options, cookie) => await getKuaishouData$1(methodType, options, cookie);
	createBoundKuaishouApiMethod = (methodType, cookie) => async (options) => await getKuaishouData$1(methodType, options, cookie);
	kuaishou$1 = {
...
var init_routes$1 = __esmMin(() => {
	...
	createKuaishouRoutes = (cookie, requestConfig = getKuaishouDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(KuaishouMethodRoutes)) router$2.get(path$1, createKuaishouValidationMiddleware(method), createKuaishouRouteHandler(getKuaishouData$1, method, cookie, requestConfig));
		return router$2;
	};
});
var kuaishouUtils;
...
var init_XiaohongshuApi = __esmMin(() => {
	init_DataFetchers();
	createXiaohongshuApiMethod = (methodType) => async (options, cookie, requestConfig) => await getXiaohongshuData$1(methodType, options, cookie, requestConfig);
	createBoundXiaohongshuApiMethod = (methodType, cookie, requestConfig) => async (options) => await getXiaohongshuData$1(methodType, options, cookie, requestConfig);
	xiaohongshu$1 = {
...
var init_routes = __esmMin(() => {
	...
	createXiaohongshuRoutes = (cookie, requestConfig = getXiaohongshuDefaultConfig(cookie)) => {
		...
		for (const [method, path$1] of Object.entries(XiaohongshuMethodRoutes)) router$2.get(path$1, createXiaohongshuValidationMiddleware(method), createXiaohongshuRouteHandler(getXiaohongshuData$1, method, cookie, requestConfig));
		return router$2;
	};
});
...
var import_express, createAmagiClient;
var init_server = __esmMin(() => {
	...
	createAmagiClient = (options) => {
		...
		const getDouyinDataWithCookie = async (methodType, options$1) => await getDouyinData$1(methodType, options$1, douyinCookie, requestConfig);
		const getBilibiliDataWithCookie = async (methodType, options$1) => await getBilibiliData$1(methodType, options$1, bilibiliCookie, requestConfig);
		const getKuaishouDataWithCookie = async (methodType, options$1) => await getKuaishouData$1(methodType, options$1, kuaishouCookie, requestConfig);
		const getXiaohongshuDataWithCookie = async (methodType, options$1) => await getXiaohongshuData$1(methodType, options$1, xiaohongshuCookie, requestConfig);
		return {
			startServer,
...
var init_XiaohongshuAPIParams = __esmMin(() => {});
...
var init_src = __esmMin(() => {
	...
	CreateAmagiApp.getDouyinData = getDouyinData$1;
	CreateAmagiApp.getBilibiliData = getBilibiliData$1;
	CreateAmagiApp.getKuaishouData = getKuaishouData$1;
	CreateAmagiApp.getXiaohongshuData = getXiaohongshuData$1;
...
}), AmagiError, AmagiBase, amagiClient, getBilibiliData, getDouyinData, getKuaishouData;
var init_amagiClient = __esmMin(() => {
	...
	getBilibiliData = amagiClient.getBilibiliData.bind(amagiClient);
	getDouyinData = amagiClient.getDouyinData.bind(amagiClient);
	getKuaishouData = amagiClient.getKuaishouData.bind(amagiClient);
	amagiClient.getXiaohongshuData.bind(amagiClient);
});
...
var getAuthors$1;
var init_authors$1 = __esmMin(async () => {
	await init_db();
	await init_amagiClient();
	getAuthors$1 = async (req, res) => {
		try {
			const { groupId } = req.query;
			if (!groupId || typeof groupId !== "string") return createBadRequestResponse(res, "请提供群组ID");
			const validSubscriptions = (await (await getBilibiliDB()).getGroupSubscriptions(groupId)).filter((sub) => sub.bilibiliUser);
			if (validSubscriptions.length === 0) return createSuccessResponse(res, []);
			const authors = [];
...
var init_parse$1 = __esmMin(() => {
	...
	parseVideo = async (req, res) => {
		try {
			...
			const videoDetail = (await getBilibiliData("单个视频作品数据", {
				bvid: bvid || "",
				typeMode: "strict"
			})).data.data;
			const streamData = (await getBilibiliData("单个视频下载信息数据", {
				avid: videoDetail.aid,
				cid: videoDetail.cid,
				typeMode: "strict"
			})).data.data;
			const [commentsResponse, emojiResponse] = await Promise.all([getBilibiliData("评论数据", {
	...
	parseDynamic = async (req, res) => {
		try {
			...
			const dynamicInfo = (await getBilibiliData("动态详情数据", {
				dynamic_id,
				typeMode: "strict"
			})).data;
			const dynamicCard = (await getBilibiliData("动态卡片数据", {
	...
	getUserInfo$1 = async (req, res) => {
		try {
			const { host_mid } = req.query;
			if (!host_mid) return createBadRequestResponse(res, "请提供用户ID (host_mid)");
			const userData = (await getBilibiliData("用户主页数据", {
...
var init_api$2 = __esmMin(async () => {
	...
	router.delete("/contents/:id", deleteContent$1);
	router.get("/authors", getAuthors$1);
	router.post("/parse/video", parseVideo);
	router.post("/parse/dynamic", parseDynamic);
	router.get("/user", getUserInfo$1);
	router.post("/verify", verifyCaptcha);
});
var getAuthors;
var init_authors = __esmMin(async () => {
	await init_db();
	await init_amagiClient();
	getAuthors = async (req, res) => {
		try {
			const { groupId } = req.query;
			if (!groupId || typeof groupId !== "string") return createBadRequestResponse(res, "请提供群组ID");
			const validSubscriptions = (await (await getDouyinDB()).getGroupSubscriptions(groupId)).filter((sub) => sub.douyinUser);
			if (validSubscriptions.length === 0) return createSuccessResponse(res, []);
			const authors = [];
			const batchSize = 5;
...
var formatCount, formatDuration, formatTimestamp, removeTags, processCommentEmojis$2, parseComments, parseWork, getUserInfo;
var init_parse = __esmMin(() => {
	...
	getUserInfo = async (req, res) => {
		try {
			...
			if (!sec_uid || typeof sec_uid !== "string") return createBadRequestResponse(res, "请提供用户ID (sec_uid)");
			const userData = (await getDouyinData("用户主页数据", {
				sec_uid,
				typeMode: "strict"
			})).data.user;
			return createSuccessResponse(res, {
...
var init_api$1 = __esmMin(async () => {
	...
	router$1.delete("/contents/:id", deleteContent);
	router$1.get("/authors", getAuthors);
	router$1.post("/parse", parseWork);
	router$1.get("/user", getUserInfo);
});
		...
		switch (this.Type) {
			case "one_video": {
				...
				if (Config.bilibili.sendContent.some((content) => content === "video")) if (Config.upload.usefilelimit && Number(videoSize) > Number(Config.upload.filelimit) && !Config.upload.compress) this.e.reply(`设定的最大上传大小为 ${Config.upload.filelimit}MB\n当前解析到的视频大小为 ${Number(videoSize)}MB\n视频太大了，还是去B站看吧~`, { reply: true });
					...
					if (this.forceBurnDanmaku || Config.bilibili.burnDanmaku) try {
						...
						const danmakuPromises = Array.from({ length: segmentCount }, (_$1, i) => this.amagi.getBilibiliData("实时弹幕", {
							cid,
							segment_index: i + 1,
							typeMode: "strict"
			...
			case "dynamic_info": {
				...
				switch (dynamicInfo.data.data.item.type) {
					...
					case DynamicType.ARTICLE: {
						const articleInfoBase = await this.amagi.getBilibiliData("专栏文章基本信息", {
		...
		switch (this.type) {
			case "one_work": {
				...
				if (Config.douyin.sendContent.includes("comment")) {
					const douyinCommentsRes = await douyinComments(CommentsData, Emoji((await this.amagi.getDouyinData("Emoji数据", { typeMode: "strict" })).data));
...
const fetchKuaishouData = async (type, opt) => {
	switch (type) {
		case "one_work": return {
			VideoData: await getKuaishouData("单个视频作品数据", {
				photoId: opt.photoId,
				typeMode: "strict"
			}),
			CommentsData: await getKuaishouData("评论数据", {
				photoId: opt.photoId,
				typeMode: "strict"
			}),
			EmojiData: await getKuaishouData("Emoji数据", { typeMode: "strict" })
		};
		case "work_comments": return (await getKuaishouData("评论数据", {
			photoId: opt.photoId,
			typeMode: "strict"
		})).data;
		case "emoji_list": return await getKuaishouData("Emoji数据", { typeMode: "strict" });
		default: break;
	}
};
await init_module();
...
var handleSetDouyinPush = wrapWithErrorHandler(async (e) => {
	...
	const data$1 = await getDouyinData("搜索数据", {
		query,
		type: "用户",
		typeMode: "strict"
	});
...
var handleSetBilibiliPush = wrapWithErrorHandler(async (e) => {
	...
	if (match && match[1]) {
		const data$1 = await getBilibiliData("用户主页数据", {
...
var handleTestDouyinPush = wrapWithErrorHandler(async (e) => {
	...
	const workInfo = await getDouyinData("聚合解析", {
