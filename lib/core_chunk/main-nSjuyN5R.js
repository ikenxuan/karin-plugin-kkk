import { n as __esmMin, o as __toESM, r as __export } from "./rolldown-runtime-B88OHfCz.js";
import { Ct as createKuaishouRoutes, Dt as logMiddleware, Et as init_esm, Ot as logger$1, St as createDouyinRoutes, Tt as getDouyinData, _t as DynamicType, bt as bilibiliApiUrls, gt as Client, ht as require_lib, kt as wbi_sign, mt as require_lib$1, pt as require_dist, t as require_heic_convert, vt as MajorType, wt as getBilibiliData, xt as createBilibiliRoutes, yt as amagi } from "./vendor-BF_bg3y9.js";
import { n as client_default, r as init_client } from "./template-OqJZ0_Dq.js";
...
var pluginPath, pkg, Root, Cfg, configInstance, getConfigInstance, Config, Base, Count, uploadFile, downloadVideo, downloadFile, processFilename, Common, mergeFile, FFmpeg, baseHeaders, Networks, Render, img$1, Bilibili, qnd, generateGradientStyle, cover, generateDecorationCard, oid, bilibiliProcessVideos, getvideosize, formatVideoStats$1, alignTwoColumns$1, getStringDisplayWidth$1, init_root = __esmMin(() => {
	pluginPath = ((startUrl) => {
	...
	})(fileURLToPath(import.meta.url)), pkg = JSON.parse(fs.readFileSync(path.join(pluginPath, "package.json"), "utf-8")), Root = {
		pluginName: pkg.name,
	...
	init_root(), Cfg = class {
		...
		defCfgPath;
		constructor() {
			this.dirCfgPath = `${karinPathBase}/${Root.pluginName}/config`, this.defCfgPath = `${Root.pluginPath}/config/default_config/`;
		}
		initCfg() {
			copyConfigSync(this.defCfgPath, this.dirCfgPath);
			const files = filesByExt(this.dirCfgPath, ".yaml", "name");
			for (const file of files) {
				const config$1 = YAML.parseDocument(fs.readFileSync(`${this.dirCfgPath}/${file}`, "utf8")), defConfig = YAML.parseDocument(fs.readFileSync(`${this.defCfgPath}/${file}`, "utf8")), { differences, result } = this.mergeObjectsWithPriority(config$1, defConfig);
				differences && fs.writeFileSync(`${this.dirCfgPath}/${file}`, result.toString({ lineWidth: -1 }));
			}
			return setTimeout(() => {
		...
		getDefOrConfig(name) {
			return {
				...this.getYaml("default_config", name),
				...this.getYaml("config", name)
			};
		}
		async All() {
			const { getDouyinDB: getDouyinDB$1, getBilibiliDB: getBilibiliDB$1 } = await init_db().then(() => db_exports), douyinDB$1 = await getDouyinDB$1(), bilibiliDB$1 = await getBilibiliDB$1(), allConfig = {}, files = fs.readdirSync(this.defCfgPath);
			for (const file of files) {
			...
			if (allConfig.pushlist) try {
				if (allConfig.pushlist.douyin) for (const item of allConfig.pushlist.douyin) {
					const filterWords = await douyinDB$1.getFilterWords(item.sec_uid), filterTags = await douyinDB$1.getFilterTags(item.sec_uid), userInfo = await douyinDB$1.getDouyinUser(item.sec_uid);
					userInfo && (item.filterMode = userInfo.filterMode || "blacklist"), item.Keywords = filterWords, item.Tags = filterTags;
				}
				if (allConfig.pushlist.bilibili) for (const item of allConfig.pushlist.bilibili) {
					const filterWords = await bilibiliDB$1.getFilterWords(item.host_mid), filterTags = await bilibiliDB$1.getFilterTags(item.host_mid), userInfo = await bilibiliDB$1.getOrCreateBilibiliUser(item.host_mid);
					userInfo && (item.filterMode = userInfo.filterMode || "blacklist"), item.Keywords = filterWords, item.Tags = filterTags;
				}
			} catch (error) {
		...
		getYaml(type, name) {
			return requireFileSync("config" === type ? `${this.dirCfgPath}/${name}.yaml` : `${this.defCfgPath}/${name}.yaml`, { force: !0 });
		}
		Modify(name, key, value, type = "config") {
			const path$1 = "config" === type ? `${this.dirCfgPath}/${name}.yaml` : `${this.defCfgPath}/${name}.yaml`, yamlData = YAML.parseDocument(fs.readFileSync(path$1, "utf8")), keys = key.split(".");
			this.setNestedValue(yamlData.contents, keys, value), fs.writeFileSync(path$1, yamlData.toString({ lineWidth: -1 }), "utf8");
		}
		async ModifyPro(name, config$1, type = "config") {
			const { getDouyinDB: getDouyinDB$1, getBilibiliDB: getBilibiliDB$1 } = await init_db().then(() => db_exports), douyinDB$1 = await getDouyinDB$1(), bilibiliDB$1 = await getBilibiliDB$1(), filePath = "config" === type ? `${this.dirCfgPath}/${name}.yaml` : `${this.defCfgPath}/${name}.yaml`;
			try {
				const existingContent = fs.readFileSync(filePath, "utf8"), doc = YAML.parseDocument(existingContent);
				let filterCfg = config$1;
				if ("pushlist" === name && ("douyin" in config$1 || "bilibili" in config$1)) {
					const cleanedConfig = { ...config$1 };
					"douyin" in cleanedConfig && (cleanedConfig.douyin = cleanedConfig.douyin.map((item) => {
						const { Keywords, Tags, filterMode,...rest } = item;
						return rest;
					})), "bilibili" in cleanedConfig && (cleanedConfig.bilibili = cleanedConfig.bilibili.map((item) => {
						const { Keywords, Tags, filterMode,...rest } = item;
						return rest;
					})), filterCfg = cleanedConfig;
				}
				const newConfigNode = YAML.parseDocument(YAML.stringify(filterCfg)).contents;
				return this.deepMergeYaml(doc.contents, newConfigNode), fs.writeFileSync(filePath, doc.toString({ lineWidth: -1 }), "utf8"), "douyin" in config$1 && (await this.syncFilterConfigToDb(config$1.douyin, douyinDB$1, "sec_uid"), logger.debug("已同步抖音过滤配置到数据库")), "bilibili" in config$1 && (await this.syncFilterConfigToDb(config$1.bilibili, bilibiliDB$1, "host_mid"), logger.debug("已同步B站过滤配置到数据库")), !0;
			} catch (error) {
				return logger.error(`修改配置文件时发生错误：${error}`), !1;
			}
		}
		async syncFilterConfigToDb(items, db$1, idField) {
			for (const item of items) {
				...
				if (id) {
					if (item.filterMode && await db$1.updateFilterMode(id, item.filterMode), item.Keywords && Array.isArray(item.Keywords)) {
						const existingWords = await db$1.getFilterWords(id);
						for (const word of existingWords) item.Keywords.includes(word) || await db$1.removeFilterWord(id, word);
						for (const word of item.Keywords) existingWords.includes(word) || await db$1.addFilterWord(id, word);
					}
					if (item.Tags && Array.isArray(item.Tags)) {
						const existingTags = await db$1.getFilterTags(id);
						for (const tag of existingTags) item.Tags.includes(tag) || await db$1.removeFilterTag(id, tag);
						for (const tag of item.Tags) existingTags.includes(tag) || await db$1.addFilterTag(id, tag);
					}
				}
			}
		}
		deepMergeYaml(target, source) {
			if (YAML.isMap(target) && YAML.isMap(source)) for (const pair of source.items) {
				const key = pair.key, sourceVal = pair.value, targetVal = target.get(key);
				void 0 === targetVal ? target.set(key, sourceVal) : YAML.isMap(targetVal) && YAML.isMap(sourceVal) ? this.deepMergeYaml(targetVal, sourceVal) : YAML.isSeq(targetVal) && YAML.isSeq(sourceVal) ? (targetVal.items = sourceVal.items, targetVal.flow = sourceVal.flow) : target.set(key, sourceVal);
			}
		}
		setNestedValue(map, keys, value) {
			if (1 === keys.length) return void map.set(keys[0], value);
			const subKey = keys[0];
			let subMap = map.get(subKey);
			subMap && YAML.isMap(subMap) || (subMap = new YAML.YAMLMap(), map.set(subKey, subMap)), this.setNestedValue(subMap, keys.slice(1), value);
		}
		mergeObjectsWithPriority(userDoc, defaultDoc) {
			let differences = !1;
			const mergeYamlNodes = (target, source) => {
				if (YAML.isMap(target) && YAML.isMap(source)) for (const pair of source.items) {
					const key = pair.key, value = pair.value, existing = target.get(key);
					void 0 === existing ? (differences = !0, target.set(key, value)) : YAML.isMap(value) && YAML.isMap(existing) ? mergeYamlNodes(existing, value) : existing !== value && (differences = !0, target.set(key, value));
				}
			};
			return mergeYamlNodes(defaultDoc.contents, userDoc.contents), {
				differences,
				result: defaultDoc
			};
		}
		async syncConfigToDatabase() {
			try {
				const { getDouyinDB: getDouyinDB$1, getBilibiliDB: getBilibiliDB$1 } = await init_db().then(() => db_exports), douyinDB$1 = await getDouyinDB$1(), bilibiliDB$1 = await getBilibiliDB$1(), pushCfg = this.getYaml("config", "pushlist");
				pushCfg.bilibili && await bilibiliDB$1.syncConfigSubscriptions(pushCfg.bilibili), pushCfg.douyin && await douyinDB$1.syncConfigSubscriptions(pushCfg.douyin), logger.debug("[BilibiliDB] + [DouyinDB] 配置已同步到数据库");
			} catch (error) {
				logger.error("同步配置到数据库失败:", error);
			}
		}
	}, configInstance = null, getConfigInstance = () => (configInstance || (configInstance = new Proxy(new Cfg().initCfg(), { get: (target, prop) => prop in target ? target[prop] : target.getDefOrConfig(prop) })), configInstance), Config = new Proxy({}, { get: (target, prop) => getConfigInstance()[prop] });
}), init_Base = __esmMin(() => {
	init_esm(), init_utils(), init_Config(), Base = class {
		e;
		headers;
		amagi;
		constructor(e) {
			this.e = e, this.headers = baseHeaders;
			const client = Client({
				cookies: {
				...
				request: {
					...
					proxy: !!Config.request.proxy?.switch && Config.request.proxy
				}
			});
			this.amagi = new Proxy(client, { get(target, prop) {
				const method = target[prop];
				return "function" == typeof method ? async (...args) => {
					const result = await Function.prototype.apply.call(method, target, args);
					return result || (logger.warn(`Amagi API调用 (${String(prop)}) 返回了空值`), result);
				} : method;
			} });
		}
	}, Count = (count) => count > 1e4 ? (count / 1e4).toFixed(1) + "万" : count?.toString() ?? "无法获取", uploadFile = async (event, file, videoUrl, options) => {
		let File, selfId, contact, sendStatus = !0, newFileSize = file.totalBytes;
		if (options?.active ? (selfId = options?.activeOption?.uin, contact = karin.contactGroup(options?.activeOption?.group_id)) : (selfId = event.selfId, contact = event.contact), Config.upload.compress && file.totalBytes > Config.upload.compresstrigger) {
			...
			const message = [segment.text(`视频大小 (${file.totalBytes} MB) 触发压缩条件（设定值：${Config.upload.compresstrigger} MB），正在进行压缩至${Config.upload.compressvalue} MB...`), options?.message_id ? segment.reply(options.message_id) : segment.text("")], msg1 = await karin.sendMsg(selfId, contact, message), targetBitrate = .75 * Common.calculateBitrate(Config.upload.compresstrigger, Duration), startTime = Date.now();
			...
			newFileSize = await Common.getVideoFileSize(file.filepath), logger.debug(`原始视频大小为: ${file.totalBytes.toFixed(1)} MB, ${logger.green(`经 FFmpeg 压缩后最终视频大小为: ${newFileSize.toFixed(1)} MB，原视频文件已删除`)}`);
		...
		options && (options.useGroupFile = Config.upload.usegroupfile && newFileSize > Config.upload.groupfilevalue), Config.upload.sendbase64 && !options?.useGroupFile ? (File = `base64://${(await fs.promises.readFile(file.filepath)).toString("base64")}`, logger.mark(`已开启视频文件 base64转换 正在进行${logger.yellow("base64转换中")}...`)) : File = options?.useGroupFile ? file.filepath : `file://${file.filepath}`;
		try {
			if (options?.active) if (options.useGroupFile) {
				const bot = karin.getBot(String(options.activeOption?.uin));
				logger.mark(`${logger.blue("主动消息:")} 视频大小: ${newFileSize.toFixed(1)}MB 正在通过${logger.yellow("bot.uploadFile")}回复...`), await bot.uploadFile(contact, File, file.originTitle ? `${file.originTitle}.mp4` : `${File.split("/").pop()}`);
			} else logger.mark(`${logger.blue("主动消息:")} 视频大小: ${newFileSize.toFixed(1)}MB 正在通过${logger.yellow("karin.sendMsg")}回复...`), sendStatus = !!(await karin.sendMsg(selfId, contact, [segment.video(File)])).messageId;
			else options?.useGroupFile ? (logger.mark(`${logger.blue("被动消息:")} 视频大小: ${newFileSize.toFixed(1)}MB 正在通过${logger.yellow("e.bot.uploadFile")}回复...`), await event.bot.uploadFile(event.contact, File, file.originTitle ? `${file.originTitle}.mp4` : `${File.split("/").pop()}`)) : (logger.mark(`${logger.blue("被动消息:")} 视频大小: ${newFileSize.toFixed(1)}MB 正在通过${logger.yellow("e.reply")}回复...`), sendStatus = !!(await event.reply(segment.video(File) || videoUrl)).messageId);
			return sendStatus;
		} catch (error) {
			return options && !1 === options.active && await event.reply("视频文件上传失败" + JSON.stringify(error, null, 2)), logger.error("视频文件上传错误," + String(error)), !1;
		} finally {
			const filePath = file.filepath;
			logger.mark(`临时预览地址：http://localhost:${process.env.HTTP_PORT}/api/kkk/video/${encodeURIComponent(filePath.split("/").pop() ?? "")}`), Config.app.removeCache && logger.info(`文件 ${filePath} 将在 10 分钟后删除`), setTimeout(async () => {
		...
		}).getHeaders(), fileSizeInMB = ((fileHeaders["content-range"]?.match(/\/(\d+)/) ? parseInt(fileHeaders["content-range"]?.match(/\/(\d+)/)[1], 10) : 0) / 1048576).toFixed(2), fileSize = parseInt(parseFloat(fileSizeInMB).toFixed(2));
		if (Config.upload.usefilelimit && fileSize > Config.upload.filelimit) {
			const message = segment.text(`视频：「${downloadOpt.title.originTitle ?? "Error: 文件名获取失败"}」大小 (${fileSizeInMB} MB) 超出最大限制（设定值：${Config.upload.filelimit} MB），已取消上传`), selfId = event.selfId || uploadOpt?.activeOption?.uin, contact = event.contact || karin.contactGroup(uploadOpt?.activeOption?.group_id) || karin.contactFriend(selfId);
			return await karin.sendMsg(selfId, contact, message), !1;
		}
		...
		return res = {
			...res,
			...downloadOpt.title
		}, res.totalBytes = Number((res.totalBytes / 1048576).toFixed(2)), await uploadFile(event, res, downloadOpt.video_url, uploadOpt);
	}, downloadFile = async (videoUrl, opt) => {
		const startTime = Date.now(), { filepath, totalBytes } = await new Networks({
			url: videoUrl,
			headers: opt.headers ?? baseHeaders,
			filepath: Common.tempDri.video + opt.title,
			timeout: 3e4
		}).downloadStream((downloadedBytes, totalBytes$1) => {
			const progressPercentage = downloadedBytes / totalBytes$1 * 100, red = Math.floor(255 - 255 * progressPercentage / 100), coloredPercentage = logger.chalk.rgb(red, 255, 0)(`${progressPercentage.toFixed(1)}%`), speed = downloadedBytes / ((Date.now() - startTime) / 1e3), formattedSpeed = (speed / 1048576).toFixed(1) + " MB/s", remainingTime = (totalBytes$1 - downloadedBytes) / speed, formattedRemainingTime = remainingTime > 60 ? `${Math.floor(remainingTime / 60)}min ${Math.floor(remainingTime % 60)}s` : `${remainingTime.toFixed(0)}s`, downloadedSizeMB = (downloadedBytes / 1048576).toFixed(1), totalSizeMB = (totalBytes$1 / 1048576).toFixed(1);
			console.log(`⬇️  ${opt.title} ${function(progressPercentage$1) {
				const filledLength = Math.floor(progressPercentage$1 / 100 * 45);
				let progress = "";
				return progress += "█".repeat(filledLength), progress += "░".repeat(Math.max(0, 45 - filledLength - 1)), `[${progress}]`;
			}(progressPercentage)} ${coloredPercentage} ${downloadedSizeMB}/${totalSizeMB} MB | ${formattedSpeed} 剩余: ${formattedRemainingTime}\r`);
		}, 3);
		return {
			filepath,
			totalBytes
		...
		const nameWithoutExt = filename.substring(0, lastDotIndex), extension = filename.substring(lastDotIndex);
		return nameWithoutExt.substring(0, maxLength).replace(/[\\/:*?"<>|\r\n\s]/g, " ") + "..." + extension;
	};
}), init_Common = __esmMin(() => {
	init_Config(), init_root(), Common = new class {
		tempDri;
		constructor() {
			this.tempDri = {
		...
		async getReplyMessage(e) {
			if (e.replyId) {
				...
				for (const v of reply.elements) {
					if ("text" === v.type) return v.text;
					if ("json" === v.type) return v.data;
				}
			}
			return "";
		}
		chineseToArabic(chineseNumber) {
			...
			for (let i = chineseNumber.length - 1; i >= 0; i--) {
				...
				if (void 0 !== units[char]) unit = units[char], 1e4 !== unit && 1e8 !== unit || (result += temp * unit, temp = 0);
				else {
					const num = chineseToArabicMap[char];
					temp += unit > 1 ? num * unit : num, unit = 1;
				}
			}
			return result + temp;
		}
		formatCookies(cookies) {
			return cookies.map((cookie) => {
				const [nameValue] = cookie.split(";").map((part) => part.trim()), [name, value] = nameValue.split("=");
				return `${name}=${value}`;
			}).join("; ");
		}
		calculateBitrate(targetSizeMB, duration) {
			return 1024 * targetSizeMB * 1024 * 8 / duration / 1024;
		}
		async getVideoFileSize(filePath) {
			try {
				return (await fs.promises.stat(filePath)).size / 1048576;
			} catch (error) {
				throw console.error("获取文件大小时发生错误:", error), error;
			}
		}
		async removeFile(path$1, force = !1) {
			if (path$1 = path$1.replace(/\\/g, "/"), Config.app.removeCache) try {
				return await fs.promises.unlink(path$1), logger.mark("缓存文件: ", path$1 + " 删除成功！"), !0;
			} catch (err) {
				return logger.error("缓存文件: ", path$1 + " 删除失败！", err), !1;
			}
			else if (force) try {
				return await fs.promises.unlink(path$1), logger.mark("缓存文件: ", path$1 + " 删除成功！"), !0;
			} catch (err) {
		...
		getCurrentTime() {
			const now = /* @__PURE__ */ new Date(), year = now.getFullYear(), month = now.getMonth() + 1, day = now.getDate(), hour = now.getHours(), minute = now.getMinutes(), second = now.getSeconds();
		...
		useDarkTheme() {
			...
			if (0 === configTheme) {
				...
				date >= 6 && date < 18 && (dark = !1);
			} else 1 === configTheme ? dark = !1 : 2 === configTheme && (dark = !0);
			return dark;
		}
		timeSince(timestamp) {
			const elapsed = Date.now() - timestamp, seconds = Math.floor(elapsed / 1e3), minutes = Math.floor(seconds / 60), hours = Math.floor(minutes / 60), remainingSeconds = seconds % 60;
			return hours > 0 ? `${hours}小时${minutes % 60}分钟${remainingSeconds}秒` : minutes > 0 ? `${minutes}分钟${remainingSeconds}秒` : `${seconds}秒`;
		}
		validateVideoRequest(filename, res) {
			if (!filename) return createNotFoundResponse(res, "无效的文件名"), null;
			const intendedBaseDir = path.resolve(Common.tempDri.video), requestedPath = path.join(intendedBaseDir, filename), resolvedPath = path.normalize(requestedPath);
			return !resolvedPath.startsWith(intendedBaseDir + path.sep) || filename.includes("/") || filename.includes("\\") ? (logger.warn(`潜在的路径穿越尝试或无效文件名: ${filename}, 解析路径: ${resolvedPath}`), createNotFoundResponse(res, "无效的文件名或路径"), null) : path.basename(filename) !== filename ? (logger.warn(`文件名包含路径分隔符: ${filename}`), createNotFoundResponse(res, "无效的文件名"), null) : fs.existsSync(resolvedPath) ? resolvedPath : (createNotFoundResponse(res, "视频文件未找到"), null);
		}
		count(num) {
			return num > 1e4 ? (num / 1e4).toFixed(1) + "万" : num.toString();
		}
	}();
}), init_FFmpeg = __esmMin(() => {
	init_utils(), mergeFile = async (type, options) => await new FFmpeg(type).FFmpeg(options), FFmpeg = class {
		type;
		constructor(type) {
			this.type = type;
		}
		async FFmpeg(opt) {
			switch (this.type) {
				case "二合一（视频 + 音频）": {
					...
					return result.status ? logger.mark(`视频合成成功！文件地址：${opt.resultPath}`) : logger.error(result), await opt.callback(result.status, opt.resultPath), result;
				}
				case "视频*3 + 音频": {
					...
					return result ? logger.mark(`视频合成成功！文件地址：${opt.resultPath}`) : logger.error(result), await opt.callback(result.status, opt.resultPath), result;
				}
				case "获取指定视频文件时长": {
				...
				case "压缩视频": {
					const result = await ffmpeg(`-y -i "${opt.path}" -b:v ${opt.targetBitrate}k -maxrate ${opt.maxRate ?? 1.5 * opt.targetBitrate}k -bufsize ${opt.bufSize ?? 2 * opt.targetBitrate}k -crf ${opt.crf ?? 35} -preset medium -c:v libx264 -vf "scale='if(gte(iw/ih,16/9),1280,-1)':'if(gte(iw/ih,16/9),-1,720)',scale=ceil(iw/2)*2:ceil(ih/2)*2" "${opt.resultPath}"`);
					return result.status ? (logger.mark(`视频已压缩并保存到: ${opt.resultPath}`), Common.removeFile(opt.path)) : (logger.error(opt.path + " 压缩失败！"), logger.error(result)), opt.resultPath;
				}
			}
		}
	};
}), init_Networks = __esmMin(() => {
	baseHeaders = {
		Accept: "*/*",
		...
		get config() {
			...
			return "POST" === this.method && this.body && (config$1.data = this.body), config$1;
		}
		async downloadStream(progressCallback, retryCount = 0) {
			const controller = new AbortController(), timeoutId = setTimeout(() => controller.abort(), this.timeout);
			try {
				...
				if (clearTimeout(timeoutId), !(response.status >= 200 && response.status < 300)) throw new Error(`无法获取 ${this.url}。状态: ${response.status} ${response.statusText}`);
				const totalBytes = parseInt(response.headers["content-length"] ?? "0", 10);
				if (isNaN(totalBytes)) throw new Error("无效的 content-length 头");
				let downloadedBytes = 0, lastPrintedPercentage = -1;
				const writer = fs.createWriteStream(this.filepath), intervalId = setInterval(() => {
					const progressPercentage = Math.floor(downloadedBytes / totalBytes * 100);
					progressPercentage !== lastPrintedPercentage && (progressCallback(downloadedBytes, totalBytes), lastPrintedPercentage = progressPercentage);
				}, totalBytes < 10485760 ? 1e3 : 500), onData = (chunk) => {
					downloadedBytes += chunk.length;
				};
				return response.data.on("data", onData), await pipeline(response.data, writer), clearInterval(intervalId), response.data.off("data", onData), writer.end(), {
					filepath: this.filepath,
					totalBytes
				};
			} catch (error) {
				if (clearTimeout(timeoutId), error instanceof AxiosError ? logger.error(`请求在 ${this.timeout / 1e3} 秒后超时`) : logger.error("下载失败:", error), retryCount < this.maxRetries) {
					const delay = Math.min(1e3 * Math.pow(2, retryCount), 1e3);
					return logger.warn(`正在重试下载... (${retryCount + 1}/${this.maxRetries})，将在 ${delay / 1e3} 秒后重试`), await new Promise((resolve) => setTimeout(resolve, delay)), this.downloadStream(progressCallback, retryCount + 1);
				}
		...
		async getLongLink(url = "") {
			...
			try {
				return (await this.axiosInstance.head(this.url || url)).request.res.responseUrl;
			} catch (error) {
				const axiosError = error;
				if (axiosError.response) {
					if (302 === axiosError.response.status) {
						const redirectUrl = axiosError.response.headers.location;
						return logger.info(`检测到302重定向，目标地址: ${redirectUrl}`), await this.getLongLink(redirectUrl);
					}
					if (403 === axiosError.response.status) return errorMsg = `403 Forbidden 禁止访问！${this.url || url}`, logger.error(errorMsg), errorMsg;
				}
				return logger.error(errorMsg), errorMsg;
			}
		}
		async getLocation() {
			try {
				...
				if (error instanceof AxiosError) throw logger.error(`获取 ${this.url} 重定向地址失败，接口响应状态码: ${error.response?.status}`), new Error(error.stack);
			}
		}
		async getData() {
			try {
				const result = await this.returnResult();
				if (504 === result.status) return result;
				if (429 === result.status) throw logger.error("HTTP 响应状态码: 429"), /* @__PURE__ */ new Error("ratelimit triggered, 触发 https://www.douyin.com/ 的速率限制！！！");
				return result.data;
			} catch (error) {
		...
		async getHeaders() {
			try {
				...
				throw logger.error(error), error;
			}
		}
...
}), init_Render = __esmMin(async () => {
	await init_client(), await init_module(), await init_Config(), Render = async (path$1, data) => {
		const pathParts = path$1.split("/");
		let templateType, templateName;
		if (2 === pathParts.length) [templateType, templateName] = pathParts;
		else {
			if (3 !== pathParts.length) throw new Error(`不支持的路径格式: ${path$1}`);
			templateType = pathParts[0], templateName = `${pathParts[1]}/${pathParts[2]}`;
		}
		const outputDir = path.join(karinPathHtml, Root.pluginName, templateType), result = await client_default({
			templateType,
			templateName,
		...
		}), renderResult = await render.render({
			name: `${Root.pluginName}/${templateType}/${templateName}`,
			file: result.htmlPath,
			multiPage: !!Config.app.multiPageRender && Config.app.multiPageHeight,
			selector: "#container",
			fullPage: !1,
			type: "png",
			pageGotoParams: {
				waitUntil: "load",
				timeout: 1e3 * Config.app.RenderWaitTime
			}
		}), ret = [];
...
}), init_utils = __esmMin(async () => {
	init_root(), init_Base(), init_Common(), init_FFmpeg(), init_Networks(), init_Render();
});
function checkvip$2(member) {
	return 1 === member.vip.status ? `<span style="color: ${member.vip.nickname_color ?? "#FB7299"}; font-weight: 700;">${member.name}</span>` : `<span style="color: ${Common.useDarkTheme() ? "#e9e9e9" : "#313131"}; font-weight: 700;">${member.name}</span>`;
}
function br$3(data) {
	return data.replace(/\n/g, "<br>");
}
	...
	for (const tag of rich_text_nodes) {
		...
		switch (tag.type) {
			...
			case "RICH_TEXT_NODE_TYPE_EMOJI": {
				const regex$1 = new RegExp(tag.orig_text.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), emojiUrl = tag.emoji.gif_url || tag.emoji.icon_url;
...
var emoteToUrl, init_bilibili$2 = __esmMin(async () => {
	await init_esm(), await init_utils(), await init_Config(), await init_bilibili$1(), Bilibili = class extends Base {
		e;
		type;
		STATUS;
		isVIP;
		...
		constructor(e, data) {
			super(e), this.e = e, this.isVIP = !1, this.Type = data?.type, this.islogin = "isLogin" === data?.USER?.STATUS, this.downloadfilename = "", this.headers.Referer = "https://api.bilibili.com/", this.headers.Cookie = Config.cookies.bilibili;
		}
		async RESOURCES(iddata) {
			switch (Config.app.EmojiReply && await this.e.bot.setMsgReaction(this.e.contact, this.e.messageId, Config.app.EmojiReplyID, !0), Config.bilibili.tip && await this.e.reply("检测到B站链接，开始解析"), this.Type) {
				case "one_video": {
					...
					this.islogin = "isLogin" === (await checkCk()).Status, this.downloadfilename = infoData.data.data.title.substring(0, 50).replace(/[\\/:*?"<>|\r\n\s]/g, " ");
					...
					if (Config.bilibili.sendContent.some((content) => "info" === content)) if ("text" === Config.bilibili.videoInfoMode) {
						const replyContent = [], { coin, like, share, view, favorite, danmaku } = infoData.data.data.stat, contentMap = {
						...
						[
							...
							Config.bilibili.displayContent.includes(item) && contentMap[item] && replyContent.push(contentMap[item]);
						}), replyContent.length > 0 && this.e.reply(replyContent);
					} else {
					...
					if (this.islogin && Config.bilibili.videoQuality > 64) {
						...
						playUrlData.data.data.dash.video = simplify, correctList = await bilibiliProcessVideos({
							accept_description: playUrlData.data.data.accept_description,
							bvid: infoData.data.data.bvid,
							qn: Config.bilibili.videoQuality
						}, simplify, playUrlData.data.data.dash.audio[0].base_url), playUrlData.data.data.dash.video = correctList.videoList, playUrlData.data.data.accept_description = correctList.accept_description, videoSize = await getvideosize(correctList.videoList[0].base_url, playUrlData.data.data.dash.audio[0].base_url, infoData.data.data.bvid);
					} else videoSize = (nockData.data.durl[0].size / 1048576).toFixed(2);
					if (Config.bilibili.sendContent.some((content) => "comment" === content)) {
						...
						commentsdata?.length ? (img$1 = await Render("bilibili/comment", {
							Type: "视频",
							...
							VideoSize: Config.bilibili.videoQuality < 64 ? (nockData.data.durl[0].size / 1048576).toFixed(2) : videoSize,
							ImageLength: 0,
							shareurl: "https://b23.tv/" + infoData.data.data.bvid
						}), this.e.reply(img$1)) : this.e.reply("这个视频没有评论 ~");
					}
					Config.bilibili.sendContent.some((content) => "video" === content) && (Config.upload.usefilelimit && Number(videoSize) > Number(Config.upload.filelimit) && !Config.upload.compress ? this.e.reply(`设定的最大上传大小为 ${Config.upload.filelimit}MB\n当前解析到的视频大小为 ${Number(videoSize)}MB\n视频太大了，还是去B站看吧~`, { reply: !0 }) : await this.getvideo(Config.bilibili.videoQuality > 64 ? { playUrlData: nockData.data } : {
				...
				case "bangumi_video_info": {
					...
					this.islogin = "isLogin" === (await checkCk()).Status, this.isVIP = (await checkCk()).isVIP;
					const Episodes = [];
					...
					const context = await karin.ctx(this.e, { reply: !0 }), regex = /第([一二三四五六七八九十百千万0-9]+)集/.exec(context.msg);
					let Episode;
					if (!regex || !regex[1]) return logger.debug(Episode), this.e.reply("匹配内容失败，请重新发送链接再次解析"), !0;
					Episode = regex[1], /^[一二三四五六七八九十百千万]+$/.test(Episode) && (Episode = Common.chineseToArabic(Episode).toString()), this.downloadfilename = videoInfo.data.result.episodes[Number(Episode) - 1].share_copy.substring(0, 50).replace(/[\\/:*?"<>|\r\n\s]/g, " "), this.e.reply(`收到请求，第${Episode}集\n${this.downloadfilename}\n正在下载中`);
					const bangumidataBASEURL = bilibiliApiUrls.番剧视频流信息({
					...
					}), Params = await async function(apiURL) {
						if ("" === Config.cookies.bilibili || null === Config.cookies.bilibili) return "&platform=html5";
						const loginInfo = await getBilibiliData("登录基本信息", Config.cookies.bilibili), genSign = await wbi_sign(apiURL, Config.cookies.bilibili), qn = [
							6,
							16,
							32,
							64,
						...
						let isvip;
						return isvip = 1 === loginInfo.data.data.vipStatus, isvip ? `&fnval=16&fourk=1&${genSign}` : `&qn=${qn[3]}&fnval=16`;
					}(bangumidataBASEURL);
					this.islogin || this.e.reply("B站ck未配置或已失效，无法获取视频流，可尝试【#B站登录】以配置新ck");
					const playUrlData = await new Networks({
						url: bangumidataBASEURL + Params,
						headers: this.headers
					}).getData();
					if ("会员" === videoInfo.data.result.episodes[Number(Episode) - 1].badge && !this.isVIP) return logger.warn("该CK不是大会员，无法获取视频流"), !0;
					if (0 === Config.bilibili.videoQuality) {
						...
						playUrlData.result.dash.video = correctList.videoList, playUrlData.result.cept_description = correctList.accept_description;
					}
					await this.getvideo({
				...
				case "dynamic_info": {
					const dynamicInfo = await this.amagi.getBilibiliData("动态详情数据", {
						...
						typeMode: "strict"
					}), dynamicInfoCard = await this.amagi.getBilibiliData("动态卡片数据", {
						dynamic_id: dynamicInfo.data.data.item.id_str,
						typeMode: "strict"
					}), commentsData = dynamicInfo.data.data.item.type !== DynamicType.LIVE_RCMD && await this.amagi.getBilibiliData("评论数据", {
						type: mapping_table(dynamicInfo.data.data.item.type),
						oid: oid(dynamicInfo.data, dynamicInfoCard.data),
						number: Config.bilibili.numcomment,
						typeMode: "strict"
					}), dynamicCARD = JSON.parse(dynamicInfoCard.data.data.card.card), userProfileData = await this.amagi.getBilibiliData("用户主页数据", {
					...
					switch (dynamicInfo.data.data.item.type) {
						case DynamicType.DRAW: {
							const imgArray = [];
							for (const img$2 of dynamicInfo.data.data.item.modules.module_dynamic.major.opus.pics) img$2.url && imgArray.push(segment.image(img$2.url));
							if (Config.bilibili.sendContent.some((content) => "comment" === content) && commentsData) {
								const commentsdata = bilibiliComments(commentsData.data, dynamicInfo.data.data.item.modules.module_author.mid.toString());
								if (img$1 = await Render("bilibili/comment", {
									Type: "动态",
									CommentsData: commentsdata,
									...
									shareurl: "动态分享链接"
								}), 1 === imgArray.length && this.e.reply(imgArray[0]), imgArray.length > 1) {
							...
							if ("topic" in dynamicInfo.data.data.item.modules.module_dynamic && null !== dynamicInfo.data.data.item.modules.module_dynamic.topic) {
						...
						case DynamicType.FORWARD: {
							const text = replacetext(br$3(dynamicInfo.data.data.item.modules.module_dynamic.desc.text), dynamicInfo.data.data.item.modules.module_dynamic.desc.rich_text_nodes), imgList = [];
							for (const richTxtItem of dynamicInfo.data.data.item.modules.module_dynamic.desc.rich_text_nodes) if ("RICH_TEXT_NODE_TYPE_VIEW_PICTURE" === richTxtItem.type) for (const pic of richTxtItem.pics) imgList.push(pic.src);
							let data = {};
							switch (dynamicInfo.data.data.item.orig.type) {
								...
								case DynamicType.DRAW: {
									const dynamicCARD2 = await this.amagi.getBilibiliData("动态卡片数据", {
										...
										typeMode: "strict"
									}), cardData = JSON.parse(dynamicCARD2.data.data.card.card);
									data = {
										username: checkvip$2(dynamicInfo.data.data.item.orig.modules.module_author),
										create_time: Common.convertTimestampToDateTime(dynamicInfo.data.data.item.orig.modules.module_author.pub_ts),
										avatar_url: dynamicInfo.data.data.item.orig.modules.module_author.face,
								...
								case DynamicType.FORWARD:
								default: logger.warn(`UP主：${userProfileData.data.data.card.name}的${logger.green("转发动态")}转发的原动态类型为「${logger.yellow(dynamicInfo.data.item.orig.type)}」暂未支持解析`);
							}
						...
						case DynamicType.AV:
							if ("MAJOR_TYPE_ARCHIVE" === dynamicInfo.data.data.item.modules.module_dynamic.major.type) {
								const bvid = dynamicInfo.data.data.item.modules.module_dynamic.major.archive.bvid, INFODATA = await getBilibiliData("单个视频作品数据", "", {
									bvid,
									typeMode: "strict"
								}), dycrad = dynamicInfoCard.data.data.card && dynamicInfoCard.data.data.card.card && JSON.parse(dynamicInfoCard.data.data.card.card);
								commentsData && Config.bilibili.sendContent.some((item) => "comment" === item) && this.e.reply(await Render("bilibili/comment", {
									Type: "动态",
				...
				case "live_room_detail": {
					const liveInfo = await this.amagi.getBilibiliData("直播间信息", {
						...
						typeMode: "strict"
					}), roomInitInfo = await this.amagi.getBilibiliData("直播间初始化信息", {
						room_id: iddata.room_id,
						typeMode: "strict"
					}), userProfileData = await this.amagi.getBilibiliData("用户主页数据", {
						host_mid: roomInitInfo.data.data.uid,
						typeMode: "strict"
					});
					if (0 === roomInitInfo.data.data.live_status) return this.e.reply(`「${userProfileData.data.data.card.name}」\n未开播，正在休息中~`), !0;
					const img$2 = await Render("bilibili/dynamic/DYNAMIC_TYPE_LIVE_RCMD", {
						...
						create_time: "-62170012800" === liveInfo.data.data.live_time ? "获取失败" : liveInfo.data.data.live_time,
						now_time: Common.getCurrentTime(),
		...
		async getvideo({ infoData, playUrlData }) {
			switch (this.islogin) {
				case !0: {
					const bmp4 = await downloadFile("one_video" === this.Type ? playUrlData.data?.dash?.video[0].base_url : playUrlData.result.dash.video[0].base_url, {
						title: `Bil_V_${"one_video" === this.Type ? infoData && infoData.data.bvid : infoData && infoData.result.season_id}.mp4`,
						headers: this.headers
					}), bmp3 = await downloadFile("one_video" === this.Type ? playUrlData.data?.dash?.audio[0].base_url : playUrlData.result.dash.audio[0].base_url, {
						title: `Bil_A_${"one_video" === this.Type ? infoData && infoData.data.bvid : infoData && infoData.result.season_id}.mp3`,
						headers: this.headers
					});
					bmp4.filepath && bmp3.filepath && await mergeFile("二合一（视频 + 音频）", {
						path: bmp4.filepath,
						path2: bmp3.filepath,
						resultPath: Common.tempDri.video + `Bil_Result_${"one_video" === this.Type ? infoData && infoData.data.bvid : infoData && infoData.result.season_id}.mp4`,
						callback: async (success, resultPath) => {
							if (success) {
								...
								fs.renameSync(resultPath, filePath), logger.mark(`视频文件重命名完成: ${resultPath.split("/").pop()} -> ${filePath.split("/").pop()}`), logger.mark("正在尝试删除缓存文件"), await Common.removeFile(bmp4.filepath, !0), await Common.removeFile(bmp3.filepath, !0);
								const stats = fs.statSync(filePath), fileSizeInMB = Number((stats.size / 1048576).toFixed(2));
								return fileSizeInMB > Config.upload.groupfilevalue ? await uploadFile(this.e, {
									filepath: filePath,
									totalBytes: fileSizeInMB,
									originTitle: this.downloadfilename
								}, "", { useGroupFile: !0 }) : await uploadFile(this.e, {
									filepath: filePath,
									totalBytes: fileSizeInMB,
									originTitle: this.downloadfilename
								}, "");
							}
							return await Common.removeFile(bmp4.filepath, !0), await Common.removeFile(bmp3.filepath, !0), !0;
						}
					});
					break;
				}
				case !1: await downloadVideo(this.e, {
		...
		if (0 !== qualityOptions.qn || 0 !== Config.bilibili.videoQuality) {
			...
			if (!matchedVideo) {
				const sortedVideos = [...videoList].sort((a, b) => a.id - b.id), lowerVideos = sortedVideos.filter((video) => video.id < targetQuality), higherVideos = sortedVideos.filter((video) => video.id > targetQuality);
				matchedVideo = lowerVideos.length > 0 ? lowerVideos[lowerVideos.length - 1] : higherVideos.length > 0 ? higherVideos[0] : sortedVideos[0];
			}
			return qualityOptions.accept_description = [qnd[matchedVideo.id] || qualityOptions.accept_description[0]], videoList = [matchedVideo], {
		...
		let closestId = null, smallestDifference = Infinity;
		if (sizes.forEach((size, index) => {
			if (size <= (qualityOptions?.maxAutoVideoSize ?? Config.bilibili.maxAutoVideoSize)) {
				...
				difference < smallestDifference && (smallestDifference = difference, closestId = Object.keys(results)[index]);
			}
		}), null !== closestId) {
			const closestQuality = qnd[Number(closestId)];
			qualityOptions.accept_description = qualityOptions.accept_description.filter((desc) => desc === closestQuality), 0 === qualityOptions.accept_description.length && (qualityOptions.accept_description = [closestQuality]), videoList = [videoList.find((video) => video.id === Number(closestId))];
		} else videoList = [[...videoList].pop()], qualityOptions.accept_description = [[...qualityOptions.accept_description].pop()];
		return {
			accept_description: qualityOptions.accept_description,
			videoList
		};
		...
		}).getHeaders(), audioheaders = await new Networks({
			url: audiourl,
			headers: {
				...baseHeaders,
				Referer: `https://api.bilibili.com/video/${bvid}`,
		...
		}).getHeaders(), videoSize = videoheaders["content-range"]?.match(/\/(\d+)/) ? parseInt(videoheaders["content-range"]?.match(/\/(\d+)/)[1], 10) : 0, audioSize = audioheaders["content-range"]?.match(/\/(\d+)/) ? parseInt(audioheaders["content-range"]?.match(/\/(\d+)/)[1], 10) : 0, videoSizeInMB = (videoSize / 1048576).toFixed(2), audioSizeInMB = (audioSize / 1048576).toFixed(2);
		...
		const viewText = `📊 播放量: ${Count(view)}`, danmakuText = `💬 弹幕: ${Count(danmaku)}`, likeText = `👍 点赞: ${Count(like)}`, coinText = `🪙 投币: ${Count(coin)}`, shareText = `🔄 转发: ${Count(share)}`, favoriteText = `⭐ 收藏: ${Count(favorite)}`, firstColItems = [
			viewText,
			likeText,
			shareText
		], maxFirstColLength = Math.max(...firstColItems.map((item) => getStringDisplayWidth$1(item)));
		...
		for (let i = 0; i < str.length; i++) {
			...
			code && (code > 65535 ? (width += 2, i++) : width += code >= 12288 && code <= 40959 || code >= 65280 && code <= 65519 || 8230 === code || 8212 === code || code >= 11904 && code <= 12031 || code >= 12288 && code <= 12351 || code >= 12736 && code <= 12783 || code >= 12800 && code <= 13055 || code >= 13056 && code <= 13311 || code >= 44032 && code <= 55215 || code >= 63744 && code <= 64255 || code >= 65072 && code <= 65103 ? 2 : 8205 === code || code >= 65024 && code <= 65039 || code >= 127995 && code <= 127999 ? 0 : 1);
		}
		return width;
	};
});
function bilibiliComments(commentsData, host_mid) {
	...
	let res, jsonArray = [];
	if (404 === commentsData.code) return null;
	if (commentsData.data.top && commentsData.data.top.upper) {
		const topReply = commentsData.data.top.upper, ctime = getRelativeTimeFromTimestamp$3(topReply.ctime), emote = topReply.content.emote;
		let message = topReply.content.message;
		message && emote && (message = emoteToUrl(message, emote));
		const avatar = topReply.member.avatar, frame = topReply.member.pendant.image, uname = checkvip$1(topReply.member), level = topReply.member.level_info.current_level, vipstatus = topReply.member.vip.status, like = topReply.like, replylength = topReply.rcount, location = topReply.reply_control?.location?.replace("IP属地：", "") ?? "", obj = {
			id: 0,
			ctime,
			message,
			avatar,
			...
			level,
			vipstatus,
			img_src: topReply.content && topReply.content.pictures && topReply.content.pictures.length > 0 ? topReply.content.pictures[0].img_src : null,
			replylength,
			location,
			like,
			icon_big_vip: 1 === vipstatus ? "https://i0.hdslb.com/bfs/seed/jinkela/short/user-avatar/big-vip.svg" : null,
			members: topReply.content.members,
			isTop: !0,
			isUP: topReply.mid_str === host_mid
		};
		jsonArray.push(obj);
	}
	for (const [i, reply] of commentsData.data.replies.entries()) {
		const ctime = getRelativeTimeFromTimestamp$3(reply.ctime), emote = reply.content.emote;
		let message = reply.content.message;
		message && emote && (message = emoteToUrl(message, emote));
		const avatar = reply.member.avatar, frame = reply.member.pendant.image, uname = checkvip$1(reply.member), level = reply.member.level_info.current_level, vipstatus = reply.member.vip.vipStatus, like = reply.like, replylength = reply.rcount, location = reply.reply_control?.location?.replace("IP属地：", "") ?? "", obj = {
			id: i + 1,
			ctime,
			message,
			avatar,
			...
			level,
			vipstatus,
			img_src: reply.content && reply.content.pictures && reply.content.pictures.length > 0 ? reply.content.pictures[0].img_src : null,
			replylength,
			location,
			like,
			icon_big_vip: 1 === vipstatus ? "https://i0.hdslb.com/bfs/seed/jinkela/short/user-avatar/big-vip.svg" : null,
			...
			isTop: !1,
			isUP: reply.mid_str === host_mid
		};
		jsonArray.push(obj);
	}
	jsonArray.sort((a, b) => a.isTop && !b.isTop ? -1 : !a.isTop && b.isTop ? 1 : a.isTop && b.isTop ? 0 : b.like - a.like);
	for (const i of jsonArray) i.like > 1e4 && (i.like = (i.like / 1e4).toFixed(1) + "w");
	jsonArray = function(data) {
		for (const i in data) data[i].message && (data[i].message = data[i].message.replace(/ /g, " "));
		return data;
	}(jsonArray);
	...
	return res = function(data) {
		for (const i in data) {
			let message = data[i].message;
			message = message?.replace(/\n/g, "<br>"), data[i].message = message;
		}
		return data;
	}(jsonArray), res = [...res.filter((c) => c.isTop), ...res.filter((c) => !c.isTop)].slice(0, Config.bilibili.numcomment), res;
}
function checkvip$1(member) {
	return 1 === member.vip.vipStatus ? `<span style="color: ${member.vip.nickname_color ?? "#FB7299"}; font-weight: 700;">${member.uname}</span>` : `<span style="color: #888">${member.uname}</span>`;
}
function getRelativeTimeFromTimestamp$3(timestamp) {
	...
	if (differenceInSeconds < 60) return differenceInSeconds + "秒前";
	if (differenceInSeconds < 3600) return Math.floor(differenceInSeconds / 60) + "分钟前";
	if (differenceInSeconds < 86400) return Math.floor(differenceInSeconds / 3600) + "小时前";
	if (differenceInSeconds < 2592e3) return Math.floor(differenceInSeconds / 86400) + "天前";
	if (differenceInSeconds < 7776e3) return Math.floor(differenceInSeconds / 2592e3) + "个月前";
	{
		const date = /* @__PURE__ */ new Date(1e3 * timestamp);
...
var init_comments = __esmMin(async () => {
	await init_module(), await init_Config(), emoteToUrl = (message, emote) => {
		for (const key in emote) Object.prototype.hasOwnProperty.call(emote, key) && message.includes(key) && message.includes("[") && message.includes("]") && (message = message.replace(new RegExp(key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), `<img src="${emote[key].url}"/>`));
		return message;
	};
});
async function checkCk() {
	if ("" === Config.cookies.bilibili || null === Config.cookies.bilibili) return {
		Status: "!isLogin",
		isVIP: !1
	};
	let isVIP;
	return isVIP = 1 === (await getBilibiliData("登录基本信息", Config.cookies.bilibili)).data.data.vipStatus, {
		Status: "isLogin",
		isVIP
	};
}
var init_genParams = __esmMin(() => {
	init_esm(), init_Config();
});
var bilibiliLogin, bilibiliBaseHeaders, Bilibilipush, skipDynamic$1, init_getID = __esmMin(() => {}), init_login = __esmMin(async () => {
	await init_esm(), await init_utils(), await init_Config(), bilibiliLogin = async (e) => {
		const qrcodeurl = await getBilibiliData("申请二维码", { typeMode: "strict" }), qrimg = await Render("bilibili/qrcodeImg", { share_url: qrcodeurl.data.data.url }), base64Data = qrimg[0]?.file;
		if (!base64Data) throw new Error("生成二维码图片失败");
		const cleanBase64 = base64Data.replace(/^base64:\/\//, ""), buffer = Buffer.from(cleanBase64, "base64");
		fs.writeFileSync(`${Common.tempDri.default}BilibiliLoginQrcode.png`, buffer);
		const qrcode_key = qrcodeurl.data.data.qrcode_key, messageIds = [], qrcodeMsg = await e.reply(qrimg, { reply: !0 });
		...
		const recallMessages = async () => {
		...
		}, handleLoginSuccess = async (responseData) => {
			const setCookieHeader = responseData.data.data.headers["set-cookie"];
			let cookieString;
			cookieString = Array.isArray(setCookieHeader) ? setCookieHeader.join("; ") : setCookieHeader || "", Config.Modify("cookies", "bilibili", cookieString), await e.reply("登录成功！用户登录凭证已保存至cookies.yaml", { reply: !0 }), await recallMessages();
		}, handleQrScanned = async () => {
			const scannedMsg = await e.reply("二维码已扫码，未确认", { reply: !0 });
			...
			index > -1 && messageIds.splice(index, 1);
		}, handleQrExpired = async () => {
			await e.reply("二维码已失效", { reply: !0 }), await recallMessages();
		};
		let hasScanned = !1;
		for (;;) try {
			...
			switch (qrcodeStatusData.data.data.data.code) {
				...
				case 86038:
					...
					return;
				case 86090: hasScanned || (await handleQrScanned(), hasScanned = !0);
			}
			await common.sleep(3e3);
		} catch (error) {
			console.error("轮询二维码状态时发生错误:", error), await e.reply("登录过程中发生错误，请重试", { reply: !0 }), await recallMessages();
			return;
		}
	};
}), init_bilibili$1 = __esmMin(async () => {
	init_bilibili$2(), init_comments(), init_genParams(), init_getID(), init_login(), init_push();
});
function br$1(data) {
	return data.replace(/\n/g, "<br>");
}
function checkvip(member) {
	return 1 === member.vip.status ? `<span style="color: ${member.vip.nickname_color ?? "#FB7299"}; font-weight: 700;">${member.name}</span>` : `<span style="color: ${Common.useDarkTheme() ? "#EDEDED" : "#606060"}">${member.name}</span>`;
}
function extractEmojisData(data) {
	const emojisData = [];
	return data.forEach((packages) => {
...
var BilibiliDBBase, DouyinDBBase, douyinDB, douyinInitializing, bilibiliDB, bilibiliInitializing, getDouyinDB, getBilibiliDB, initAllDatabases$1, douyinDBInstance, bilibiliDBInstance, cleanOldDynamicCache, init_push = __esmMin(async () => {
	await init_esm(), await init_module(), await init_Config(), await init_bilibili$1(), bilibiliBaseHeaders = {
		...baseHeaders,
		Referer: "https://api.bilibili.com/",
		Cookie: Config.cookies.bilibili
	}, Bilibilipush = class extends Base {
		force = !1;
		constructor(e = {}, force = !1) {
			super(e), "QQBot" !== this.e.bot?.adapter?.name ? this.force = force : e.reply("不支持QQBot，请使用其他适配器");
		}
		async action() {
			await this.syncConfigToDatabase();
			const deletedCount = await cleanOldDynamicCache("bilibili");
			deletedCount > 0 && logger.info(`已清理 ${deletedCount} 条过期的B站动态缓存记录`);
			const data = await this.getDynamicList(Config.pushlist.bilibili), pushdata = await this.excludeAlreadyPushed(data.willbepushlist);
			return 0 === Object.keys(pushdata).length || (this.force ? await this.forcepush(pushdata) : await this.getdata(pushdata));
		}
		async syncConfigToDatabase() {
			Config.pushlist.bilibili && 0 !== Config.pushlist.bilibili.length && await bilibiliDBInstance.syncConfigSubscriptions(Config.pushlist.bilibili);
		}
		async getdata(data) {
			for (const dynamicId in data) {
				...
				let send_video = !0, img$2 = [];
				const dynamicCARDINFO = await this.amagi.getBilibiliData("动态卡片数据", {
					dynamic_id: dynamicId,
					typeMode: "strict"
				}), dycrad = dynamicCARDINFO.data.data.card && dynamicCARDINFO.data.data.card.card && JSON.parse(dynamicCARDINFO.data.data.card.card);
				if (!skip) {
					...
					switch (emojiDATA = extractEmojisData(emojiDATA.data.data.packages), data[dynamicId].dynamic_type) {
						case DynamicType.DRAW:
							if (null !== data[dynamicId].Dynamic_Data.modules.module_dynamic.topic && data[dynamicId].Dynamic_Data.modules.module_dynamic && null !== data[dynamicId].Dynamic_Data.modules.module_dynamic.topic) {
							...
							break;
						case DynamicType.AV:
							if ("MAJOR_TYPE_ARCHIVE" === data[dynamicId].Dynamic_Data.modules.module_dynamic.major?.type) {
								const bvid = data[dynamicId].Dynamic_Data?.modules.module_dynamic.major?.archive?.bvid ?? "", INFODATA = await getBilibiliData("单个视频作品数据", "", {
									bvid,
									typeMode: "strict"
								});
								INFODATA.data.data.redirect_url && (send_video = !1, logger.debug(`UP主：${INFODATA.data.data.owner.name} 的该动态类型为${logger.yellow("番剧或影视")}，默认跳过不下载，直达：${logger.green(INFODATA.data.data.redirect_url)}`)), img$2 = await Render("bilibili/dynamic/DYNAMIC_TYPE_AV", {
									image_url: INFODATA.data.data.pic,
							...
							switch (data[dynamicId].Dynamic_Data.orig.type) {
								...
								case DynamicType.DRAW: {
									const dynamicCARD = await getBilibiliData("动态卡片数据", Config.cookies.bilibili, {
										...
										typeMode: "strict"
									}), cardData = JSON.parse(dynamicCARD.data.data.card.card);
									param = {
										username: checkvip(data[dynamicId].Dynamic_Data.orig.modules.module_author),
										create_time: Common.convertTimestampToDateTime(data[dynamicId].Dynamic_Data.orig.modules.module_author.pub_ts),
										avatar_url: data[dynamicId].Dynamic_Data.orig.modules.module_author.face,
								...
								case DynamicType.FORWARD:
								default: logger.warn(`UP主：${data[dynamicId].remark}的${logger.green("转发动态")}转发的原动态类型为「${logger.yellow(data[dynamicId].Dynamic_Data.orig.type)}」暂未支持解析`);
							}
						...
						default: skip = !0, logger.warn(`UP主：${data[dynamicId].remark}「${data[dynamicId].dynamic_type}」动态类型的暂未支持推送\n动态地址：${"https://t.bilibili.com/" + data[dynamicId].Dynamic_Data.id_str}`);
					}
				}
				for (const target of data[dynamicId].targets) {
					let status = null;
					if (!skip) {
						const { groupId, botId } = target, bot = karin.getBot(botId), Contact = karin.contactGroup(groupId);
						if (status = await karin.sendMsg(botId, Contact, img$2 ? [...img$2] : []), Config.bilibili.push.parsedynamic && status.messageId) switch (data[dynamicId].dynamic_type) {
							case "DYNAMIC_TYPE_AV":
								if (send_video) {
									...
									const playUrlData = await this.amagi.getBilibiliData("单个视频下载信息数据", {
										...
										cid: dycrad.cid,
										typeMode: "strict"
									}), simplify = playUrlData.data.data.dash.video.filter((item, index, self) => self.findIndex((t) => t.id === item.id) === index);
									if (playUrlData.data.data.dash.video = simplify, correctList = await bilibiliProcessVideos({
										accept_description: playUrlData.data.data.accept_description,
									...
									}, simplify, playUrlData.data.data.dash.audio[0].base_url), playUrlData.data.data.dash.video = correctList.videoList, playUrlData.data.data.accept_description = correctList.accept_description, videoSize = await getvideosize(correctList.videoList[0].base_url, playUrlData.data.data.dash.audio[0].base_url, dynamicCARDINFO.data.data.card.desc.bvid), Config.upload.usefilelimit && Number(videoSize) > Number(Config.upload.filelimit) && !Config.upload.compress) {
									...
									const infoData = await this.amagi.getBilibiliData("单个视频作品数据", {
										bvid: dynamicCARDINFO.data.data.card.desc.bvid,
										typeMode: "strict"
									}), mp4File = await downloadFile(playUrlData.data?.data?.dash?.video[0].base_url, {
										title: `Bil_V_${infoData.data.data.bvid}.mp4`,
										headers: bilibiliBaseHeaders
									}), mp3File = await downloadFile(playUrlData.data?.data?.dash?.audio[0].base_url, {
										title: `Bil_A_${infoData.data.data.bvid}.mp3`,
										headers: bilibiliBaseHeaders
									});
									mp4File.filepath && mp3File.filepath && await mergeFile("二合一（视频 + 音频）", {
										...
										callback: async (success, resultPath) => {
											if (success) {
												...
												fs.renameSync(resultPath, filePath), logger.mark(`视频文件重命名完成: ${resultPath.split("/").pop()} -> ${filePath.split("/").pop()}`), logger.mark("正在尝试删除缓存文件"), await Common.removeFile(mp4File.filepath, !0), await Common.removeFile(mp3File.filepath, !0);
												const stats = fs.statSync(filePath), fileSizeInMB = Number((stats.size / 1048576).toFixed(2));
												return fileSizeInMB > Config.upload.groupfilevalue ? await uploadFile(this.e, {
													filepath: filePath,
													totalBytes: fileSizeInMB,
											...
											return await Common.removeFile(mp4File.filepath, !0), await Common.removeFile(mp3File.filepath, !0), !0;
										}
									});
								}
								break;
					...
					(skip || status && status?.messageId) && await bilibiliDBInstance.addDynamicCache(dynamicId, data[dynamicId].host_mid, target.groupId, data[dynamicId].dynamic_type);
				}
			}
		}
		async getDynamicList(userList) {
			...
			try {
				const filteredUserList = userList.filter((item) => !1 !== item.switch);
				for (const item of filteredUserList) {
					...
					if (dynamic_list.data.data.items.length > 0) for (const dynamic of dynamic_list.data.data.items) {
						const now = Date.now(), createTime = 1e3 * dynamic.modules.module_author.pub_ts, timeDifference = now - createTime, is_top = "置顶" === dynamic.modules.module_tag?.text;
						let shouldPush = !1;
						const timeDiffSeconds = Math.round(timeDifference / 1e3), timeDiffHours = Math.round(timeDifference / 1e3 / 60 / 60 * 100) / 100;
						if (logger.debug(`\n              前期获取该动态基本信息：\n              UP主：${dynamic.modules.module_author.name}\n              动态ID：${dynamic.id_str}\n              发布时间：${Common.convertTimestampToDateTime(createTime / 1e3)}\n              发布时间戳（ms）：${createTime}\n              当前时间戳（ms）：${now}\n              时间差（ms）：${timeDifference} ms (${timeDiffSeconds}s) (${timeDiffHours}h)\n              是否置顶：${is_top}\n              是否在一天内：${timeDifference < 864e5 ? logger.green("true") : logger.red("false")}\n              `), is_top && timeDifference < 864e5 || timeDifference < 864e5 ? (shouldPush = !0, logger.debug(logger.green(`根据以上判断，shoulPush 为 true，将对该动态纳入当天推送列表：https://t.bilibili.com/${dynamic.id_str}\n`))) : logger.debug(logger.yellow(`根据以上判断，shoulPush 为 false，跳过该动态：https://t.bilibili.com/${dynamic.id_str}\n`)), timeDifference < 864e5 || shouldPush) {
							...
							willbepushlist[dynamic.id_str] || (willbepushlist[dynamic.id_str] = {
								remark: item.remark,
								host_mid: item.host_mid,
		...
		async excludeAlreadyPushed(willBePushList) {
			for (const dynamicId in willBePushList) {
				const pushItem = willBePushList[dynamicId], newTargets = [];
				for (const target of pushItem.targets) await bilibiliDBInstance.isDynamicPushed(dynamicId, pushItem.host_mid, target.groupId) || newTargets.push(target);
				newTargets.length > 0 ? pushItem.targets = newTargets : delete willBePushList[dynamicId];
			}
			return willBePushList;
		}
		async setting(data) {
			const groupInfo = await this.e.bot.getGroupInfo("groupId" in this.e && this.e.groupId ? this.e.groupId : ""), host_mid = Number(data.data.card.mid), config$1 = Config.pushlist, groupId = "groupId" in this.e && this.e.groupId ? this.e.groupId : "", botId = this.e.selfId;
			config$1.bilibili || (config$1.bilibili = []);
			const existingItem = config$1.bilibili.find((item) => item.host_mid === host_mid), isSubscribed = await bilibiliDBInstance.isSubscribed(host_mid, groupId);
			if (existingItem) {
				let has = !1, groupIndexToRemove = -1;
				for (let index = 0; index < existingItem.group_id.length; index++) if (existingItem.group_id[index].split(":")[0] === String(groupId)) {
					has = !0, groupIndexToRemove = index;
					break;
				}
				if (has) {
					if (existingItem.group_id.splice(groupIndexToRemove, 1), isSubscribed && await bilibiliDBInstance.unsubscribeBilibiliUser(groupId, host_mid), logger.info(`\n删除成功！${data.data.card.name}\nUID：${host_mid}`), await this.e.reply(`群：${groupInfo.groupName}(${groupId})\n删除成功！${data.data.card.name}\nUID：${host_mid}`), 0 === existingItem.group_id.length) {
						const index = config$1.bilibili.indexOf(existingItem);
						config$1.bilibili.splice(index, 1);
					}
				} else await bilibiliDBInstance.subscribeBilibiliUser(groupId, botId, host_mid, data.data.card.name), existingItem.group_id.push(`${groupId}:${botId}`), await this.e.reply(`群：${groupInfo.groupName}(${groupId})\n添加成功！${data.data.card.name}\nUID：${host_mid}`), !1 === Config.bilibili.push.switch && await this.e.reply("请发送「#kkk设置B站推送开启」以进行推送"), logger.info(`\n设置成功！${data.data.card.name}\nUID：${host_mid}`);
			} else await bilibiliDBInstance.subscribeBilibiliUser(groupId, botId, host_mid, data.data.card.name), config$1.bilibili.push({
				switch: !0,
				host_mid,
				group_id: [`${groupId}:${botId}`],
				remark: data.data.card.name
			}), await this.e.reply(`群：${groupInfo.groupName}(${groupId})\n添加成功！${data.data.card.name}\nUID：${host_mid}`), !1 === Config.bilibili.push.switch && await this.e.reply("请发送「#kkk设置B站推送开启」以进行推送");
			Config.Modify("pushlist", "bilibili", config$1.bilibili), await this.renderPushList();
		}
		async checkremark() {
			const config$1 = Config.pushlist, abclist = [];
			if (null === Config.pushlist.bilibili || 0 === Config.pushlist.bilibili.length) return !0;
			for (const i of Config.pushlist.bilibili) {
				const remark = i.remark, group_id = i.group_id, host_mid = i.host_mid;
				void 0 !== remark && "" !== remark || abclist.push({
					host_mid,
					group_id
				});
			}
			if (abclist.length > 0) {
				for (const i of abclist) {
					const remark = (await this.amagi.getBilibiliData("用户主页数据", {
						host_mid: i.host_mid,
						typeMode: "strict"
					})).data.data.card.name, matchingItemIndex = config$1.bilibili.findIndex((item) => item.host_mid === i.host_mid);
					-1 !== matchingItemIndex && (config$1.bilibili[matchingItemIndex].remark = remark);
				}
				Config.Modify("pushlist", "bilibili", config$1.bilibili);
			}
		}
		async forcepush(data) {
			const currentGroupId = "groupId" in this.e && this.e.groupId ? this.e.groupId : "", currentBotId = this.e.selfId;
			if (this.e.msg.includes("全部")) await this.getdata(data);
			else {
				const subscribedUids = (await bilibiliDBInstance.getGroupSubscriptions(currentGroupId)).map((sub) => sub.host_mid), filteredData = {};
				for (const dynamicId in data) subscribedUids.includes(data[dynamicId].host_mid) && (filteredData[dynamicId] = {
					...data[dynamicId],
					targets: [{
		...
		async renderPushList() {
			await this.syncConfigToDatabase();
			const groupInfo = await this.e.bot.getGroupInfo("groupId" in this.e && this.e.groupId ? this.e.groupId : ""), subscriptions = await bilibiliDBInstance.getGroupSubscriptions(groupInfo.groupId);
			if (0 === subscriptions.length) return void await this.e.reply(`当前群：${groupInfo.groupName}(${groupInfo.groupId})\n没有设置任何B站UP推送！\n可使用「#设置B站推送 + UP主UID」进行设置`);
			const renderOpt = [];
			for (const subscription of subscriptions) {
				const host_mid = subscription.host_mid, userInfo = await this.amagi.getBilibiliData("用户主页数据", {
					host_mid,
					typeMode: "strict"
				});
				renderOpt.push({
		...
		const tags = [];
		if (PushItem.Dynamic_Data.modules.module_dynamic?.desc?.rich_text_nodes) for (const node of PushItem.Dynamic_Data.modules.module_dynamic.desc.rich_text_nodes) "topic" === node.type && node.orig_text && tags.push(node.orig_text);
		if (PushItem.Dynamic_Data.type === DynamicType.FORWARD && "orig" in PushItem.Dynamic_Data && (PushItem.Dynamic_Data.orig.modules.module_dynamic.major.type === MajorType.DRAW || PushItem.Dynamic_Data.orig.modules.module_dynamic.major.type === MajorType.OPUS || PushItem.Dynamic_Data.orig.modules.module_dynamic.major.type === MajorType.LIVE_RCMD)) for (const node of PushItem.Dynamic_Data.orig.modules.module_dynamic.major.opus.summary.rich_text_nodes) "topic" === node.type && tags.push(node.orig_text);
		return logger.debug(`检查动态是否需要过滤：https://t.bilibili.com/${PushItem.Dynamic_Data.id_str}`), await bilibiliDBInstance.shouldFilter(PushItem, tags);
	};
}), init_bilibili = __esmMin(async () => {
	await init_utils(), await init_Config(), await init_push(), BilibiliDBBase = class {
		db;
		dbPath;
		constructor() {
		...
		async init() {
			try {
				logger.debug(logger.green("--------------------------[BilibiliDB] 开始初始化数据库--------------------------")), logger.debug("[BilibiliDB] 正在连接数据库..."), await fs.promises.mkdir(path.dirname(this.dbPath), { recursive: !0 }), this.db = new sqlite3.Database(this.dbPath), await this.createTables(), logger.debug("[BilibiliDB] 数据库模型同步成功"), logger.debug("[BilibiliDB] 正在同步配置订阅..."), logger.debug("[BilibiliDB] 配置项数量:", Config.pushlist.bilibili?.length || 0), await this.syncConfigSubscriptions(Config.pushlist.bilibili), logger.debug("[BilibiliDB] 配置订阅同步成功"), logger.debug(logger.green("--------------------------[BilibiliDB] 初始化数据库完成--------------------------"));
			} catch (error) {
				throw logger.error("[BilibiliDB] 数据库初始化失败:", error), error;
			}
			return this;
		}
		async createTables() {
			...
			if (user) {
				if (remark && user.remark !== remark) {
					...
					await this.runQuery("UPDATE BilibiliUsers SET remark = ?, updatedAt = ? WHERE host_mid = ?", [
						...
						now,
						host_mid
					]), user.remark = remark, user.updatedAt = now;
				}
			} else {
				const now = (/* @__PURE__ */ new Date()).toISOString();
		...
		async subscribeBilibiliUser(groupId, botId, host_mid, remark = "") {
			await this.getOrCreateGroup(groupId, botId), await this.getOrCreateBilibiliUser(host_mid, remark);
		...
		async unsubscribeBilibiliUser(groupId, host_mid) {
			...
			return await this.runQuery("DELETE FROM DynamicCaches WHERE groupId = ? AND host_mid = ?", [groupId, host_mid]), result.changes > 0;
		}
		...
		async getGroupDynamicCache(groupId, host_mid) {
			...
			return host_mid && (sql += " AND host_mid = ?", params.push(host_mid)), sql += " ORDER BY createdAt DESC", await this.allQuery(sql, params);
		}
		...
		async syncConfigSubscriptions(configItems) {
			...
			for (const item of configItems) {
				const host_mid = item.host_mid, remark = item.remark ?? "";
				await this.getOrCreateBilibiliUser(host_mid, remark);
				for (const groupWithBot of item.group_id) {
					const [groupId, botId] = groupWithBot.split(":");
					groupId && botId && (await this.getOrCreateGroup(groupId, botId), configSubscriptions.has(groupId) || configSubscriptions.set(groupId, /* @__PURE__ */ new Set()), configSubscriptions.get(groupId)?.add(host_mid), await this.isSubscribed(host_mid, groupId) || await this.subscribeBilibiliUser(groupId, botId, host_mid, remark));
				}
			}
			const allGroups = await this.allQuery("SELECT * FROM Groups");
			for (const group of allGroups) {
				const groupId = group.id, configUps = configSubscriptions.get(groupId) ?? /* @__PURE__ */ new Set(), dbSubscriptions = await this.getGroupSubscriptions(groupId);
				for (const subscription of dbSubscriptions) {
					const host_mid = subscription.host_mid;
					configUps.has(host_mid) || (await this.unsubscribeBilibiliUser(groupId, host_mid), logger.mark(`已删除群组 ${groupId} 对UP主 ${host_mid} 的订阅`));
				}
			}
			const allUsers = await this.allQuery("SELECT * FROM BilibiliUsers");
			for (const user of allUsers) {
				const host_mid = user.host_mid;
				0 === (await this.getUserSubscribedGroups(host_mid)).length && (await this.runQuery("DELETE FROM FilterWords WHERE host_mid = ?", [host_mid]), await this.runQuery("DELETE FROM FilterTags WHERE host_mid = ?", [host_mid]), await this.runQuery("DELETE FROM BilibiliUsers WHERE host_mid = ?", [host_mid]), logger.mark(`已删除UP主 ${host_mid} 的记录及相关过滤设置（不再被任何群组订阅）`));
			}
		}
		async updateFilterMode(host_mid, filterMode) {
			const user = await this.getOrCreateBilibiliUser(host_mid), now = (/* @__PURE__ */ new Date()).toISOString();
			return await this.runQuery("UPDATE BilibiliUsers SET filterMode = ?, updatedAt = ? WHERE host_mid = ?", [
				filterMode,
				now,
				host_mid
			]), {
		...
		async getFilterConfig(host_mid) {
			const user = await this.getOrCreateBilibiliUser(host_mid), filterWords = await this.getFilterWords(host_mid), filterTags = await this.getFilterTags(host_mid);
			return {
		...
		async extractTextAndTags(dynamicData) {
			...
			if (moduleDynamic.major && moduleDynamic.major.live_rcmd) {
				...
				text += content.live_play_info.title + " ", tags.push(content.live_play_info.area_name);
			}
			if (moduleDynamic.desc && moduleDynamic.desc.text && (text += moduleDynamic.desc.text + " "), moduleDynamic.major && moduleDynamic.major.archive && moduleDynamic.major.archive.title && (text += moduleDynamic.major.archive.title + " "), moduleDynamic.desc && moduleDynamic.desc.rich_text_nodes) for (const node of moduleDynamic.desc.rich_text_nodes) "RICH_TEXT_NODE_TYPE_TEXT" !== node.type && tags.push(node.orig_text);
		...
		async shouldFilter(PushItem, extraTags = []) {
			...
			logger.debug(`\n      提取主动态的文本和标签：\n      文本：${mainText}\n      标签：[${mainTags.join("][")}]\n      `);
			let allTags = [...mainTags, ...extraTags], allText = mainText;
		...
		get groupRepository() {
			return { find: async (options) => options?.where?.botId ? await this.getBotGroups(options.where.botId) : await this.allQuery("SELECT * FROM Groups") };
		}
		get dynamicCacheRepository() {
			return {
				find: async (options = {}) => {
					...
					if (where.groupId && (conditions.push("groupId = ?"), params.push(where.groupId)), where.host_mid && (conditions.push("host_mid = ?"), params.push(where.host_mid)), where.dynamic_id && (conditions.push("dynamic_id = ?"), params.push(where.dynamic_id)), conditions.length > 0 && (sql += " WHERE " + conditions.join(" AND ")), order) {
						const orderClauses = [], allowedFields = [
							"id",
							"dynamic_id",
							"host_mid",
							"groupId",
						...
						for (const [field, direction] of Object.entries(order)) allowedFields.includes(field) && allowedDirections.includes(direction) && orderClauses.push(`${field} ${direction}`);
						orderClauses.length > 0 && (sql += " ORDER BY " + orderClauses.join(", "));
					}
					take && (sql += " LIMIT ?", params.push(take.toString()));
				...
				delete: async (conditions) => {
					...
					return groupId && host_mid ? { affected: (await this.runQuery("DELETE FROM DynamicCaches WHERE groupId = ? AND host_mid = ?", [groupId, host_mid])).changes } : groupId ? { affected: (await this.runQuery("DELETE FROM DynamicCaches WHERE groupId = ?", [groupId])).changes } : host_mid ? { affected: (await this.runQuery("DELETE FROM DynamicCaches WHERE host_mid = ?", [host_mid])).changes } : dynamic_id ? { affected: (await this.runQuery("DELETE FROM DynamicCaches WHERE dynamic_id = ?", [dynamic_id])).changes } : { affected: 0 };
				}
			};
		}
	};
}), init_douyin = __esmMin(async () => {
	await init_utils(), await init_Config(), DouyinDBBase = class {
		db;
		dbPath;
		constructor() {
		...
		async init() {
			try {
				logger.debug(logger.green("--------------------------[DouyinDB] 开始初始化数据库--------------------------")), logger.debug("[DouyinDB] 正在连接数据库..."), await fs.promises.mkdir(path.dirname(this.dbPath), { recursive: !0 }), this.db = new sqlite3.Database(this.dbPath), await this.createTables(), logger.debug("[DouyinDB] 数据库模型同步成功"), logger.debug("[DouyinDB] 正在同步配置订阅..."), logger.debug("[DouyinDB] 配置项数量:", Config.pushlist.douyin?.length || 0), await this.syncConfigSubscriptions(Config.pushlist.douyin), logger.debug("[DouyinDB] 配置订阅同步成功"), logger.debug(logger.green("--------------------------[DouyinDB] 初始化数据库完成--------------------------"));
			} catch (error) {
				throw logger.error("[DouyinDB] 数据库初始化失败:", error), error;
			}
			return this;
		}
		async createTables() {
		...
		async getOrCreateDouyinUser(sec_uid, short_id = "", remark = "") {
			...
			if (user) {
				...
				if (remark && user.remark !== remark && (updates.push("remark = ?"), params.push(remark), user.remark = remark, needUpdate = !0), short_id && user.short_id !== short_id && (updates.push("short_id = ?"), params.push(short_id), user.short_id = short_id, needUpdate = !0), needUpdate) {
					const now = (/* @__PURE__ */ new Date()).toISOString();
					updates.push("updatedAt = ?"), params.push(now), params.push(sec_uid), await this.runQuery(`UPDATE DouyinUsers SET ${updates.join(", ")} WHERE sec_uid = ?`, params), user.updatedAt = now;
				}
			} else {
				const now = (/* @__PURE__ */ new Date()).toISOString();
		...
		async subscribeDouyinUser(groupId, botId, sec_uid, short_id = "", remark = "") {
			await this.getOrCreateGroup(groupId, botId), await this.getOrCreateDouyinUser(sec_uid, short_id, remark);
		...
		async unsubscribeDouyinUser(groupId, sec_uid) {
			...
			return await this.runQuery("DELETE FROM AwemeCaches WHERE groupId = ? AND sec_uid = ?", [groupId, sec_uid]), result.changes > 0;
		}
		...
		async getDouyinUser(sec_uid) {
			...
			return user && (user.living = !!user.living), user || null;
		}
		async updateLiveStatus(sec_uid, living) {
			if (!await this.getDouyinUser(sec_uid)) return !1;
			const now = (/* @__PURE__ */ new Date()).toISOString();
		...
		async getLiveStatus(sec_uid) {
			return { living: (await this.getDouyinUser(sec_uid))?.living || !1 };
		}
		async syncConfigSubscriptions(configItems) {
			const configSubscriptions = /* @__PURE__ */ new Map();
			for (const item of configItems) {
				const sec_uid = item.sec_uid, short_id = item.short_id ?? "", remark = item.remark ?? "";
				await this.getOrCreateDouyinUser(sec_uid, short_id, remark);
				for (const groupWithBot of item.group_id) {
					const [groupId, botId] = groupWithBot.split(":");
					groupId && botId && (await this.getOrCreateGroup(groupId, botId), configSubscriptions.has(groupId) || configSubscriptions.set(groupId, /* @__PURE__ */ new Set()), configSubscriptions.get(groupId)?.add(sec_uid), await this.isSubscribed(sec_uid, groupId) || await this.subscribeDouyinUser(groupId, botId, sec_uid, short_id, remark));
				}
			}
			const allGroups = await this.allQuery("SELECT * FROM Groups");
			for (const group of allGroups) {
				const groupId = group.id, configUsers = configSubscriptions.get(groupId) ?? /* @__PURE__ */ new Set(), dbSubscriptions = await this.getGroupSubscriptions(groupId);
				for (const subscription of dbSubscriptions) {
					const sec_uid = subscription.sec_uid;
					configUsers.has(sec_uid) || (await this.unsubscribeDouyinUser(groupId, sec_uid), logger.mark(`已删除群组 ${groupId} 对抖音用户 ${sec_uid} 的订阅`));
				}
			}
			const allUsers = await this.allQuery("SELECT * FROM DouyinUsers");
			for (const user of allUsers) {
				const sec_uid = user.sec_uid;
				0 === (await this.getUserSubscribedGroups(sec_uid)).length && (await this.runQuery("DELETE FROM FilterWords WHERE sec_uid = ?", [sec_uid]), await this.runQuery("DELETE FROM FilterTags WHERE sec_uid = ?", [sec_uid]), await this.runQuery("DELETE FROM DouyinUsers WHERE sec_uid = ?", [sec_uid]), logger.mark(`已删除抖音用户 ${sec_uid} 的记录及相关过滤设置（不再被任何群组订阅）`));
			}
		}
		...
		async updateFilterMode(sec_uid, filterMode) {
			const user = await this.getOrCreateDouyinUser(sec_uid), now = (/* @__PURE__ */ new Date()).toISOString();
			return await this.runQuery("UPDATE DouyinUsers SET filterMode = ?, updatedAt = ? WHERE sec_uid = ?", [
				filterMode,
				now,
				sec_uid
			]), {
		...
		async getFilterConfig(sec_uid) {
			const user = await this.getOrCreateDouyinUser(sec_uid), filterWords = await this.getFilterWords(sec_uid), filterTags = await this.getFilterTags(sec_uid);
			return {
				filterMode: user.filterMode,
				filterWords,
				filterTags
		...
		async shouldFilter(PushItem, tags = []) {
			...
			if (!sec_uid) return logger.warn(`推送项缺少 sec_uid 参数: ${JSON.stringify(PushItem)}`), !1;
			const { filterMode, filterWords, filterTags } = await this.getFilterConfig(sec_uid);
			logger.debug(`\n      获取用户${PushItem.remark}（${PushItem.sec_uid}）的过滤配置：\n      过滤模式：${filterMode}\n      过滤词：${filterWords}\n      过滤标签：${filterTags}\n      `);
			const desc = PushItem.Detail_Data.desc ?? "", hasFilterWord = filterWords.some((word) => desc.includes(word)), hasFilterTag = filterTags.some((filterTag) => tags.some((tag) => tag === filterTag));
		...
		get awemeCacheRepository() {
			return {
				find: async (options = {}) => {
					...
					const params = [], conditions = [];
					if (where.groupId && (conditions.push("groupId = ?"), params.push(where.groupId)), where.sec_uid && (conditions.push("sec_uid = ?"), params.push(where.sec_uid)), where.aweme_id && (conditions.push("aweme_id = ?"), params.push(where.aweme_id)), conditions.length > 0 && (sql += " WHERE " + conditions.join(" AND ")), order) {
						const orderClauses = [], allowedFields = [
							"id",
							"aweme_id",
							"sec_uid",
				...
				delete: async (conditions) => {
					...
					return groupId && sec_uid ? { affected: (await this.runQuery("DELETE FROM AwemeCaches WHERE groupId = ? AND sec_uid = ?", [groupId, sec_uid])).changes } : groupId ? { affected: (await this.runQuery("DELETE FROM AwemeCaches WHERE groupId = ?", [groupId])).changes } : sec_uid ? { affected: (await this.runQuery("DELETE FROM AwemeCaches WHERE sec_uid = ?", [sec_uid])).changes } : aweme_id ? { affected: (await this.runQuery("DELETE FROM AwemeCaches WHERE aweme_id = ?", [aweme_id])).changes } : { affected: 0 };
				}
			};
		}
	};
...
}), init_db = __esmMin(async () => {
	await init_bilibili(), await init_douyin(), init_bilibili(), init_douyin(), douyinDB = null, douyinInitializing = !1, bilibiliDB = null, bilibiliInitializing = !1, getDouyinDB = async () => {
		if (douyinDB) return douyinDB;
		if (douyinInitializing) return await new Promise((resolve) => setTimeout(resolve, 100)), douyinDB;
		douyinInitializing = !0;
		try {
			return douyinDB = await new DouyinDBBase().init();
		} finally {
			douyinInitializing = !1;
		}
	}, getBilibiliDB = async () => {
		if (bilibiliDB) return bilibiliDB;
		if (bilibiliInitializing) return await new Promise((resolve) => setTimeout(resolve, 100)), bilibiliDB;
		bilibiliInitializing = !0;
		try {
			return bilibiliDB = await new BilibiliDBBase().init();
		} finally {
			bilibiliInitializing = !1;
		}
	...
	}, douyinDBInstance = await getDouyinDB(), bilibiliDBInstance = await getBilibiliDB(), cleanOldDynamicCache = async (platform, days = 7) => "douyin" === platform ? await (await getDouyinDB()).cleanOldAwemeCache(days) : await (await getBilibiliDB()).cleanOldDynamicCache(days);
}), init_module = __esmMin(async () => {
	init_db(), init_utils();
});
await init_esm(), await init_utils(), await init_Config();
var base64Decode = (str) => Buffer.from(str, "base64").toString("utf8"), multiLayerDecode = (str) => {
	try {
		let decoded = base64Decode(str);
		return decoded = ((str) => decodeURIComponent(str))(decoded), decoded = base64Decode(decoded), decoded = ((str) => str.split("").reverse().join(""))(decoded), decoded = ((str) => Buffer.from(str, "hex").toString("utf8"))(decoded), decoded = ((str, offset = 5) => str.split("").map((char) => {
			const code = char.charCodeAt(0);
			return String.fromCharCode(code - offset);
		}).join(""))(decoded, 5), decoded;
	} catch (error) {
...
const signatureVerificationMiddleware = (req, res, next) => {
	try {
		const encodedSignature = req.headers["x-signature"], timestamp = req.headers["x-timestamp"], nonce = req.headers["x-nonce"], token = req.headers.authorization?.replace("Bearer ", "") || "";
		if (!encodedSignature || !timestamp || !nonce) return createBadRequestResponse(res, "缺少必要的签名参数");
		const currentTime = Date.now(), requestTime = parseInt(timestamp);
		if (Math.abs(currentTime - requestTime) > 3e5) return createBadRequestResponse(res, "请求时间戳已过期");
		...
		const signatureString = `${req.method.toUpperCase()}|${req.headers["x-original-url"] || req.originalUrl}|${"GET" === req.method ? "" : JSON.stringify(req.body || {})}|${timestamp}|${nonce}`, expectedSignature = crypto.createHmac("sha256", token).update(signatureString).digest("hex");
		if (decodedSignature !== expectedSignature) return logger.warn(`签名验证失败: 期望=${expectedSignature}, 解码后实际=${decodedSignature}, 签名字符串=${signatureString}`), createBadRequestResponse(res, "签名验证失败");
		next();
	} catch (error) {
		return logger.error("签名验证中间件错误:", error), createServerErrorResponse(res, "签名验证失败");
	}
};
await init_esm(), await init_db(), await init_Config();
await init_esm();
var import_lib = __toESM(require_lib()), import_lib$1 = __toESM(require_lib$1()), import_dist = __toESM(require_dist());
await init_module(), await init_Config();
var { initAllDatabases } = await init_db().then(() => db_exports);
await initAllDatabases();
var server = express();
server.use(import_lib$1.default()), server.use("/", import_dist.createProxyMiddleware({
	target: "https://developer.huawei.com",
	changeOrigin: !0
})), server.listen(3780);
var app$1 = express.Router();
app$1.use(express.json()), app$1.use(express.urlencoded({ extended: !0 })), Config.app.APIServer && Config.app.APIServerMount ? (app$1.use(logMiddleware([
	"/api/bilibili",
	"/api/douyin",
	"/api/kuaishou"
])), app$1.use("/amagi/api/bilibili", createBilibiliRoutes(Config.cookies.bilibili)), app$1.use("/amagi/api/douyin", createDouyinRoutes(Config.cookies.douyin)), app$1.use("/amagi/api/kuaishou", createKuaishouRoutes(Config.cookies.kuaishou)), logger$1.mark(`Amagi server listening on ${logger$1.green("http://localhost:")}${logger$1.green(process.env.HTTP_PORT)} API docs: ${logger$1.yellow("https://amagi.apifox.cn")}`)) : Config.app.APIServer && new Client({ cookies: {
	bilibili: Config.cookies.bilibili,
	douyin: Config.cookies.douyin,
	kuaishou: Config.cookies.kuaishou
} }).startServer(Config.app.APIServerPort), app$1.get("/stream/:filename", (req, res) => {
	const filename = req.params.filename, videoPath = Common.validateVideoRequest(filename, res);
	if (videoPath) try {
		const fileSize = fs.statSync(videoPath).size, range$1 = req.headers.range;
		if (range$1) {
			const parts = range$1.replace(/bytes=/, "").split("-"), start = parseInt(parts[0], 10), end = parts[1] ? parseInt(parts[1], 10) : fileSize - 1;
			if (start >= fileSize || end >= fileSize || start > end) return void res.status(416).send("Requested range not satisfiable");
			const chunksize = end - start + 1, file = fs.createReadStream(videoPath, {
				start,
				end
			}), head = {
			...
			res.writeHead(206, head), file.pipe(res), file.on("error", (err) => {
				logger.error(`读取视频文件流时出错 (Range: ${start}-${end}): ${err.message}`), res.writableEnded || res.end();
			});
		} else {
			const head = {
			...
			file.pipe(res), file.on("error", (err) => {
				if (logger.error(`读取视频文件流时出错 (Full): ${err.message}`), res.headersSent) res.writableEnded || res.end();
				else try {
					createNotFoundResponse(res, "读取视频文件时出错");
				} catch (e) {
					logger.error("发送读取错误响应失败:", e), res.writableEnded || res.end();
				}
			});
		}
	} catch (error) {
		"ENOENT" === error.code ? res.headersSent ? res.writableEnded || res.end() : createNotFoundResponse(res, "视频文件未找到") : (logger.error(`处理视频数据请求时发生错误: ${error.message}`), res.headersSent ? res.writableEnded || res.end() : createNotFoundResponse(res, "服务器内部错误"));
	}
}), app$1.get("/video/:filename", (req, res) => {
	const filename = req.params.filename;
	if (!Common.validateVideoRequest(filename, res)) return;
	const videoDataUrl = `/api/kkk/stream/${encodeURIComponent(filename)}`, resPath = path.join(Root.pluginPath, "/resources") + "/".replace(/\\/g, "/"), htmlContent = template(path.join(resPath, "template", "videoView", "index.html"), {
		videoDataUrl,
		filename
	});
	res.setHeader("Content-Type", "text/html; charset=utf-8"), res.send(htmlContent);
});
var middleware = Config.app.webAuth ? [authMiddleware, signatureVerificationMiddleware] : [];
app$1.use("/getLongLink", ...middleware, async (req, res) => {
	try {
		const { link } = req.body;
		if (!link || "string" != typeof link) return createBadRequestResponse(res, "请提供有效的链接");
		...
		finalUrl.includes("douyin.com") || finalUrl.includes("iesdouyin.com") || finalUrl.includes("webcast.amemv.com") || finalUrl.includes("live.douyin.com") ? platform = "douyin" : finalUrl.includes("bilibili.com") || finalUrl.includes("b23.tv") ? platform = "bilibili" : (finalUrl.includes("kuaishou.com") || finalUrl.includes("kwai.com") || finalUrl.includes("chenzhongtech.com")) && (platform = "kuaishou"), createSuccessResponse(res, {
			originalUrl: link,
			finalUrl,
			platform
		}), logger.debug(`链接重定向获取成功: ${link} -> ${platform}`);
	} catch (error) {
		logger.error(`链接重定向获取失败: ${error.message}`), res.status(500).json({
			success: !1,
			message: "服务器内部错误",
			error: error.message
		});
	}
}), app$1.use("/douyin/data", ...middleware, async (req, res) => {
	...
	try {
		...
		logger.error(`抖音数据获取失败: ${error.message}`), res.status(500).json({
			success: !1,
			message: "抖音数据获取失败",
			error: error.message
		});
	}
}), app$1.use("/bilibili/data", ...middleware, async (req, res) => {
	...
	try {
		...
		logger.error(`哔哩哔哩数据获取失败: ${error.message}`), res.status(500).json({
			success: !1,
			message: "哔哩哔哩数据获取失败",
			error: error.message
		});
	}
}), app$1.use("/kuaishou/data", ...middleware, async (req, res) => {
	...
	try {
		...
		logger.error(`快手数据获取失败: ${error.message}`), res.status(500).json({
			success: !1,
			message: "快手数据获取失败",
			error: error.message
		});
	}
}), app$1.get("/content/douyin", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		...
		return createSuccessResponse(res, await Promise.all(caches.map(async (cache) => {
			...
			cache.douyinUser && (authorName = cache.douyinUser.remark || cache.douyinUser.short_id || cache.douyinUser.sec_uid);
		...
		return logger.error("获取抖音内容列表失败:", error), createServerErrorResponse(res, "获取抖音内容列表失败: " + error);
	}
}), app$1.get("/content/bilibili", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		const { groupId } = req.query;
		if (!groupId) return createBadRequestResponse(res, "请提供群组ID");
		...
		return createSuccessResponse(res, await Promise.all(caches.map(async (cache) => {
			...
			cache.bilibiliUser && (authorName = cache.bilibiliUser.remark || cache.host_mid.toString());
		...
		return logger.error("获取B站内容列表失败:", error), createServerErrorResponse(res, "获取B站内容列表失败: " + error);
	}
}), app$1.get("/groups", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		const douyinDB$1 = await getDouyinDB(), bilibiliDB$1 = await getBilibiliDB(), [douyinGroups, bilibiliGroups] = await Promise.all([douyinDB$1.groupRepository.find(), bilibiliDB$1.groupRepository.find()]), allGroupsMap = /* @__PURE__ */ new Map();
		douyinGroups.forEach((group) => {
			...
			allGroupsMap.has(group.id) || allGroupsMap.set(group.id, {
				id: group.id,
				botId: group.botId
			});
		});
		...
		for (const group of allGroupsMap.values()) {
			const douyinSubscriptions = await douyinDB$1.getGroupSubscriptions(group.id), bilibiliSubscriptions = await bilibiliDB$1.getGroupSubscriptions(group.id);
			if (douyinSubscriptions.length > 0 || bilibiliSubscriptions.length > 0) {
				const bot = getBot(group.botId), groupInfo = await bot.getGroupInfo(group.id), groupAvatarUrl = await bot.getGroupAvatarUrl(group.id);
				groupList.push({
					id: group.id,
		...
		return logger.error("获取群组列表失败:", error), createServerErrorResponse(res, "获取群组列表失败");
	}
}), app$1.get("/authors", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		const { groupId } = req.query;
		if (!groupId) return createBadRequestResponse(res, "请提供群组ID");
		const douyinDB$1 = await getDouyinDB(), bilibiliDB$1 = await getBilibiliDB(), [douyinSubscriptions, bilibiliSubscriptions] = await Promise.all([douyinDB$1.getGroupSubscriptions(groupId), bilibiliDB$1.getGroupSubscriptions(groupId)]), fetchDouyinAuthors = async (subscriptions) => {
			const validSubscriptions = subscriptions.filter((sub) => sub.douyinUser);
			if (0 === validSubscriptions.length) return [];
			const results = [];
			for (let i = 0; i < validSubscriptions.length; i += 5) {
				const batchPromises = validSubscriptions.slice(i, i + 5).map(async (subscription) => {
					try {
						const userProfile = await getDouyinData("用户主页数据", {
						...
						return logger.warn(`获取抖音用户数据失败 (${subscription.douyinUser.sec_uid}):`, error), {
							id: subscription.douyinUser.sec_uid,
							name: subscription.douyinUser.remark || subscription.douyinUser.short_id || subscription.douyinUser.sec_uid,
							avatar: "",
							platform: "douyin"
			...
			return results;
		}, fetchBilibiliAuthors = async (subscriptions) => {
			...
			for (let i = 0; i < validSubscriptions.length; i += 5) {
				const batchPromises = validSubscriptions.slice(i, i + 5).map(async (subscription) => {
					try {
						...
						return logger.warn(`获取B站用户数据失败 (${subscription.bilibiliUser.host_mid}):`, error), {
							id: subscription.bilibiliUser.host_mid.toString(),
							name: subscription.bilibiliUser.remark || subscription.bilibiliUser.host_mid.toString(),
							avatar: "",
							platform: "bilibili"
			...
			return results;
		}, [douyinAuthors, bilibiliAuthors] = await Promise.all([fetchDouyinAuthors(douyinSubscriptions), fetchBilibiliAuthors(bilibiliSubscriptions)]);
		return createSuccessResponse(res, [...douyinAuthors, ...bilibiliAuthors]);
	} catch (error) {
		return logger.error("获取作者列表失败:", error), createServerErrorResponse(res, "获取作者列表失败");
	}
}), app$1.post("/content/douyin", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		const { contentId, groupId, authorId } = req.body;
		return contentId && groupId && authorId ? (await (await getDouyinDB()).addAwemeCache(contentId, authorId, groupId), createSuccessResponse(res, { message: "添加成功" })) : createBadRequestResponse(res, "请提供内容ID、群组ID和作者ID");
	} catch (error) {
		return logger.error("添加抖音内容失败:", error), createServerErrorResponse(res, "添加抖音内容失败");
	}
}), app$1.post("/content/bilibili", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		const { contentId, groupId, authorId } = req.body;
		if (!contentId || !groupId || !authorId) return createBadRequestResponse(res, "请提供内容ID、群组ID和作者ID");
		const bilibiliDB$1 = await getBilibiliDB(), host_mid = parseInt(authorId);
		return await bilibiliDB$1.addDynamicCache(contentId, host_mid, groupId, "manual"), createSuccessResponse(res, { message: "添加成功" });
	} catch (error) {
		return logger.error("添加B站内容失败:", error), createServerErrorResponse(res, "添加B站内容失败");
	}
}), app$1.post("/content/delete", authMiddleware, signatureVerificationMiddleware, async (req, res) => {
	try {
		const { id, platform, groupId } = req.body;
		return id && platform && groupId ? ("douyin" === platform ? await (await getDouyinDB()).awemeCacheRepository.delete({
			aweme_id: id,
			groupId
		}) : "bilibili" === platform && await (await getBilibiliDB()).dynamicCacheRepository.delete({
			dynamic_id: id,
			groupId
		}), createSuccessResponse(res, { message: "删除成功" })) : createBadRequestResponse(res, "请提供内容ID、平台类型和群组ID");
	} catch (error) {
		return logger.error("删除内容失败:", error), createServerErrorResponse(res, "删除内容失败");
	}
});
...
staticRouter.use(express.static(path.join(Root.pluginPath, "lib", "web_chunk"), {
	...
	setHeaders: (res, path$1) => {
		path$1.endsWith(".html") ? res.setHeader("Cache-Control", "no-cache") : res.setHeader("Cache-Control", "public, max-age=31536000");
	}
	...
	disableDotRule: !0
})), staticRouter.use(express.static(path.join(Root.pluginPath, "lib", "web_chunk"), { redirect: !1 })), app.use("/kkk", staticRouter), app.use("/api/kkk", app$1), mkdirSync(`${karinPathBase}/${Root.pluginName}/data`), mkdirSync(Common.tempDri.images), mkdirSync(Common.tempDri.video), console.log(""), console.log("-------------------------- karin-plugin-kkk --------------------------"), logger.info(`${logger.violet(`[插件:v${Root.pluginVersion}]`)} ${logger.green(Root.pluginName)} 初始化完成~`), logger.info(`${logger.violet("[server]")} ${logger.yellow("外部解析页面:")} ${logger.green(`http://127.0.0.1:${process.env.HTTP_PORT}/kkk/`)}`), logger.info(`${logger.violet("[server]")} ${logger.yellow("推送历史管理:")} ${logger.green(`http://127.0.0.1:${process.env.HTTP_PORT}/kkk/database`)}`), console.log("-------------------------- karin-plugin-kkk --------------------------"), console.log(""), await init_module(), await init_Config();
const webConfig = defineConfig({
				...
				children: [components.accordion.createItem("cfg:app", {
					...
					children: [
						...
						components.input.number("RenderWaitTime", {
							label: "渲染图片的等待时间",
							description: "linux" === os.platform() ? "单位：秒，Linux系统下不能为0" : "单位：秒，传递 0 可禁用",
							defaultValue: all.app.RenderWaitTime.toString(),
							rules: ["linux" === os.platform() ? {
								min: 1,
				...
				children: [components.accordion.createItem("cfg:douyin", {
					...
					children: [
						...
						components.input.number("maxAutoVideoSize", {
							...
							isDisabled: "adapt" !== all.douyin.videoQuality || !all.douyin.switch,
							rules: [{
								min: 1,
								max: 2e4
							}]
						...
						components.checkbox.group("displayContent", {
							...
							isDisabled: !all.douyin.switch || all.douyin.switch && "image" === all.douyin.videoInfoMode,
							checkbox: [
						...
						components.input.number("maxAutoVideoSize", {
							...
							isDisabled: "adapt" !== all.douyin.push.videoQuality || !all.douyin.push.switch,
							rules: [{
								min: 1,
								max: 2e4
							}]
				...
				children: [components.accordion.createItem("cfg:bilibili", {
					...
					children: [
						...
						components.input.number("numcomment", {
							...
							rules: [{ min: 1 }],
							isDisabled: !all.bilibili.sendContent.some((content) => "comment" === content) || !all.bilibili.switch
						}),
						components.switch.create("realCommentCount", {
							label: "显示真实评论数量",
							description: "评论图是否显示真实评论数量，关闭则显示解析到的评论数量",
						...
						components.input.number("maxAutoVideoSize", {
							...
							isDisabled: 0 !== all.bilibili.videoQuality || !all.bilibili.switch,
							rules: [{
								min: 1,
								max: 2e4
							}]
						...
						components.checkbox.group("displayContent", {
							...
							isDisabled: !all.bilibili.switch || all.bilibili.switch && "image" === all.bilibili.videoInfoMode,
							checkbox: [
								components.checkbox.create("displayContent:checkbox:1", {
									label: "封面",
									value: "cover"
						...
						components.input.number("push:pushMaxAutoVideoSize", {
							...
							isDisabled: !all.bilibili.push.parsedynamic || 0 !== all.bilibili.push.pushVideoQuality || !all.bilibili.push.switch,
							rules: [{
								min: 1,
								max: 2e4
							}],
				...
				children: [components.accordion.createItem("cfg:xiaohongshu", {
					...
					children: [
						...
						components.input.number("numcomment", {
							...
							rules: [{ min: 1 }],
							isDisabled: !all.xiaohongshu.sendContent.some((content) => "comment" === content) || !all.xiaohongshu.switch
						}),
						components.radio.group("videoQuality", {
							label: "画质偏好",
							description: "解析视频的分辨率偏好。",
						...
						components.input.number("maxAutoVideoSize", {
							...
							isDisabled: "adapt" !== all.xiaohongshu.videoQuality || !all.xiaohongshu.switch,
							rules: [{
								min: 1,
								max: 2e4
							}]
	...
	save: async (config$1) => {
		const formatCfg = processFrontendData(config$1), oldAllCfg = await Config.All(), mergeCfg = _.mergeWith({}, oldAllCfg, formatCfg, customizer);
		cleanFlattenedFields(mergeCfg);
		let success = !1, isChange = !1;
		for (const key of Object.keys(mergeCfg)) {
			const configValue = mergeCfg[key];
			configValue && "object" == typeof configValue && Object.keys(configValue).length > 0 && (isChange = deepEqual(configValue, oldAllCfg[key]), isChange && await Config.ModifyPro(key, configValue) && (success = !0));
		}
		return await Config.syncConfigToDatabase(), {
			mergeCfg,
			formatCfg,
			success,
...
var web_config_default = webConfig, customizer = (value, srcValue) => {
	if (Array.isArray(srcValue)) return srcValue;
}, deepEqual = (a, b) => {
	if (a === b) return !1;
	if ("string" == typeof a && "string" == typeof b && a !== b) return !0;
	if ("number" == typeof a && "number" == typeof b && a !== b) return !0;
	if ("boolean" == typeof a && "boolean" == typeof b && a !== b) return !0;
	if (null === a || null === b || typeof a != typeof b) return !0;
	if (Array.isArray(a) && Array.isArray(b)) {
		if (a.length !== b.length) return !0;
		for (let i = 0; i < a.length; i++) if (deepEqual(a[i], b[i])) return !0;
	}
	let isChange = !1;
	if ("object" == typeof a && "object" == typeof b) {
		...
		for (const key of keysA) {
			if (!keysB.includes(key)) return isChange = !0, !0;
			if (deepEqual(a[key], b[key])) return isChange = !0, !0;
		}
	}
	return !1;
}, convertToNumber = (value) => /^\d+$/.test(value) ? parseInt(value, 10) : value, getFirstObject = (arr) => arr.length > 0 ? arr[0] : {}, setNestedProperty = (obj, keys, value) => {
	let current = obj;
	for (let i = 0; i < keys.length - 1; i++) {
		const key = keys[i];
		current[key] && "object" == typeof current[key] || (current[key] = {}), current = current[key];
	}
	current[keys[keys.length - 1]] = value;
}, processFrontendData = (data) => {
	const result = {}, configKeys = Object.keys(data).filter((key) => !key.includes("pushlist") && key in data);
	for (const key of configKeys) {
		const value = data[key], firstObj = Array.isArray(value) ? getFirstObject(value) : {}, objKeys = Object.keys(firstObj);
		if (0 === objKeys.length) continue;
		const configObj = {};
		let hasValidData = !1;
		const nestedProps = objKeys.filter((prop) => prop.includes(":")), flatProps = objKeys.filter((prop) => !prop.includes(":"));
		for (const prop of nestedProps) {
			let propValue = firstObj[prop];
			"string" == typeof propValue && (propValue = convertToNumber(propValue)), null != propValue && (setNestedProperty(configObj, prop.split(":"), propValue), hasValidData = !0);
		}
		for (const prop of flatProps) {
			let propValue = firstObj[prop];
			"string" == typeof propValue && (propValue = convertToNumber(propValue)), null != propValue && (configObj[prop] = propValue, hasValidData = !0);
		}
		hasValidData && Object.keys(configObj).length > 0 && (result[key] = configObj);
	}
	return result.pushlist = {
...
}, cleanFlattenedFields = (obj) => {
	if (obj && "object" == typeof obj) {
		for (const [, value] of Object.entries(obj)) if ("object" == typeof value && null !== value && !Array.isArray(value)) {
			cleanFlattenedFields(value);
			const valueObj = value, flattenedKeys = Object.keys(valueObj).filter((k) => k.includes("."));
			for (const flatKey of flattenedKeys) hasNestedStructure(valueObj, flatKey.split(".")) && delete valueObj[flatKey];
		}
	}
}, hasNestedStructure = (obj, path$1) => {
	let current = obj;
	for (let i = 0; i < path$1.length - 1; i++) {
		const key = path$1[i];
		if (!current[key] || "object" != typeof current[key]) return !1;
		current = current[key];
	}
...
await init_utils(), await init_Config();
var processCommentEmojis$1 = (text, emojiData) => {
	...
	for (const emoji of emojiData) if (emoji.name && emoji.url && processedText.includes(emoji.name)) {
		const escapedName = emoji.name.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), regex = new RegExp(escapedName, "g");
		processedText = processedText.replace(regex, `<img src="${emoji.url}" alt="${emoji.name}" />`);
	}
	return processedText.split(/(<img[^>]*>)/).map((part) => part.startsWith("<img") ? part : part.trim() ? `<span>${part}</span>` : part).join("");
}, processTextFormatting = (text) => {
	let processedText = text.replace(/\n/g, "<br>");
	return processedText = processedText.replace(/ {2,}/g, (match) => "&nbsp;".repeat(match.length)), processedText;
}, processAtUsers$1 = async (text, userIds) => {
...
}, processCommentImage = async (imageUrl) => {
	if (!imageUrl) return null;
	const headers = await new Networks({
		url: imageUrl,
		type: "arraybuffer"
	}).getHeaders();
	if (headers["content-type"] && "image/heic" === headers["content-type"]) {
...
await init_utils(), await init_Config();
var img, mp4size = "", DouYin = class extends Base {
	e;
	type;
	is_mp4;
	is_slides;
	...
	constructor(e, iddata) {
		super(e), this.e = e, this.type = iddata?.type, this.is_mp4 = iddata?.is_mp4, this.is_slides = !1;
	}
	async RESOURCES(data) {
		switch (Config.app.EmojiReply && !this.e.isPrivate && await this.e.bot.setMsgReaction(this.e.contact, this.e.messageId, Config.app.EmojiReplyID, !0), Config.douyin.tip && this.e.reply("检测到抖音链接，开始解析"), this.type) {
			case "one_work": {
				const VideoData = await this.amagi.getDouyinData("聚合解析", {
					aweme_id: data.aweme_id,
					typeMode: "strict"
				}), CommentsData = await this.amagi.getDouyinData("评论数据", {
					aweme_id: data.aweme_id,
					number: Config.douyin.numcomment,
					typeMode: "strict"
				});
				this.is_slides = !0 === VideoData.data.aweme_detail.is_slides;
				let g_title, g_video_url = "", imagenum = 0;
				const image_res = [];
				if (!1 === this.is_mp4) switch (!0) {
					case !1 === this.is_slides && null !== VideoData.data.aweme_detail.images: {
						const image_data = [], imageres = [];
						let image_url = "";
						const images = VideoData.data.aweme_detail.images ?? [];
						for (const [index, imageItem] of images.entries()) if (image_url = imageItem.url_list[2] || imageItem.url_list[1], g_title = VideoData.data.aweme_detail.preview_title.substring(0, 50).replace(/[\\/:*?"<>|\r\n]/g, " "), imageres.push(segment.image(image_url)), imagenum++, !1 === Config.app.removeCache) {
						...
						image_data.push(res), image_res.push(image_data), 1 === imageres.length ? await this.e.reply(segment.image(image_url)) : await this.e.bot.sendForwardMsg(this.e.contact, res, {
							source: "图片合集",
					...
					case !0 === VideoData.data.aweme_detail.is_slides && null !== VideoData.data.aweme_detail.images: {
						const images = [], temp = [], liveimgbgm = await downloadFile(VideoData.data.aweme_detail.music.play_url.uri, {
							title: `Douyin_tmp_A_${Date.now()}.mp3`,
							headers: this.headers
						});
						temp.push(liveimgbgm);
						const images1 = VideoData.data.aweme_detail.images ?? [];
						images1.length || logger.debug("未获取到合辑的图片数据");
						for (const item of images1) {
							if (imagenum++, 2 === item.clip_type) {
							...
							if (liveimg.filepath) {
								...
								await mergeFile("视频*3 + 音频", {
									...
									callback: async (success, resultPath) => {
										if (success) {
											const filePath = Common.tempDri.video + `tmp_${Date.now()}.mp4`;
											return fs.renameSync(resultPath, filePath), logger.mark(`视频文件重命名完成: ${resultPath.split("/").pop()} -> ${filePath.split("/").pop()}`), logger.mark("正在尝试删除缓存文件"), await Common.removeFile(liveimg.filepath, !0), temp.push({
												filepath: filePath,
												totalBytes: 0
											}), images.push(segment.video("file://" + filePath)), !0;
										}
										return await Common.removeFile(liveimg.filepath, !0), !0;
									}
								});
							}
						}
						...
						try {
							...
							for (const item of temp) await Common.removeFile(item.filepath, !0);
						}
						break;
					}
				}
				if (VideoData.data.aweme_detail.music) {
					...
					if (!1 === this.is_mp4 && !1 === Config.app.removeCache && void 0 !== music_url) try {
					...
					music_url && !1 === this.is_mp4 && void 0 !== music_url && await this.e.reply(segment.record(music_url, !1));
				}
				let FPS;
				if (this.is_mp4) {
					const video = VideoData.data.aweme_detail.video;
					FPS = video.bit_rate[0].FPS, logger.debug(`开始排除不符合条件的视频分辨率；\n\n              共拥有${logger.yellow(video.bit_rate.length)}个视频源\n\n              视频ID：${logger.green(VideoData.data.aweme_detail.aweme_id)}\n\n              分享链接：${logger.green(VideoData.data.aweme_detail.share_url)}\n              `), video.bit_rate = douyinProcessVideos(video.bit_rate, Config.douyin.videoQuality, Config.douyin.maxAutoVideoSize), g_video_url = await new Networks({
						url: video.bit_rate[0].play_addr.url_list[2],
						headers: {
							...this.headers,
							Referer: video.bit_rate[0].play_addr.url_list[0]
						}
					}).getLongLink(), g_title = VideoData.data.aweme_detail.preview_title.substring(0, 80).replace(/[\\/:*?"<>|\r\n]/g, " "), mp4size = (video.bit_rate[0].play_addr.data_size / 1048576).toFixed(2);
				}
				if (Config.douyin.sendContent.includes("info")) if ("text" === Config.douyin.videoInfoMode) {
					const replyContent = [], { digg_count, share_count, collect_count, comment_count, recommend_count } = VideoData.data.aweme_detail.statistics, contentMap = {
					...
					[
						...
						Config.douyin.displayContent.includes(item) && contentMap[item] && replyContent.push(contentMap[item]);
					}), replyContent.length > 0 && this.e.reply(replyContent);
				} else {
				...
				if (Config.douyin.sendContent.includes("comment")) {
					const commentsArray = await async function(data, emojidata) {
						let jsonArray = [];
						if (null === data.data.comments) return [];
						let id = 1;
						for (const comment of data.data.comments) {
							const cid = comment.cid, aweme_id = comment.aweme_id, nickname = comment.user.nickname, userimageurl = comment.user.avatar_thumb.url_list[0];
							let text = comment.text;
							const ip = comment.ip_label ?? "未知", time = comment.create_time, label_type = comment.label_type ?? -1, sticker = comment.sticker ? comment.sticker.animate_url.url_list[0] : null;
							let digg_count = comment.digg_count;
							const imageurl = comment.image_list && comment.image_list?.[0] && comment.image_list?.[0].origin_url && comment.image_list?.[0].origin_url.url_list ? comment.image_list?.[0].origin_url.url_list[0] : null, status_label = comment.label_list?.[0]?.text ?? null, userintextlongid = comment.text_extra && comment.text_extra[0] && comment.text_extra[0].sec_uid ? comment.text_extra[0].sec_uid && comment.text_extra.map((extra) => extra.sec_uid) : null, search_text = comment.text_extra && comment.text_extra[0] && comment.text_extra[0].search_text ? comment.text_extra[0].search_text && comment.text_extra.map((extra) => ({
								search_text: extra.search_text,
								search_query_id: extra.search_query_id
							})) : null, relativeTime = getRelativeTimeFromTimestamp$2(time), reply_comment_total = comment.reply_comment_total;
							text = processTextFormatting(text), text = await processAtUsers$1(text, userintextlongid), text = processCommentEmojis$1(text, emojidata);
							const processedImageUrl = await processCommentImage(imageurl);
							digg_count > 1e4 && (digg_count = (digg_count / 1e4).toFixed(1) + "w");
							const replyComment = await getDouyinData("指定评论回复数据", {
								aweme_id,
								comment_id: cid,
								typeMode: "strict",
								number: 2
							}, Config.cookies.douyin), commentObj = {
								id: id++,
						...
						jsonArray.sort((a, b) => {
							const aCount = "string" == typeof a.digg_count && a.digg_count.includes("w") ? 1e4 * parseFloat(a.digg_count) : "number" == typeof a.digg_count ? a.digg_count : 0;
							return ("string" == typeof b.digg_count && b.digg_count.includes("w") ? 1e4 * parseFloat(b.digg_count) : "number" == typeof b.digg_count ? b.digg_count : 0) - aCount;
						});
						const indexLabelTypeOne = jsonArray.findIndex((comment) => 1 === comment.label_type);
						if (-1 !== indexLabelTypeOne) {
							const commentTypeOne = jsonArray.splice(indexLabelTypeOne, 1)[0];
							jsonArray.unshift(commentTypeOne);
						}
						return { jsonArray };
					}(CommentsData, Emoji((await this.amagi.getDouyinData("Emoji数据", { typeMode: "strict" })).data));
					if (commentsArray.jsonArray.length) {
						...
						await this.e.reply(img$2);
					} else await this.e.reply("这个作品没有评论 ~");
				}
				return this.is_mp4 && Config.douyin.sendContent.includes("video") && await downloadVideo(this.e, {
				...
				}, { message_id: this.e.messageId }), !0;
			}
			case "user_dynamic": return !0;
			case "music_work": {
				const MusicData = await this.amagi.getDouyinData("音乐数据", {
					music_id: data.music_id,
					typeMode: "strict"
				}), sec_uid = MusicData.data.music_info.sec_uid, UserData = await this.amagi.getDouyinData("用户主页数据", {
					sec_uid,
					typeMode: "strict"
				});
				return MusicData.data.music_info.play_url ? (img = await Render("douyin/musicinfo", {
				...
				]), await this.e.reply(segment.record(MusicData.data.music_info.play_url.uri, !1)), !0) : (await this.e.reply("解析错误！该音乐抖音未提供下载链接，无法下载", { reply: !0 }), !0);
			}
			case "live_room_detail": {
				...
				if (1 === UserInfoData.data.user.live_status) {
					const live_data = await this.amagi.getDouyinData("直播间信息数据", {
						sec_uid: UserInfoData.data.user.sec_uid,
						typeMode: "strict"
					}), room_data = JSON.parse(UserInfoData.data.user.room_data), img$2 = await Render("douyin/live", {
...
function douyinProcessVideos(videos, videoQuality, maxAutoVideoSize) {
	const mp4Videos = videos.filter((video) => "dash" !== video.format);
	if (0 === mp4Videos.length) return logger.warn("没有找到可用的 mp4 格式视频"), videos.slice(0, 1);
	logger.debug(`过滤后剩余 ${mp4Videos.length} 个 mp4 格式视频`);
	const videosByQuality = /* @__PURE__ */ new Map();
	if (mp4Videos.forEach((video) => {
		const quality = (gearName = video.gear_name).includes("lowest_4") || gearName.includes("2160") ? "4k" : gearName.includes("1440") || gearName.includes("2k") ? "2k" : gearName.includes("1080") ? "1080p" : gearName.includes("720") ? "720p" : (gearName.includes("540"), "540p");
		var gearName;
		videosByQuality.has(quality) || videosByQuality.set(quality, []), videosByQuality.get(quality).push(video);
	}), videosByQuality.forEach((videos$1) => {
		videos$1.sort((a, b) => b.play_addr.data_size - a.play_addr.data_size);
	}), "adapt" === videoQuality) {
		const sizeLimitBytes = 1024 * (maxAutoVideoSize || Config.upload.filelimit) * 1024;
		for (const quality of [
			...
			if (qualityVideos && qualityVideos.length > 0) {
				...
				if (suitableVideo) return logger.debug(`自动选择画质: ${quality}, 文件大小: ${(suitableVideo.play_addr.data_size / 1048576).toFixed(2)}MB`), [suitableVideo];
			}
		}
		let smallestVideo = mp4Videos[0];
		return mp4Videos.forEach((video) => {
			video.play_addr.data_size < smallestVideo.play_addr.data_size && (smallestVideo = video);
		}), logger.debug(`未找到符合大小限制的视频，选择最小视频: ${(smallestVideo.play_addr.data_size / 1048576).toFixed(2)}MB`), [smallestVideo];
	}
	const targetQuality = videoQuality, targetVideos = videosByQuality.get(targetQuality);
	if (targetVideos && targetVideos.length > 0) return logger.debug(`选择固定画质: ${targetQuality}, 文件大小: ${(targetVideos[0].play_addr.data_size / 1048576).toFixed(2)}MB`), [targetVideos[0]];
	const qualityPriority = [
		...
		"720p",
		"540p"
	], targetIndex = qualityPriority.indexOf(targetQuality);
	for (let i = targetIndex + 1; i < qualityPriority.length; i++) {
		const fallbackVideos = videosByQuality.get(qualityPriority[i]);
		if (fallbackVideos && fallbackVideos.length > 0) return logger.debug(`目标画质 ${targetQuality} 不可用，降级到: ${qualityPriority[i]}`), [fallbackVideos[0]];
	}
	for (let i = targetIndex - 1; i >= 0; i--) {
		const fallbackVideos = videosByQuality.get(qualityPriority[i]);
		if (fallbackVideos && fallbackVideos.length > 0) return logger.debug(`目标画质 ${targetQuality} 不可用，升级到: ${qualityPriority[i]}`), [fallbackVideos[0]];
	}
	return logger.warn("未找到任何匹配的画质，返回默认视频"), [mp4Videos[0]];
}
function Time(delay) {
	const currentDate = /* @__PURE__ */ new Date();
	return currentDate.setHours(currentDate.getHours() + delay), `${currentDate.getFullYear().toString()}/${(currentDate.getMonth() + 1).toString()}/${String(currentDate.getDate()).padStart(2, "0")} ${String(currentDate.getHours()).padStart(2, "0")}:${String(currentDate.getMinutes()).padStart(2, "0")}:${String(currentDate.getSeconds()).padStart(2, "0")}`;
}
const Emoji = (data) => {
	const ListArray = [];
...
var formatVideoStats = (digg_count, share_count, collect_count, comment_count, recommend_count) => {
	const diggText = `❤ 点赞: ${Count(digg_count)}`, shareText = `🔄 转发: ${Count(share_count)}`, collectText = `⭐ 收藏: ${Count(collect_count)}`, commentText = `💬 评论: ${Count(comment_count)}`, recommendText = `👍 推荐: ${Count(recommend_count)}`, firstColItems = [diggText, shareText], maxFirstColLength = Math.max(...firstColItems.map((item) => getStringDisplayWidth(item)));
...
async function getDouyinID(event, url, log = !0) {
	...
	switch (!0) {
		...
		case longLink.includes("live.douyin.com"):
			...
			} else longLink.includes("live.douyin.com") && (result = {
				type: "live_room_detail",
		...
		default: logger.warn("无法获取作品ID");
	}
	return log && console.log(result), result;
}
await init_module(), await init_Config();
var douyinBaseHeaders = {
...
}, DouYinpush = class extends Base {
	force = !1;
	constructor(e = {}, force = !1) {
		super(e), "QQBot" !== this.e.bot?.adapter?.name ? (this.headers.Referer = "https://www.douyin.com", this.headers.Cookie = Config.cookies.douyin, this.force = force) : e.reply("不支持QQBot，请使用其他适配器");
	}
	async action() {
		await this.syncConfigToDatabase();
		const deletedCount = await cleanOldDynamicCache("douyin");
		if (deletedCount > 0 && logger.info(`已清理 ${deletedCount} 条过期的抖音作品缓存记录`), await this.checkremark()) return !0;
		const data = await this.getDynamicList(Config.pushlist.douyin);
		return 0 === Object.keys(data).length || (this.force ? await this.forcepush(data) : await this.getdata(data));
	}
	async syncConfigToDatabase() {
		Config.pushlist.douyin && 0 !== Config.pushlist.douyin.length && await douyinDBInstance.syncConfigSubscriptions(Config.pushlist.douyin);
	}
	async getdata(data) {
		...
		for (const awemeId in data) {
			...
			const pushItem = data[awemeId], Detail_Data = pushItem.Detail_Data, skip = await skipDynamic(pushItem);
			skip && logger.warn(`作品 https://www.douyin.com/video/${awemeId} 已被处理，跳过`);
			let img$2 = [], iddata = {
				is_mp4: !0,
				type: "one_work"
			};
			if (skip || (iddata = await getDouyinID(this.e, Detail_Data.share_url ?? "https://live.douyin.com/" + Detail_Data.room_data?.owner.web_rid, !1)), !skip) if (pushItem.living && "room_data" in pushItem.Detail_Data && Detail_Data.live_data) img$2 = await Render("douyin/live", {
				...
				const realUrl = "web" === Config.douyin.push.shareType && await new Networks({
				...
				img$2 = await Render("douyin/dynamic", {
					...
					avater_url: "https://p3-pc.douyinpic.com/aweme/1080x1080/" + Detail_Data.user_info.data.user.avatar_larger.uri,
					share_url: "web" === Config.douyin.push.shareType ? realUrl : `https://aweme.snssdk.com/aweme/v1/play/?video_id=${Detail_Data.video.play_addr.uri}&ratio=1080p&line=0`,
					...
					cooperation_info: (() => {
						...
						if (!raw) return;
						const rawCreators = Array.isArray(raw.co_creators) ? raw.co_creators : [], author = Detail_Data.author, authorUid = author?.uid, authorSecUid = author?.sec_uid, authorNickname = author?.nickname, authorInCreators = rawCreators.some((c) => authorUid && c.uid && c.uid === authorUid || authorSecUid && c.sec_uid && c.sec_uid === authorSecUid || authorNickname && c.nickname && c.nickname === authorNickname), co_creators = rawCreators.map((c) => {
			...
			for (const target of pushItem.targets) try {
				...
				if (!skip) {
					...
					if (status = await karin.sendMsg(botId, Contact, img$2 ? [...img$2] : []), pushItem.living && "room_data" in pushItem.Detail_Data && status.message_id && await douyinDBInstance.updateLiveStatus(pushItem.sec_uid, !0), Config.douyin.push.parsedynamic && status.message_id) {
						if (iddata.is_mp4) try {
							...
							}).getLongLink(), await downloadVideo(this.e, {
						...
						else if (!iddata.is_mp4 && "one_work" === iddata.type) {
							const imageres = [];
							let image_url;
							for (const item of Detail_Data.images) image_url = item.url_list[2] ?? item.url_list[1], imageres.push(segment.image(image_url));
							const bot = karin.getBot(botId), forwardMsg = common.makeForward(imageres, botId, bot.account.name);
							await bot.sendForwardMsg(karin.contactFriend(botId), forwardMsg);
						}
					}
				}
				(skip || !pushItem.living && status.message_id) && await douyinDBInstance.addAwemeCache(awemeId, pushItem.sec_uid, groupId);
			} catch (error) {
		...
		try {
			...
			for (const item of filteredUserList) {
				...
				const videolist = await this.amagi.getDouyinData("用户主页视频列表数据", {
					sec_uid,
					typeMode: "strict"
				}), userinfo = await this.amagi.getDouyinData("用户主页数据", {
					sec_uid,
					typeMode: "strict"
				...
				if (0 === targets.length) continue;
				if (videolist.data.aweme_list.length > 0) for (const aweme of videolist.data.aweme_list) {
					logger.debug(`开始处理作品：${aweme.aweme_id}`);
					const now = Date.now(), timeDifference = now - 1e3 * aweme.create_time, is_top = 1 === aweme.is_top;
					let shouldPush = !1;
					const timeDiffSeconds = Math.round(timeDifference / 1e3), timeDiffHours = Math.round(timeDifference / 1e3 / 60 / 60 * 100) / 100;
					logger.debug(`\n              前期获取该作品基本信息：\n              作者：${aweme.author.nickname}\n              作品ID：${aweme.aweme_id}\n              发布时间：${Common.convertTimestampToDateTime(aweme.create_time)}\n              发布时间戳（s）：${aweme.create_time}\n              当前时间戳（ms）：${now}\n              时间差（ms）：${timeDifference} ms (${timeDiffSeconds}s) (${timeDiffHours}h)\n              是否置顶：${is_top}\n              是否处于开播：${1 === userinfo.data.user?.live_status ? logger.green("true") : logger.red("false")}\n              是否在一天内：${timeDifference < 864e5 ? logger.green("true") : logger.red("false")}\n              `), (is_top && timeDifference < 864e5 || timeDifference < 864e5 && !is_top) && (await this.checkIfAlreadyPushed(aweme.aweme_id, sec_uid, targets.map((t) => t.groupId)) || (shouldPush = !0)), shouldPush && (willbepushlist[aweme.aweme_id] = {
						remark: item.remark,
						sec_uid,
						create_time: 1e3 * aweme.create_time,
						targets,
						Detail_Data: {
							...aweme,
							user_info: userinfo
				...
				} else liveStatus.living && (await douyinDBInstance.updateLiveStatus(sec_uid, !1), logger.info(`用户 ${item.remark ?? sec_uid} 已关播，更新直播状态`));
			}
		} catch (error) {
	...
	async checkIfAlreadyPushed(aweme_id, sec_uid, groupIds) {
		for (const groupId of groupIds) if (!await douyinDBInstance.isAwemePushed(aweme_id, sec_uid, groupId)) return !1;
		return !0;
	}
	async setting(data) {
		const groupInfo = await this.e.bot.getGroupInfo("groupId" in this.e && this.e.groupId ? this.e.groupId : ""), config$1 = Config.pushlist, groupId = "groupId" in this.e && this.e.groupId ? this.e.groupId : "", botId = this.e.selfId;
		try {
			let index = 0;
			for (; "user" !== data.data[index].card_unique_name;) index++;
			const sec_uid = data.data[index].user_list[0].user_info.sec_uid, UserInfoData = await this.amagi.getDouyinData("用户主页数据", {
				sec_uid,
				typeMode: "strict"
			});
			let user_shortid;
			user_shortid = "" === UserInfoData.data.user.unique_id ? UserInfoData.data.user.short_id : UserInfoData.data.user.unique_id, config$1.douyin ??= [];
			const existingItem = config$1.douyin.find((item) => item.sec_uid === sec_uid), isSubscribed = await douyinDBInstance.isSubscribed(sec_uid, groupId);
			if (existingItem) {
				let has = !1, groupIndexToRemove = -1;
				for (let index$1 = 0; index$1 < existingItem.group_id.length; index$1++) if (existingItem.group_id[index$1].split(":")[0] === String(groupId)) {
					has = !0, groupIndexToRemove = index$1;
					break;
				}
				if (has) {
					if (existingItem.group_id.splice(groupIndexToRemove, 1), isSubscribed && await douyinDBInstance.unsubscribeDouyinUser(groupId, sec_uid), logger.info(`\n删除成功！${UserInfoData.data.user.nickname}\n抖音号：${user_shortid}\nsec_uid${UserInfoData.data.user.sec_uid}`), await this.e.reply(`群：${groupInfo.groupName}(${groupId})\n删除成功！${UserInfoData.data.user.nickname}\n抖音号：${user_shortid}`), 0 === existingItem.group_id.length) {
						const index$1 = config$1.douyin.indexOf(existingItem);
						config$1.douyin.splice(index$1, 1);
					}
				} else existingItem.group_id.push(`${groupId}:${botId}`), isSubscribed || await douyinDBInstance.subscribeDouyinUser(groupId, botId, sec_uid, user_shortid, UserInfoData.data.user.nickname), await this.e.reply(`群：${groupInfo.groupName}(${groupId})\n添加成功！${UserInfoData.data.user.nickname}\n抖音号：${user_shortid}`), !1 === Config.douyin.push.switch && await this.e.reply("请发送「#kkk设置抖音推送开启」以进行推送"), logger.info(`\n设置成功！${UserInfoData.data.user.nickname}\n抖音号：${user_shortid}\nsec_uid${UserInfoData.data.user.sec_uid}`);
			} else config$1.douyin.push({
				switch: !0,
				sec_uid,
				group_id: [`${groupId}:${botId}`],
				remark: UserInfoData.data.user.nickname,
				short_id: user_shortid
			}), isSubscribed || await douyinDBInstance.subscribeDouyinUser(groupId, botId, sec_uid, user_shortid, UserInfoData.data.user.nickname), await this.e.reply(`群：${groupInfo.groupName}(${groupId})\n添加成功！${UserInfoData.data.user.nickname}\n抖音号：${user_shortid}`), !1 === Config.douyin.push.switch && await this.e.reply("请发送「#kkk设置抖音推送开启」以进行推送"), logger.info(`\n设置成功！${UserInfoData.data.user.nickname}\n抖音号：${user_shortid}\nsec_uid${UserInfoData.data.user.sec_uid}`);
			Config.Modify("pushlist", "douyin", config$1.douyin), await this.renderPushList();
		} catch (error) {
	...
	async renderPushList() {
		await this.syncConfigToDatabase();
		const groupInfo = await this.e.bot.getGroupInfo("groupId" in this.e && this.e.groupId ? this.e.groupId : ""), subscriptions = await douyinDBInstance.getGroupSubscriptions(groupInfo.groupId);
		if (0 === subscriptions.length) return void await this.e.reply(`当前群：${groupInfo.groupName}(${groupInfo.groupId})\n没有设置任何抖音博主推送！\n可使用「#设置抖音推送 + 抖音号」进行设置`);
		const renderOpt = [];
		for (const subscription of subscriptions) {
			const sec_uid = subscription.sec_uid, userInfo = await this.amagi.getDouyinData("用户主页数据", {
				sec_uid,
				typeMode: "strict"
			});
			renderOpt.push({
	...
	async forcepush(data) {
		...
		if (this.e.msg.includes("全部")) await this.getdata(data);
			...
			const subscribedUids = (await douyinDBInstance.getGroupSubscriptions(currentGroupId)).map((sub) => sub.sec_uid), filteredData = {};
			for (const awemeId in data) subscribedUids.includes(data[awemeId].sec_uid) && (filteredData[awemeId] = {
				...data[awemeId],
				targets: [{
					groupId: currentGroupId,
					botId: currentBotId
	...
	async checkremark() {
		const config$1 = Config.pushlist, updateList = [];
		if (null === Config.pushlist.douyin || 0 === Config.pushlist.douyin.length) return !0;
		for (const i of Config.pushlist.douyin) {
			const remark = i.remark, sec_uid = i.sec_uid;
			void 0 !== remark && "" !== remark || updateList.push({ sec_uid });
		}
		if (updateList.length > 0) {
			for (const i of updateList) {
				const remark = (await this.amagi.getDouyinData("用户主页数据", {
					sec_uid: i.sec_uid,
					typeMode: "strict"
				})).data.user.nickname, matchingItemIndex = config$1.douyin.findIndex((item) => item.sec_uid === i.sec_uid);
				-1 !== matchingItemIndex && (config$1.douyin[matchingItemIndex].remark = remark);
			}
...
}, skipDynamic = async (PushItem) => {
	if ("liveStatus" in PushItem.Detail_Data) return !1;
	const tags = [];
	if (PushItem.Detail_Data.text_extra) for (const item of PushItem.Detail_Data.text_extra) item.hashtag_name && tags.push(item.hashtag_name);
	return logger.debug(`检查作品是否需要过滤：${PushItem.Detail_Data.share_url}`), await douyinDBInstance.shouldFilter(PushItem, tags);
};
function getRelativeTimeFromTimestamp$1(timestamp) {
	const timestampInSeconds = Math.floor(timestamp / 1e3), differenceInSeconds = Math.floor(Date.now() / 1e3) - timestampInSeconds;
	if (differenceInSeconds < 30) return "刚刚";
	if (differenceInSeconds < 60) return differenceInSeconds + "秒前";
	if (differenceInSeconds < 3600) return Math.floor(differenceInSeconds / 60) + "分钟前";
	if (differenceInSeconds < 86400) return Math.floor(differenceInSeconds / 3600) + "小时前";
...
init_Config(), init_esm(), init_Config(), await init_module(), await init_module(), await init_Config();
var Kuaishou = class extends Base {
	e;
	type;
	is_mp4;
	constructor(e, iddata) {
		super(e), this.e = e, this.type = iddata?.type;
	}
	async RESOURCES(data) {
		if (Config.app.EmojiReply && await this.e.bot.setMsgReaction(this.e.contact, this.e.messageId, Config.app.EmojiReplyID, !0), 1 !== data.VideoData.data.data.visionVideoDetail.status) return await this.e.reply("不支持解析的视频"), !0;
		Config.kuaishou.tip && await this.e.reply("检测到快手链接，开始解析");
		const video_url = data.VideoData.data.data.visionVideoDetail.photo.photoUrl, transformedData = Object.entries(data.EmojiData.data.data.visionBaseEmoticons.iconUrls).map(([name, path$1]) => ({
			name,
			url: `https:${path$1}`
		})), CommentsData = await async function(data, emojidata) {
			let jsonArray = [];
			for (const i of data.data.visionCommentList.rootComments) {
				const cid = i.commentId, aweme_id = i.commentId, nickname = i.authorName, userimageurl = i.headurl, text = i.content, time = getRelativeTimeFromTimestamp$1(i.timestamp), commentObj = {
					cid,
					aweme_id,
					nickname,
					userimageurl,
			...
			jsonArray.sort((a, b) => b.digg_count - a.digg_count), jsonArray = function(data) {
				for (const i of data) {
					let text = i.text;
					text = text.replace(/\n/g, "<br>"), i.text = text;
				}
				return data;
			}(jsonArray), jsonArray = await function(data) {
				for (const i of data) {
					let text = i.text;
					text = text.replace(/(@[\S\s]+?)\(\w+\)/g, (match, p1) => `<span style="color: rgb(3,72,141);">${p1.trim()}</span>`), i.text = text;
				}
				return data;
			}(jsonArray);
			for (const i of jsonArray) i.digg_count > 1e4 && (i.digg_count = (i.digg_count / 1e4).toFixed(1) + "w");
			for (const item1 of jsonArray) for (const item2 of emojidata) item1.text.includes(item2.name) && (item1.text.includes("[") && item1.text.includes("]") ? item1.text = item1.text.replace(/\[[^\]]*\]/g, `<img src="${item2.url}"/>`).replace(/\\/g, "") : item1.text = `<img src="${item2.url}"/>`, item1.text += "&#160");
			return jsonArray.slice(0, Math.min(jsonArray.length, Config.kuaishou.numcomment));
		}(data.CommentsData.data, transformedData), fileHeaders = await new Networks({
			url: video_url,
			headers: this.headers
		}).getHeaders(), fileSizeInMB = ((fileHeaders["content-range"]?.match(/\/(\d+)/) ? parseInt(fileHeaders["content-range"]?.match(/\/(\d+)/)[1], 10) : 0) / 1048576).toFixed(2), img$2 = await Render("kuaishou/comment", {
			Type: "视频",
		...
		return await this.e.reply(img$2), await downloadVideo(this.e, {
			video_url,
			title: {
				timestampTitle: `tmp_${Date.now()}.mp4`,
...
init_bilibili$1(), await init_module(), await init_Config();
const task = Config.app.removeCache && karin.task("[kkk-视频缓存自动删除]", "0 0 4 * * *", async () => {
	try {
		await removeAllFiles(Common.tempDri.video), logger.mark(Common.tempDri.video + "目录下所有文件已删除");
	} catch (err) {
		console.error("删除文件时出错:", err);
	}
}), biLogin = karin.command(/^#?(kkk)?\s*B站\s*(扫码)?\s*登录$/i, async (e) => (await bilibiliLogin(e), !0), {
	perm: Config.bilibili.loginPerm,
	name: "kkk-ck管理"
}), dylogin = karin.command(/^#?(kkk)?抖音(扫码)?登录$/, async (e) => (await e.reply("暂时不可用"), !0), {
	perm: Config.douyin.loginPerm,
	name: "kkk-ck管理"
}), setdyck = karin.command(/^#?(kkk)?s*设置抖音ck$/i, async (e) => {
	const msg = await e.reply("请发在120秒内送抖音ck\n教程：https://ikenxuan.github.io/kkkkkk-10086/docs/intro/other#%E9%85%8D%E7%BD%AE%E4%B8%8D%E5%90%8C%E5%B9%B3%E5%8F%B0%E7%9A%84-cookies\n"), context = await karin.ctx(e);
	return Config.Modify("cookies", "douyin", context.msg), await e.bot.recallMsg(e.contact, msg.messageId), await e.reply("设置成功！", { at: !0 }), !0;
}, {
	perm: "master",
	name: "kkk-ck管理",
	event: "message.friend"
}), setbilick = karin.command(/^#?(kkk)?s*设置s*(B站)ck$/i, async (e) => {
	const msg = await e.reply("请发在120秒内送B站ck\n教程：https://ikenxuan.github.io/kkkkkk-10086/docs/intro/other#%E9%85%8D%E7%BD%AE%E4%B8%8D%E5%90%8C%E5%B9%B3%E5%8F%B0%E7%9A%84-cookies\n"), context = await karin.ctx(e);
	return Config.Modify("cookies", "bilibili", context.msg), await e.bot.recallMsg(e.contact, msg.messageId), await e.reply("设置成功！", { at: !0 }), !0;
}, {
	perm: "master",
	name: "kkk-ck管理",
	event: "message.friend"
...
async function removeAllFiles(dir) {
	...
	for (const file of files) {
		...
		(await fs.promises.stat(filePath)).isDirectory() ? (await removeAllFiles(filePath), await fs.promises.rmdir(filePath)) : await fs.promises.unlink(filePath);
	}
}
await init_module(), await init_Config();
var HELP_MENU_CONFIG = [
	{
		...
		items: [{
			...
			description: (() => {
				const platforms = [];
				return Config.douyin?.switch && platforms.push("抖音"), Config.bilibili?.switch && platforms.push("哔哩哔哩"), Config.kuaishou?.switch && platforms.push("快手"), Config.xiaohongshu?.switch && platforms.push("小红书"), platforms.length > 0 ? `支持「${platforms.join("」「")}」` : "暂无可用平台";
			})(),
			icon: "Link",
			roles: ["member", "master"]
		}, {
	...
	},
	{
		...
		subGroups: [{
			...
			items: [{
				...
				icon: "Bell",
				roles: "all" === Config.douyin.push.permission ? ["member", "master"] : ["master"]
			}, {
				title: "#设置B站推送 + UP主UID",
				description: "在群聊中发送以对该群订阅该blogger的作品更新",
				icon: "Bell",
				roles: "all" === Config.bilibili.push.permission ? ["member", "master"] : ["master"]
			}]
		}]
	},
	{
...
function buildMenuForRole(role) {
	...
	return HELP_MENU_CONFIG.map((group) => {
		const items = filterItems(group.items), subGroups = group.subGroups?.map((sg) => ({
			title: sg.title,
...
const help = karin.command(/^#?kkk帮助$/, async (e) => {
	const masters = config.master().filter((id) => "console" !== id), role = e.sender && masters.includes(e.sender.userId) ? "master" : "member", img$2 = await Render("other/help", {
		title: "KKK插件帮助页面",
		menu: buildMenuForRole(role),
		role
	});
	return await e.reply(img$2), !0;
}, { name: "kkk-帮助" }), version = karin.command(/^#?kkk(版本|更新日志)$/, async (e) => {
	const changelogContent = fs.readFileSync(Root.pluginPath + "/CHANGELOG.md", "utf8"), img$2 = await Render("other/changelog", {
		markdown: logs(Root.pluginVersion, changelogContent, 10, !1),
		Tip: !1
	});
	return e.reply(img$2), !0;
}, { name: "kkk-版本" });
init_esm();
var LogCollector = class LogCollector {
	...
	static isAmagiIntercepted = !1;
	constructor() {
		LogCollector.isAmagiIntercepted || (this.interceptAmagiLogger(), LogCollector.isAmagiIntercepted = !0), this.registerInstance();
	}
	startCollecting() {
		this.isCollecting || (this.isCollecting = !0, this.logs = []);
	}
	stopCollecting() {
		this.isCollecting && (this.isCollecting = !1, this.unregisterInstance());
	}
	interceptAmagiLogger() {
		this.originalAmagiWarn = logger$1.warn.bind(logger$1), this.originalAmagiError = logger$1.error.bind(logger$1), logger$1.warn = (message, ...args) => {
			LogCollector.collectToAllInstances("warn", [message, ...args]), this.originalAmagiWarn && this.originalAmagiWarn(message, ...args);
		}, logger$1.error = (message, ...args) => {
			LogCollector.collectToAllInstances("error", [message, ...args]), this.originalAmagiError && this.originalAmagiError(message, ...args);
		};
	}
	static activeInstances = [];
	static collectToAllInstances(level, args) {
		LogCollector.activeInstances.forEach((instance) => {
			instance.isCollecting && instance.collectAmagiLog(level, args);
		});
	}
	collectAmagiLog(level, args) {
		...
		const logEntry = {
			level,
			message: args.map((arg) => "string" == typeof arg ? arg : JSON.stringify(arg)).join(" "),
	...
	registerInstance() {
		LogCollector.activeInstances.includes(this) || LogCollector.activeInstances.push(this);
	}
	unregisterInstance() {
		const index = LogCollector.activeInstances.indexOf(this);
		index > -1 && LogCollector.activeInstances.splice(index, 1);
	}
};
await init_module(), await init_Config();
const wrapWithErrorHandler = (fn, options) => async (...args) => {
	...
	try {
		return logCollector.startCollecting(), await fn(...args);
	} catch (error) {
		const collectedLogs = logCollector.getFormattedLogs();
		if (!Config.app.errorLogSendTo) throw error;
		throw await handleBusinessError(error, options, collectedLogs, args[0]), error;
	} finally {
		logCollector.stopCollecting();
	}
};
var handleBusinessError = async (error, options, logs$1, event) => {
	const triggerCommand = event?.msg || "未知命令或处于非消息环境", img$2 = await Render("other/handlerError", {
		type: "business_error",
		platform: "system",
		error: {
	...
	if (event && Config.app.errorLogSendTo.some((item) => "trigger" === item)) try {
		event.reply(img$2);
	} catch (replyError) {
		logger.error(`发送错误消息给用户失败: ${replyError}`);
	}
	if (Config.app.errorLogSendTo.some((item) => "master" === item)) try {
		const botId = (pushlist = Config.pushlist, {
			douyin: { botId: pushlist.douyin?.[0]?.group_id?.[0]?.split(":")?.[1] || "" },
			bilibili: { botId: pushlist.bilibili?.[0]?.group_id?.[0]?.split(":")?.[1] || "" }
		}), list = config.master();
		let master = list[0];
		"console" === master && (master = list[1]);
		const selectedBotId = botId.douyin.botId || botId.bilibili.botId || "", isPushTask = !event || options.businessName.includes("推送");
		if (selectedBotId && master) if (isPushTask) await karin.sendMaster(selectedBotId, master, [segment.text(`${options.businessName} 任务执行出错！\n请即时解决以消除警告`), ...img$2]);
			...
			const Adapter = karin.getBot(selectedBotId), groupID = event && "groupId" in event ? event.groupId : "", groupInfo = await Adapter?.getGroupInfo(groupID);
...
await init_esm(), await init_module(), await init_db(), await init_Config();
var handleDouyinPush = wrapWithErrorHandler(async () => (await new DouYinpush().action(), !0), { businessName: "抖音推送任务" }), handleBilibiliPush = wrapWithErrorHandler(async () => (await new Bilibilipush().action(), !0), { businessName: "B站推送任务" }), handleForcePush = wrapWithErrorHandler(async (e) => e.msg.includes("抖音") ? (await new DouYinpush().action(), !0) : !e.msg.includes("B站") || (await new Bilibilipush().action(), !0), { businessName: "强制推送" }), handleSetDouyinPush = wrapWithErrorHandler(async (e) => {
	const data = await getDouyinData("搜索数据", Config.cookies.douyin, {
		query: e.msg.replace(/^#设置抖音推送/, ""),
		typeMode: "strict"
	});
	return await new DouYinpush(e).setting(data.data), !0;
}, { businessName: "设置抖音推送" }), handleSetBilibiliPush = wrapWithErrorHandler(async (e) => {
	if (!Config.cookies.bilibili) return await e.reply("\n请先配置B站Cookie", { at: !0 }), !0;
	...
	return !0;
}, { businessName: "设置B站推送" }), handleBilibiliPushList = wrapWithErrorHandler(async (e) => {
	await new Bilibilipush(e).renderPushList();
}, { businessName: "B站推送列表" }), handleDouyinPushList = wrapWithErrorHandler(async (e) => {
	await new DouYinpush(e).renderPushList();
}, { businessName: "抖音推送列表" }), handleChangeBotID = wrapWithErrorHandler(async (e) => {
	const newBotId = e.msg.replace(/^#kkk设置推送机器人/, ""), newDouyinlist = Config.pushlist.douyin.map((item) => {
		const modifiedGroupIds = item.group_id.map((groupId) => {
			const [group_id, oldBotId] = groupId.split(":");
			return oldBotId && oldBotId !== newBotId && douyinDBInstance.updateGroupBotId(group_id, oldBotId, newBotId).catch((err) => {
	...
	}), newBilibililist = Config.pushlist.bilibili.map((item) => {
		const modifiedGroupIds = item.group_id.map((groupId) => {
			const [group_id, oldBotId] = groupId.split(":");
			return oldBotId && oldBotId !== newBotId && bilibiliDBInstance.updateGroupBotId(group_id, oldBotId, newBotId).catch((err) => {
	...
	return Config.Modify("pushlist", "douyin", newDouyinlist), Config.Modify("pushlist", "bilibili", newBilibililist), await e.reply("推送机器人已修改为" + newBotId), !0;
}, { businessName: "设置推送机器人" }), handleTestDouyinPush = wrapWithErrorHandler(async (e) => {
	const iddata = await getDouyinID(0, String(e.msg.match(/(http|https):\/\/.*\.(douyin|iesdouyin)\.com\/[^ ]+/g))), workInfo = await amagi.getDouyinData("聚合解析", {
		aweme_id: iddata.aweme_id,
		typeMode: "strict"
	}, Config.cookies.douyin), userProfile = await amagi.getDouyinData("用户主页数据", {
		sec_uid: workInfo.data.aweme_detail.author.sec_uid,
		typeMode: "strict"
	}, Config.cookies.douyin), realUrl = "web" === Config.douyin.push.shareType && await new Networks({
	...
	}).getLocation(), img$2 = await Render("douyin/dynamic", {
		...
		share_url: "web" === Config.douyin.push.shareType ? realUrl : `https://aweme.snssdk.com/aweme/v1/play/?video_id=${workInfo.data.aweme_detail.video.play_addr.uri}&ratio=1080p&line=0`,
		...
		cooperation_info: (() => {
			...
			if (!raw) return;
			const rawCreators = Array.isArray(raw.co_creators) ? raw.co_creators : [], author = workInfo.data.aweme_detail.author, authorUid = author?.uid, authorSecUid = author?.sec_uid, authorNickname = author?.nickname, authorInCreators = rawCreators.some((c) => authorUid && c.uid && c.uid === authorUid || authorSecUid && c.sec_uid && c.sec_uid === authorSecUid || authorNickname && c.nickname && c.nickname === authorNickname), co_creators = rawCreators.map((c) => {
				const firstUrl = c.avatar_thumb?.url_list?.[0] ?? (c.avatar_thumb?.uri ? `https://p3.douyinpic.com/${c.avatar_thumb.uri}` : void 0);
				return {
					avatar_thumb: firstUrl ? { url_list: [firstUrl] } : void 0,
					nickname: c.nickname,
...
const douyinPush = Config.douyin.push.switch && karin.task("抖音推送", Config.douyin.push.cron, handleDouyinPush, { log: Config.douyin.push.log }), bilibiliPush = Config.bilibili.push.switch && karin.task("B站推送", Config.bilibili.push.cron, handleBilibiliPush, { log: Config.bilibili.push.log }), forcePush = karin.command(/#(抖音|B站)(全部)?强制推送/, handleForcePush, {
	name: "𝑪𝒊𝒂𝒍𝒍𝒐～(∠・ω< )⌒★",
	perm: "master",
	event: "message.group"
}), setdyPush = karin.command(/^#设置抖音推送/, handleSetDouyinPush, {
	name: "kkk-推送功能-设置",
	event: "message.group",
	perm: Config.douyin.push.permission,
	dsbAdapter: ["qqbot"]
}), setbiliPush = karin.command(/^#设置[bB]站推送(?:[Uu][Ii][Dd]:)?(\d+)$/, handleSetBilibiliPush, {
	name: "kkk-推送功能-设置",
	event: "message.group",
	perm: Config.bilibili.push.permission,
	dsbAdapter: ["qqbot"]
}), bilibiliPushList = karin.command(/^#?[bB]站推送列表$/, handleBilibiliPushList, {
	name: "kkk-推送功能-列表",
	event: "message.group"
}), douyinPushList = karin.command(/^#?抖音推送列表$/, handleDouyinPushList, {
	name: "kkk-推送功能-列表",
	event: "message.group"
}), changeBotID = karin.command(/^#kkk设置推送机器人/, handleChangeBotID, {
	name: "kkk-推送功能-设置",
	perm: "master"
}), testDouyinPush = karin.command(/^#测试抖音推送\s*(https?:\/\/[^\s]+)?/, handleTestDouyinPush, {
	name: "kkk-推送功能-测试",
	event: "message.group",
	perm: Config.douyin.push.permission,
	dsbAdapter: ["qqbot"]
...
var processCommentEmojis = (text, emojiData) => {
	...
	return processedText.split(/(<img[^>]*>)/).map((part) => part.startsWith("<img") ? part : part.trim() ? `<span>${part}</span>` : part).join("");
}, processAtUsers = (text, atUsers, useDarkTheme = !1) => {
	if (!text || !atUsers || !Array.isArray(atUsers) || 0 === atUsers.length) return text;
	let processedText = text;
	for (const atUser of atUsers) if (atUser.nickname && processedText.includes(`@${atUser.nickname}`)) {
		const escapedNickname = atUser.nickname.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), regex = new RegExp(`@${escapedNickname}`, "g"), spanClass = useDarkTheme ? "text-[#c7daef]" : "text-[#13386c]";
...
function getRelativeTimeFromTimestamp(timestamp) {
	const now = Date.now(), differenceInSeconds = Math.floor((now - timestamp) / 1e3);
	if (differenceInSeconds < 30) return "刚刚";
	if (differenceInSeconds < 60) return differenceInSeconds + "秒前";
	if (differenceInSeconds < 3600) return Math.floor(differenceInSeconds / 60) + "分钟前";
	if (differenceInSeconds < 86400) return Math.floor(differenceInSeconds / 3600) + "小时前";
...
var Xiaohongshu = class extends Base {
	...
	async RESOURCES(data) {
		Config.app.EmojiReply && !this.e.isPrivate && await this.e.bot.setMsgReaction(this.e.contact, this.e.messageId, Config.app.EmojiReplyID, !0), Config.xiaohongshu.tip && await this.e.reply("检测到小红书链接，开始解析");
		const NoteData = await this.amagi.getXiaohongshuData("单个笔记数据", {
			typeMode: "strict",
			note_id: data.note_id,
			xsec_token: data.xsec_token
		}), formattedEmojis = XiaohongshuEmoji(await this.amagi.getXiaohongshuData("表情列表", { typeMode: "strict" }));
		if (Config.xiaohongshu.sendContent.some((item) => "info" === item)) {
		...
		if (Config.xiaohongshu.sendContent.some((item) => "comment" === item)) {
			const CommentData = await this.amagi.getXiaohongshuData("评论数据", {
				typeMode: "strict",
				note_id: data.note_id,
				xsec_token: data.xsec_token
			});
			if (CommentData.data.data.comments && 0 !== CommentData.data.data.comments.length) {
				const processedComments = await async function(data, emojiData, useDarkTheme = !1) {
					if (!data.data || !data.data.comments || 0 === data.data.comments.length) return [];
					const comments = [];
					for (const comment of data.data.comments) {
						const processedComment = {
							...
							isTop: Array.isArray(comment.show_tags) && comment.show_tags.some((t) => "string" == typeof t ? "user_top" === t : !(!t || "object" != typeof t || "user_top" !== t.name && "user_top" !== t.tag))
						};
						comment.sub_comments && Array.isArray(comment.sub_comments) && (processedComment.sub_comments = comment.sub_comments.map((subComment) => ({
							id: subComment.id,
							...
							isTop: Array.isArray(subComment.show_tags) && subComment.show_tags.some((t) => "string" == typeof t ? "user_top" === t : !(!t || "object" != typeof t || "user_top" !== t.name && "user_top" !== t.tag))
						}))), comments.push(processedComment);
					}
					for (const comment of comments) if (comment.content = comment.content.replace(/\n/g, "<br>").replace(/ {2,}/g, (match) => "&nbsp;".repeat(match.length)), comment.content = processAtUsers(comment.content, comment.at_users, useDarkTheme), comment.content = processCommentEmojis(comment.content, emojiData), parseInt(comment.like_count) > 1e4 && (comment.like_count = (parseInt(comment.like_count) / 1e4).toFixed(1) + "w"), comment.sub_comments && Array.isArray(comment.sub_comments)) for (const subComment of comment.sub_comments) subComment.content = subComment.content.replace(/\n/g, "<br>").replace(/ {2,}/g, (match) => "&nbsp;".repeat(match.length)), subComment.content = processAtUsers(subComment.content, subComment.at_users, useDarkTheme), subComment.content = processCommentEmojis(subComment.content, emojiData), parseInt(subComment.like_count) > 1e4 && (subComment.like_count = (parseInt(subComment.like_count) / 1e4).toFixed(1) + "w");
					return comments.sort((a, b) => Number(b.isTop) - Number(a.isTop)), comments.slice(0, Config.xiaohongshu.numcomment);
				}(CommentData.data, formattedEmojis), commentListImg = await Render("xiaohongshu/comment", {
			...
			} else await this.e.reply("这个笔记没有评论 ~");
		}
		if (!NoteData.data.data.items[0].note_card.video && Config.xiaohongshu.sendContent.includes("image")) {
			...
			1 === NoteData.data.data.items[0].note_card.image_list.length ? await this.e.reply(segment.image(NoteData.data.data.items[0].note_card.image_list[0].url_default)) : await this.e.bot.sendForwardMsg(this.e.contact, res, {
				source: "图片合集",
				summary: `查看${res.length}张图片消息`,
				prompt: "小红书图集解析结果",
				news: [{ text: "点击查看解析结果" }]
		...
		if (NoteData.data.data.items[0].note_card.video && Config.xiaohongshu.sendContent.includes("video")) {
			const video = NoteData.data.data.items[0].note_card.video, selectedVideo = function(streamData, videoQuality, maxAutoVideoSize) {
				if (!streamData) return logger.warn("没有找到视频流数据"), null;
				const codecPriority = [
					"h265",
					"h264",
					"av1",
				...
				for (const codec of codecPriority) streamData[codec] && Array.isArray(streamData[codec]) && allVideos.push(...streamData[codec]);
				if (0 === allVideos.length) return logger.warn("没有找到可用的视频流"), null;
				logger.debug(`找到 ${allVideos.length} 个视频流`);
				const getQualityLevel = (width, height) => {
					const pixels = width * height;
					return pixels >= 8294400 ? "4k" : pixels >= 3686400 ? "2k" : pixels >= 2073600 ? "1080p" : pixels >= 921600 ? "720p" : "540p";
				}, videosByQuality = /* @__PURE__ */ new Map();
				if (allVideos.forEach((video) => {
					...
					for (const quality of [
						...
						if (qualityVideos && qualityVideos.length > 0) {
							...
							if (suitableVideo) return logger.debug(`自动选择画质: ${quality}, 文件大小: ${(suitableVideo.size / 1048576).toFixed(2)}MB, 编码: ${suitableVideo.video_codec}`), suitableVideo;
						}
					}
					let smallestVideo = allVideos[0];
					return allVideos.forEach((video) => {
						video.size < smallestVideo.size && (smallestVideo = video);
					}), logger.debug(`未找到符合大小限制的视频，选择最小视频: ${(smallestVideo.size / 1048576).toFixed(2)}MB, 编码: ${smallestVideo.video_codec}`), smallestVideo;
				}
				const targetQuality = videoQuality, targetVideos = videosByQuality.get(targetQuality);
				if (targetVideos && targetVideos.length > 0) return logger.debug(`选择固定画质: ${targetQuality}, 文件大小: ${(targetVideos[0].size / 1048576).toFixed(2)}MB, 编码: ${targetVideos[0].video_codec}`), targetVideos[0];
				const qualityPriority = [
					"4k",
					"2k",
					"1080p",
				...
				for (let i = targetIndex + 1; i < qualityPriority.length; i++) {
					const fallbackVideos = videosByQuality.get(qualityPriority[i]);
					if (fallbackVideos && fallbackVideos.length > 0) return logger.debug(`目标画质 ${targetQuality} 不可用，降级到: ${qualityPriority[i]}, 编码: ${fallbackVideos[0].video_codec}`), fallbackVideos[0];
				}
				for (let i = targetIndex - 1; i >= 0; i--) {
					const fallbackVideos = videosByQuality.get(qualityPriority[i]);
					if (fallbackVideos && fallbackVideos.length > 0) return logger.debug(`目标画质 ${targetQuality} 不可用，升级到: ${qualityPriority[i]}, 编码: ${fallbackVideos[0].video_codec}`), fallbackVideos[0];
				}
				return logger.warn("未找到任何匹配的画质，返回默认视频"), allVideos[0];
			}(video.media?.stream, Config.xiaohongshu.videoQuality, Config.xiaohongshu.maxAutoVideoSize);
			selectedVideo ? await downloadVideo(this.e, {
			...
			}, { message_id: this.e.messageId }) : await this.e.reply(segment.video(video.url_default));
		}
		return !0;
	}
};
...
await init_module(), await init_Config(), await init_bilibili$1();
var reg = {
...
}, handleDouyin = wrapWithErrorHandler(async (e) => {
	const iddata = await getDouyinID(0, String(e.msg.match(/(http|https):\/\/.*\.(douyin|iesdouyin)\.com\/[^ ]+/g)));
	return await new DouYin(e, iddata).RESOURCES(iddata), !0;
}, { businessName: "抖音视频解析" }), handleBilibili = wrapWithErrorHandler(async (e) => {
	e.msg = e.msg.replace(/\\/g, "");
	let url = null;
	const urlMatch = e.msg.match(/(https?:\/\/(?:www\.bilibili\.com|m\.bilibili\.com|t\.bilibili\.com|b23\.tv|bili2233\.cn)\/[a-zA-Z0-9_\-.~:\/?#[\]@!$&'()*+,;=]+)/);
	if (urlMatch ? url = urlMatch[0] : /^BV[1-9a-zA-Z]{10}$/.test(e.msg) && (url = `https://www.bilibili.com/video/${e.msg}`), !url) return logger.warn(`未能在消息中找到有效的B站分享链接或BV号: ${e.msg}`), !0;
	const iddata = await async function(url) {
		const resp = await axios.get(url, { headers: { "User-Agent": "Apifox/1.0.0 (https://apifox.com)" } }), longLink = resp?.request?.res?.responseUrl ?? resp?.config?.url ?? url;
		let pValue, result = {};
		const parsedUrl = new URL(longLink), pParam = parsedUrl.searchParams.get("p");
		pParam && (pValue = parseInt(pParam, 10), isNaN(pValue) && (pValue = void 0));
		const pathname = parsedUrl.pathname, hostname = parsedUrl.hostname;
		switch (!0) {
			case "t.bilibili.com" === hostname && /^\/\d+/.test(pathname) || "www.bilibili.com" === hostname && /^\/opus\/\d+/.test(pathname): {
				const tMatch = "t.bilibili.com" === hostname ? pathname.match(/^\/(\d+)/) : null, opusMatch = "www.bilibili.com" === hostname ? pathname.match(/^\/opus\/(\d+)/) : null, dynamic_id = tMatch ?? opusMatch;
				result = {
					type: "dynamic_info",
			...
			case /\/bangumi\/play\/(\w+)/.test(longLink): {
				const playMatch = /\/bangumi\/play\/(\w+)/.exec(longLink), id = playMatch ? playMatch[1] : "";
				let realid = "", isEpid = !1;
				id.startsWith("ss") ? realid = id : id.startsWith("ep") && (realid = id, isEpid = !0), result = {
			...
			case /play\/(\S+?)\??/.test(longLink): {
				const playMatch = /play\/(\w+)/.exec(longLink), id = playMatch ? playMatch[1] : "";
				id.startsWith("ss") ? result.realid = "season_id" : id.startsWith("ep") && (result.realid = "ep_id"), result = {
					type: "bangumi_video_info",
					isEpid: !1,
			...
			case /^https:\/\/t\.bilibili\.com\/(\d+)/.test(longLink) || /^https:\/\/www\.bilibili\.com\/opus\/(\d+)/.test(longLink): {
				const tMatch = /^https:\/\/t\.bilibili\.com\/(\d+)/.exec(longLink), opusMatch = /^https:\/\/www\.bilibili\.com\/opus\/(\d+)/.exec(longLink), dynamic_id = tMatch ?? opusMatch;
				result = {
					type: "dynamic_info",
	...
	return await new Bilibili(e, iddata).RESOURCES(iddata), !0;
}, { businessName: "B站视频解析" }), handleKuaishou = wrapWithErrorHandler(async (e) => {
	const kuaishouUrl = e.msg.replaceAll("\\", "").match(/(https:\/\/v\.kuaishou\.com\/\w+|https:\/\/www\.kuaishou\.com\/f\/[a-zA-Z0-9]+)/g), iddata = await async function(url, log = !0) {
		...
		return log && console.log(result), result;
	}(String(kuaishouUrl)), WorkData = await async function(type, opt) {
		const client = Client({
...
}, { businessName: "快手视频解析" }), handleXiaohongshu = wrapWithErrorHandler(async (e) => {
	const url = e.msg.replaceAll("\\", "").match(/https?:\/\/[^\s"'<>]+/)?.[0];
	if (!url) return logger.warn(`未能在消息中找到有效链接: ${e.msg}`), !0;
	const iddata = await async function(url, log = !0) {
		const longLink = (await axios.get(url, { headers: { "User-Agent": "Apifox/1.0.0 (https://apifox.com)" } }))?.request?.res?.responseUrl ?? url, normalizedLink = (() => {
			try {
			...
			try {
				const u = new URL(s), t = u.searchParams.get("xsec_token") || u.searchParams.get("XSEC_TOKEN") || void 0;
				if (t) return t;
				if (u.hash) {
		...
		}, finalToken = pickToken(effectiveLink) ?? pickToken(normalizedLink);
		...
		if ("unknown" === result.type) throw new Error("无法从链接中提取小红书笔记ID");
		return log && console.log(result), result;
	}(url);
	return await new Xiaohongshu(e, iddata).RESOURCES(iddata), !0;
}, { businessName: "小红书视频解析" }), handlePrefix = wrapWithErrorHandler(async (e) => (e.msg = await Common.getReplyMessage(e), reg.douyin.test(e.msg) ? await handleDouyin(e) : reg.bilibili.test(e.msg) ? await handleBilibili(e) : reg.kuaishou.test(e.msg) ? await handleKuaishou(e) : reg.xiaohongshu.test(e.msg) ? await handleXiaohongshu(e) : void 0), { businessName: "引用解析" }), douyin = karin.command(reg.douyin, handleDouyin, {
	name: "kkk-视频功能-抖音",
	priority: Config.app.videoTool ? -Infinity : 800
}), bilibili = karin.command(reg.bilibili, handleBilibili, {
	name: "kkk-视频功能-B站",
	priority: Config.app.videoTool ? -Infinity : 800
}), kuaishou = karin.command(reg.kuaishou, handleKuaishou, {
	name: "kkk-视频功能-快手",
	priority: Config.app.videoTool ? -Infinity : 800
}), xiaohongshu = karin.command(reg.xiaohongshu, handleXiaohongshu, {
	name: "kkk-视频功能-小红书",
	priority: Config.app.videoTool ? -Infinity : 800
});
const prefix = karin.command(/^#?(解析|kkk解析)/, handlePrefix, { name: "kkk-视频功能-引用解析" }), douyinAPP = Config.douyin.switch && douyin, bilibiliAPP = Config.bilibili.switch && bilibili, kuaishouAPP = Config.kuaishou.switch && kuaishou, xiaohongshuAPP = Config.xiaohongshu.switch && xiaohongshu;
await init_module();
var versionCore = (v) => {
	((v = v.trim()).startsWith("v") || v.startsWith("V")) && (v = v.slice(1));
	const [preBuild] = v.split("+", 2), [core] = preBuild.split("-", 2);
	return core;
};
const getChangelogImage = async (props) => {
	...
	const requests = [
		...
		if ("string" == typeof res.data && res.data.length > 0) return res.data;
	...
	return changelog && await Render("other/changelog", {
	...
	const parse = (v) => {
		((v = v.trim()).startsWith("v") || v.startsWith("V")) && (v = v.slice(1));
		const [preBuild] = v.split("+", 2), [core, pre] = preBuild.split("-", 2), parts = core.split(".");
		return {
		...
		return an && !bn ? -1 : !an && bn ? 1 : a === b ? 0 : a > b ? 1 : -1;
	}, r = parse(remote), l = parse(local);
	if (r.major !== l.major) return r.major > l.major;
	if (r.minor !== l.minor) return r.minor > l.minor;
	if (r.patch !== l.patch) return r.patch > l.patch;
	const ra = r.prerelease, la = l.prerelease;
	if (0 === ra.length && 0 === la.length) return !1;
	if (0 === ra.length && la.length > 0) return !0;
	if (ra.length > 0 && 0 === la.length) return !1;
	...
	return ra.length !== la.length && ra.length > la.length;
};
await init_module();
const kkkUpdate = hooks.message.friend(async (e, next) => {
	if (e.msg.includes("更新")) {
		const msgId = await db.get("kkk:update:msgId");
			...
			if ("yes" === upd.status) {
				...
				if ("ok" === result.status) {
					if ((await e.reply(`${Root.pluginName} 更新成功！\n${result.local} -> ${result.remote}\n开始执行重启......`)).messageId) try {
						await db.del("kkk:update:msgId"), await db.del("kkk:update:lock");
					} catch {}
					await restart(e.selfId, e.contact, e.messageId);
				} else await e.reply(`${Root.pluginName} 更新失败: ${result.data ?? "更新执行失败"}`);
			} else "no" === upd.status ? await e.reply("未检测到可更新版本。") : await e.reply(`${Root.pluginName} 更新失败: ${upd.error?.message ?? String(upd.error)}`);
		} catch (error) {
			await e.reply(`${Root.pluginName} 更新失败: ${error.message}`);
		}
	}
	next();
}, { priority: 100 }), kkkUpdateCommand = karin.command(/^#?kkk更新$/, async (e) => {
	const upd = await checkPkgUpdate(Root.pluginName);
	if ("error" === upd.status) return void await e.reply(`获取远程版本失败：${upd.error?.message ?? String(upd.error)}`);
	if ("no" === upd.status) return void await e.reply(`当前已是最新版本：${upd.local}`, { reply: !0 });
	if ("yes" === upd.status && !isSemverGreater(upd.remote, upd.local)) return void await e.reply(`当前已是最新或预览版本：${upd.local}`, { reply: !0 });
	const ChangeLogImg = await getChangelogImage({
		localVersion: Root.pluginVersion,
		remoteVersion: upd.remote,
		Tip: !1
	});
	ChangeLogImg ? await e.reply([segment.text(`${Root.pluginName} 的更新日志：`), ...ChangeLogImg], { reply: !0 }) : await e.reply("获取更新日志失败，更新进程继续......", { reply: !0 });
	try {
		const result = await updatePkg(Root.pluginName);
		if ("ok" === result.status) {
			if ((await e.reply(`${Root.pluginName} 更新成功！\n${result.local} -> ${result.remote}\n开始执行重启......`)).messageId) try {
...
}, { name: "kkk-更新" }), update = karin.task("kkk-更新检测", "*/10 * * * *", async (e) => {
	...
	if ("yes" === upd.status && !isSemverGreater(upd.remote, upd.local)) return !0;
	if ("yes" !== upd.status) return !0;
	try {
		const lockedVersion = await db.get("kkk:update:lock");
		if ("string" == typeof lockedVersion && lockedVersion.length > 0) if (isSemverGreater(lockedVersion, Root.pluginVersion)) {
			if (!isSemverGreater(upd.remote, lockedVersion)) return !0;
		} else await db.del("kkk:update:lock");
	} catch {}
	try {
		await db.set("kkk:update:lock", upd.remote);
	} catch {}
	const ChangeLogImg = await getChangelogImage({
		localVersion: Root.pluginVersion,
		remoteVersion: upd.remote,
	...
	if (ChangeLogImg) {
		const msgResult = await karin.sendMaster("console" === botList[0].bot.account.name ? botList[1].bot.account.selfId : botList[0].bot.account.selfId, master, [segment.text("karin-plugin-kkk 有新的更新！"), ...ChangeLogImg]);
		try {
			await db.set("kkk:update:msgId", msgResult.messageId);
		} catch {}
	}
	return !0;
}, {
...
export { setdyck as C, Root as D, web_config_default as E, init_root as O, setbilick as S, webConfig as T, testDouyinPush as _, douyinAPP as a, biLogin as b, xiaohongshuAPP as c, changeBotID as d, douyinPush as f, setdyPush as g, setbiliPush as h, bilibiliAPP as i, bilibiliPush as l, forcePush as m, kkkUpdateCommand as n, kuaishouAPP as o, douyinPushList as p, update as r, prefix as s, kkkUpdate as t, bilibiliPushList as u, help as v, task as w, dylogin as x, version as y };
