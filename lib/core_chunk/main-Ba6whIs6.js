...
import { C as Client, b as bilibiliErrorCodeMap, D as DynamicType, g as getBilibiliData, A as AdditionalType, a as bilibiliApiUrls, w as wbi_sign, c as browserExports, M as MajorType, d as getDouyinData, h as heicConvertExports } from "./vendor-gZCn1y0D.js";
import karin, { copyConfigSync, filesByExt, watch, logger, requireFileSync, config, segment, createNotFoundResponse, ffmpeg, ffprobe, render, common, createBadRequestResponse, createServerErrorResponse, createSuccessResponse, getBot, defineConfig, components, karinPathRoot, mkdirSync } from "node-karin";
import axios, { AxiosError } from "node-karin/axios";
import { pipeline } from "stream/promises";
import path, { join } from "node:path";
import { r as renderComponentToHtml } from "./template-CH5U3pZy.js";
import { karinPathBase, karinPathTemp } from "node-karin/root";
import { getDouyinDB, getBilibiliDB, cleanOldDynamicCache, bilibiliDBInstance, douyinDBInstance } from "./db-CBQbHvPr.js";
...
async function Render(path2, params, useReact = true) {
  if (useReact) {
    return await renderWithReact(path2, params);
  }
  return await renderWithTemplate(path2, params);
}
async function renderWithReact(path2, params) {
  try {
    ...
    return await renderWithLocalComponent(templateType, templateName, params);
  } catch (error) {
    console.error("React渲染失败，回退到传统模板渲染:", error);
    return await renderWithTemplate(path2, params);
  }
}
async function renderWithLocalComponent(templateType, templateName, params) {
...
async function renderWithTemplate(path2, params) {
  const basePaths = {
    douyin: "douyin/html",
...
` + dynamicInfo.data.data.item.modules.module_dynamic.major.opus.summary.text;
          ...
          case DynamicType.WORD: {
            const text = replacetext(br$4(dynamicInfo.data.data.item.modules.module_dynamic.major.opus.summary.text), dynamicInfo.data.data.item.modules.module_dynamic.major.opus.summary.rich_text_nodes);
            ...
            this.e.reply(
              await Render("bilibili/dynamic/DYNAMIC_TYPE_WORD", {
                ...
                frame: dynamicInfo.data.data.item.modules.module_author.pendant.image,
                share_url: "https://t.bilibili.com/" + dynamicInfo.data.data.item.id_str,
                username: checkvip$2(dynamicInfo.data.data.card ?? userProfileData.data.data.card),
...
const bilibiliLogin = async (e) => {
  ...
  const qrimg = await browserExports.toDataURL(qrcodeurl.data.data.url);
  ...
  const handleLoginSuccess = async (responseData) => {
    Config.Modify("cookies", "bilibili", Array.isArray(responseData.data.data.headers["set-cookie"]) ? responseData.data.data.headers["set-cookie"] : [responseData.data.data.headers["set-cookie"]]);
...
function processFrontendData(data) {
  ...
  for (const key of configKeys) {
    ...
    for (const prop of nestedProps) {
      ...
      if (propValue !== "" && propValue !== void 0 && propValue !== null) {
...
function cleanFlattenedFields(obj) {
...
function hasNestedStructure(obj, path2) {
  let current = obj;
