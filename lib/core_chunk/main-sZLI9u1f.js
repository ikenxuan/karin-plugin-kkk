...
const wrapWithErrorHandler = (fn, options) => async (...args) => {
	const ctx = logger.runContext(async () => await fn(...args));
	try {
		return await ctx.run();
	} catch (error) {
		await new Promise((resolve$1) => setTimeout(resolve$1, 100));
		await handleBusinessError(error, options, parseLogsToStructured(ctx.logs()), args[0]);
		throw error;
	}
};
