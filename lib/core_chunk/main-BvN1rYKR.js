...
function generateASS(danmakuList, width, height, options = {}) {
	...
	for (const dm of sorted) {
		...
		if (dm.mode === 4) {
			...
			const y = height - Math.round(10 * fontScale) - idx * trackH;
			...
			const y = Math.round(10 * fontScale) + (idx + 1) * trackH;
...
var HEVC_ENCODERS, cachedEncoder, toASSColor, toASSTime, estimateWidth, escapeASS, escapeWinPath, isLandscape, MAX_OUTPUT_WIDTH;
