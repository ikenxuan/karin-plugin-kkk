...
import { At as createBilibiliRoutes, Bt as init_dist, Ct as require_lib$1, Dt as MajorType, Et as DynamicType, Ft as init_default, It as logMiddleware, Lt as logger$1, Mt as createKuaishouRoutes, Nt as getBilibiliData, Ot as amagi, Pt as getDouyinData, Rt as wbi_sign, St as require_dist, Tt as Client, jt as createDouyinRoutes, kt as bilibiliApiUrls, n as require_browser, t as require_heic_convert, wt as require_lib, zt as app$1 } from "./vendor-B09mdTl9.js";
import { n as init_client, r as reactServerRender } from "./template-Dc3RN7ZF.js";
import path, { resolve } from "node:path";
import fs from "node:fs";
import { fileURLToPath } from "node:url";
import os, { platform } from "node:os";
import crypto from "node:crypto";
import karin$1, { app, authMiddleware, checkPkgUpdate, common, components, config, copyConfigSync, createBadRequestResponse, createNotFoundResponse, createServerErrorResponse, createSuccessResponse, db, defineConfig, ffmpeg, ffprobe, filesByExt, getBot, hooks, karin, karinPathHtml, karinPathTemp, logger, mkdirSync, range, render, requireFileSync, restart, segment, updatePkg, watch } from "node-karin";
...
const version = karin$1.command(/^#?kkk(版本|更新日志)$/, async (e) => {
	const img$2 = await getChangelogImage({
		localVersion: Root.pluginVersion,
		remoteVersion: Root.pluginVersion,
		Tip: false
	});
	if (img$2) e.reply(img$2);
	else e.reply("获取更新日志失败");
	return true;
}, { name: "kkk-版本" });
init_default();
...
var handleBusinessError = async (error, options, logs, event) => {
	...
	if (Config.app.errorLogSendTo.some((item) => item === "master")) try {
		...
		if (selectedBotId && master) if (isPushTask) await karin$1.sendMaster(selectedBotId, master, [segment.text(`${options.businessName} 任务执行出错！\n请即时解决以消除警告`), ...img$2]);
		else {
			const Adapter = karin$1.getBot(selectedBotId);
			const groupID = event && "groupId" in event ? event.groupId : "";
			const groupInfo = await Adapter?.getGroupInfo(groupID);
			await karin$1.sendMaster(selectedBotId, master, [segment.text(`群：${groupInfo?.groupName || "未知"}(${groupID})\n${options.businessName} 任务执行出错！\n请即时解决以消除警告`), ...img$2]);
		}
	} catch (masterError) {
...
const kkkUpdateTest = karin$1.command("test", async (e) => {
	await db.del(UPDATE_MSGID_KEY);
	await db.del(UPDATE_LOCK_KEY);
	return Handler(e);
});
const update = karin$1.task("kkk-更新检测", "*/10 * * * *", Handler, {
	name: "kkk-更新检测",
	log: false
});
