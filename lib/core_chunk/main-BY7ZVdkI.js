...
import { A as formatDistanceToNow, Bt as init_esm, Gt as axios_default, Ht as zod_default, Jt as Chalk, Kt as dist_exports, M as differenceInSeconds, O as init_date_fns, Rt as require_express, T as zhCN, Ut as AxiosError$1, Vt as init_zod, Wt as init_axios, Yt as init_source, a as require_lib, c as Window, i as require_lib$1, j as format, k as fromUnixTime, n as require_heic_convert, o as require_qr_code_styling, qt as init_dist, r as require_dist, s as init_lib, t as snapka, w as init_locale, zt as Xhshow } from "./vendor-AuhzC1dF.js";
import { n as init_client, r as reactServerRender } from "./template-PIp9JOsq.js";
...
async function mergeVideoAudio(videoPath, audioPath, resultPath) {
	...
	if (result.status) logger.mark(`视频合成成功！文件地址：${resultPath}`);
...
async function compressVideo(options) {
	...
	} else logger.error(`${inputPath} 压缩失败！`, result);
	return result.status;
}
...
async function mergeLiveImageIndependent(options, bgmPath) {
	...
	if (result.status) logger.mark(`Live 图合成成功！文件地址：${outputPath}`);
...
async function mergeLiveImageContinuous(options, context) {
	...
	if (totalVideoDuration <= remainingBgm) {
		...
		inputArgs = `-y -stream_loop ${streamLoop} -i "${videoPath}" -stream_loop ${Math.ceil(totalVideoDuration / bgmDuration) + 1} -ss ${bgmStartTime} -i "${bgmPath}"`;
		filterComplex = "[0:v]setpts=N/FRAME_RATE/TB[v];[0:a][1:a]amix=inputs=2:duration=first:dropout_transition=3[aout]";
	}
	const result = await ffmpeg(`${inputArgs} -filter_complex "${filterComplex}" -map "[v]" -map "[aout]" -c:v libx264 -c:a aac -b:a 192k -shortest "${outputPath}"`);
	const newUsedDuration = (usedDuration + totalVideoDuration) % bgmDuration;
	if (result.status) {
		const remainingBgmTime = bgmDuration - newUsedDuration;
		logger.mark(`Live 图连续合成成功！文件地址：${outputPath}\n  本次使用 BGM：${totalVideoDuration.toFixed(2)}s（视频时长 ${originalDuration.toFixed(2)}s × ${loopCount}）\n  BGM 进度：${newUsedDuration.toFixed(2)}s / ${bgmDuration.toFixed(2)}s，剩余 ${remainingBgmTime.toFixed(2)}s`);
...
var Render_exports = __export({ Render: () => Render });
var Render;
		...
		switch (this.islogin) {
			case true: {
				...
				if (bmp4.filepath && bmp3.filepath) {
					...
					if (await mergeVideoAudio(bmp4.filepath, bmp3.filepath, resultPath)) {
