...
var init_routes$3 = __esmMin(() => {
	...
	createBilibiliRoutes = (cookie, requestConfig = getBilibiliDefaultConfig(cookie)) => {
		const router = import_express$4.Router();
		for (const [method, path$1] of Object.entries(BilibiliMethodRoutes)) router.get(path$1, createBilibiliValidationMiddleware(method), createBilibiliRouteHandler(getBilibiliData$1, method, cookie, requestConfig));
		return router;
	};
});
var bilibiliUtils;
...
var init_routes$2 = __esmMin(() => {
	...
	createDouyinRoutes = (cookie, requestConfig = getDouyinDefaultConfig(cookie)) => {
		const router = import_express$3.Router();
		for (const [method, path$1] of Object.entries(DouyinMethodRoutes)) router.get(path$1, createDouyinValidationMiddleware(method), createDouyinRouteHandler(getDouyinData$1, method, cookie, requestConfig));
		return router;
	};
});
var douyinUtils;
...
var init_routes$1 = __esmMin(() => {
	...
	createKuaishouRoutes = (cookie, requestConfig = getKuaishouDefaultConfig(cookie)) => {
		const router = import_express$2.Router();
		for (const [method, path$1] of Object.entries(KuaishouMethodRoutes)) router.get(path$1, createKuaishouValidationMiddleware(method), createKuaishouRouteHandler(getKuaishouData$1, method, cookie, requestConfig));
		return router;
	};
});
var kuaishouUtils;
...
var init_routes = __esmMin(() => {
	...
	createXiaohongshuRoutes = (cookie, requestConfig = getXiaohongshuDefaultConfig(cookie)) => {
		const router = import_express$1.Router();
		for (const [method, path$1] of Object.entries(XiaohongshuMethodRoutes)) router.get(path$1, createXiaohongshuValidationMiddleware(method), createXiaohongshuRouteHandler(getXiaohongshuData$1, method, cookie, requestConfig));
		return router;
	};
});
...
async function detectHevcEncoder() {
...
function getEncoderParams(encoder) {
	const threads = Math.max(1, Math.floor(os.cpus().length / 2));
	switch (encoder) {
		case "hevc_nvenc": return "-c:v hevc_nvenc -preset p4 -cq 28 -b:v 0";
		case "hevc_qsv": return "-c:v hevc_qsv -preset medium -global_quality 28";
		case "hevc_amf": return "-c:v hevc_amf -quality balanced -qp_i 28 -qp_p 28";
		default: return `-c:v libx265 -crf 28 -preset medium -threads ${threads}`;
	}
}
...
function generateASS(danmakuList, width, height, options = {}) {
	const { scrollTime = 8, opacity = 180, fontName = "Microsoft YaHei", danmakuArea = .5 } = options;
	...
	const topTracks = Array(trackCount).fill(0);
	const bottomTracks = Array(trackCount).fill(0);
	...
	for (const dm of sorted) {
		...
		if (dm.mode === 4) {
			...
			if (idx === -1) if (danmakuArea >= .75) idx = Math.floor(Math.random() * bottomTracks.length);
			else continue;
			...
			if (idx === -1) if (danmakuArea >= .75) idx = Math.floor(Math.random() * topTracks.length);
			else continue;
...
async function burnDanmaku(videoPath, danmakuList, outputPath, options = {}) {
	...
	if (canvas.isVertical) logger.debug(`竖屏模式: ${resolution.width}x${resolution.height} -> ${canvas.width}x${canvas.height}`);
	...
	const result = await ffmpeg(`-y -i "${videoPath}" -vf "${buildFilter(canvas, assPath)}" -r ${frameRate} ${getEncoderParams(await detectHevcEncoder())} -c:a copy "${outputPath}"`);
	Common.removeFile(assPath);
	if (result.status) {
		logger.mark(`弹幕烧录成功: ${outputPath}`);
		if (removeSource) Common.removeFile(videoPath);
	} else logger.error("弹幕烧录失败", result);
	return result.status;
}
async function mergeAndBurn(videoPath, audioPath, danmakuList, outputPath, options = {}) {
	const { removeSource = false, verticalMode = "off" } = options;
	...
	const result = await ffmpeg(`-y -i "${videoPath}" -i "${audioPath}" -f mp4 -vf "${buildFilter(canvas, assPath)}" -r ${frameRate} ${getEncoderParams(await detectHevcEncoder())} -c:a aac -b:a 192k "${outputPath}"`);
	Common.removeFile(assPath);
	if (result.status) {
	...
	} else logger.error("视频合成+弹幕烧录失败", result);
	return result.status;
}
var HEVC_ENCODERS, cachedEncoder, toASSColor, toASSTime, estimateWidth, escapeASS, escapeWinPath, isLandscape;
...
var getGroupsRouter, getAuthorsRouter, getDouyinContentRouter, getBilibiliContentRouter, addDouyinContentRouter, addBilibiliContentRouter, deleteContentRouter;
var init_content_router = __esmMin(async () => {
	await init_db();
	await init_amagiClient();
	getGroupsRouter = async (req, res) => {
		try {
	...
	getAuthorsRouter = async (req, res) => {
		try {
	...
	getDouyinContentRouter = async (req, res) => {
		try {
			const { groupId } = req.query;
			if (!groupId) return createBadRequestResponse(res, "请提供群组ID");
	...
	getBilibiliContentRouter = async (req, res) => {
		try {
			const { groupId } = req.query;
			if (!groupId) return createBadRequestResponse(res, "请提供群组ID");
	...
	addDouyinContentRouter = async (req, res) => {
		try {
	...
	addBilibiliContentRouter = async (req, res) => {
		try {
			const { contentId, groupId, authorId } = req.body;
			if (!contentId || !groupId || !authorId) return createBadRequestResponse(res, "请提供内容ID、群组ID和作者ID");
			const bilibiliDB$1 = await getBilibiliDB();
	...
	deleteContentRouter = async (req, res) => {
		try {
...
var videoStreamRouter, getVideoRouter, getLongLinkRouter, getDouyinDataRouter, getBilibiliDataRouter, getKuaishouDataRouter;
var init_router = __esmMin(async () => {
	...
	getLongLinkRouter = async (req, res) => {
		try {
			const { link } = req.body;
	...
	getBilibiliDataRouter = async (req, res) => {
		try {
			const { dataType, params } = req.body;
			if (!dataType || !params) return createBadRequestResponse(res, "请提供数据类型和参数");
			createSuccessResponse(res, {
				data: (await getBilibiliData(dataType, {
					...params,
					typeMode: "strict"
				})).data,
				platform: "bilibili",
	...
	getKuaishouDataRouter = async (req, res) => {
		try {
			const { dataType, params } = req.body;
			if (!dataType || !params) return createBadRequestResponse(res, "请提供数据类型和参数");
			createSuccessResponse(res, {
				data: (await getKuaishouData(dataType, {
					...params,
					typeMode: "strict"
				})).data,
				platform: "kuaishou",
...
var import_lib, import_lib$1, import_dist, server, proxyOptions, app$1, middleware, staticRouter;
var init_Register = __esmMin(async () => {
	...
	middleware = Config.app.webAuth ? [authMiddleware, signatureVerificationMiddleware] : [];
			...
			case "one_video": {
				...
				if (Config.bilibili.sendContent.some((content) => content === "video")) if (Config.upload.usefilelimit && Number(videoSize) > Number(Config.upload.filelimit) && !Config.upload.compress) this.e.reply(`设定的最大上传大小为 ${Config.upload.filelimit}MB\n当前解析到的视频大小为 ${Number(videoSize)}MB\n视频太大了，还是去B站看吧~`, { reply: true });
					...
					if (Config.bilibili.burnDanmaku) try {
						...
						danmakuList = (await this.amagi.getBilibiliData("实时弹幕", {
							cid,
							typeMode: "strict"
						})).data?.data?.elems || [];
			...
			case false:
				...
				if (Config.bilibili.burnDanmaku && danmakuList.length > 0) {
					...
					if (videoFile.filepath) {
						...
						if (await burnDanmaku(videoFile.filepath, danmakuList, resultPath, {
							danmakuArea: Config.bilibili.danmakuArea,
							verticalMode: Config.bilibili.verticalMode
						})) {
							const filePath = Common.tempDri.video + `${Config.app.removeCache ? "tmp_" + Date.now() : this.downloadfilename}.mp4`;
...
var DouYinpush = class extends Base {
	...
	async action() {
		...
		const data$1 = await this.getDynamicList(Config.pushlist.douyin);
