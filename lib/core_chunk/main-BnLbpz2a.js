	...
	Networks = class {
		...
		constructor(data) {
			...
			this.axiosInstance = axios.create({
				...
				maxRedirects: 5,
				validateStatus: (status) => status >= 200 && status < 300 || status === 406 || status >= 500
			});
		}
		get config() {
			const config$1 = {
