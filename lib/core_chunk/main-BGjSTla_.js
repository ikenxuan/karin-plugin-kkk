...
var init_parse$1 = __esmMin(() => {
	...
	parseDynamicRaw = async (req, res) => {
		try {
			...
			const [dynamicCardResponse, userProfileResponse] = await Promise.all([getBilibiliData("动态卡片数据", {
...
var init_video = __esmMin(() => {
	...
	getVideoPlayUrl = async (req, res) => {
		try {
			...
			const videoInfo = (await getBilibiliData("单个视频作品数据", {
				bvid,
				typeMode: "strict"
			})).data.data;
			const cid = pageNum > 1 && videoInfo.pages[pageNum - 1] ? videoInfo.pages[pageNum - 1].cid : videoInfo.cid;
			const playUrlData = (await getBilibiliData("单个视频下载信息数据", {
				avid: videoInfo.aid,
				cid,
				typeMode: "strict"
			})).data.data;
