...
var init_Config = __esmMin((() => {
	...
	Cfg = class {
		...
		Modify(name, key, value, type = "config") {
			const path$2 = type === "config" ? `${this.dirCfgPath}/${name}.yaml` : `${this.defCfgPath}/${name}.yaml`;
			const yamlData = YAML.parseDocument(fs.readFileSync(path$2, "utf8"));
			const keys = key.split(".");
			this.setNestedValue(yamlData.contents, keys, value);
			fs.writeFileSync(path$2, yamlData.toString({ lineWidth: -1 }), "utf8");
		}
...
var init_Common = __esmMin((() => {
	...
	Tools = class {
		...
		async removeFile(path$2, force = false) {
			path$2 = path$2.replace(/\\/g, "/");
			if (Config.app.removeCache) try {
				await fs.promises.unlink(path$2);
				logger.mark("缓存文件: ", path$2 + " 删除成功！");
				return true;
			} catch (err) {
				logger.error("缓存文件: ", path$2 + " 删除失败！", err);
				return false;
			}
			else if (force) try {
				await fs.promises.unlink(path$2);
	...
	Networks = class {
		...
		async downloadStream(progressCallback, retryCount = 0) {
			...
			try {
				...
				if (!(response.status >= 200 && response.status < 300)) throw new Error(`无法获取 ${this.url}。状态: ${response.status} ${response.statusText}`);
				...
				clearTimeout(timeoutId);
				if (error instanceof AxiosError) logger.error(`请求在 ${this.timeout / 1e3} 秒后超时`);
				...
				} else throw new Error(`在 ${this.maxRetries} 次尝试后下载失败: ${error}`);
			}
		}
		async getfetch() {
			try {
...
var init_Render = __esmMin((async () => {
	...
	Render = async (path$2, data) => {
		...
		} else throw new Error(`不支持的路径格式: ${path$2}`);
		...
		const result = await client_default({
			...
			if (!res.success || !res.htmlPath) throw new Error(res.error || "SSR渲染失败");
			return res;
		}).catch((err) => {
...
var init_bilibili$2 = __esmMin((async () => {
	...
	generateDecorationCard = (decorate) => {
...
staticRouter.use(express.static(path.join(Root.pluginPath, "lib", "web_chunk"), {
	redirect: false,
	setHeaders: (res, path$2) => {
		if (path$2.endsWith(".html")) res.setHeader("Cache-Control", "no-cache");
...
var hasNestedStructure = (obj, path$2) => {
	let current = obj;
	for (let i = 0; i < path$2.length - 1; i++) {
		const key = path$2[i];
		if (!current[key] || typeof current[key] !== "object") return false;
		current = current[key];
	}
	return path$2[path$2.length - 1] in current;
};
await init_esm();
var import_heic_convert = /* @__PURE__ */ __toESM(require_heic_convert());
await init_utils();
await init_Config();
			...
			case "one_work": {
					...
					case this.is_slides === false && VideoData.data.aweme_detail.images !== null: {
						...
						for (const [index, imageItem] of images.entries()) {
							...
							if (Config.app.removeCache === false) {
								mkdirSync$1(`${Common.tempDri.images}${g_title}`);
								const path$2 = `${Common.tempDri.images}${g_title}/${index + 1}.png`;
								await new Networks({
									url: image_url,
									type: "arraybuffer"
								}).getData().then((data$1) => fs.promises.writeFile(path$2, Buffer.from(data$1)));
							}
						}
				...
				if (VideoData.data.aweme_detail.music) {
					...
					if (this.is_mp4 === false && Config.app.removeCache === false && music_url !== void 0) try {
						const path$2 = Common.tempDri.images + `${g_title}/BGM.mp3`;
						await new Networks({
							url: music_url,
							type: "arraybuffer"
						}).getData().then((data$1) => fs.promises.writeFile(path$2, Buffer.from(data$1)));
...
var Kuaishou = class extends Base {
	...
	async RESOURCES(data) {
		...
		const transformedData = Object.entries(data.EmojiData.data.data.visionBaseEmoticons.iconUrls).map(([name, path$2]) => {
			return {
				name,
				url: `https:${path$2}`
			};
...
const setbiliPush = karin.command(/^#设置[bB]站推送(?:[Uu][Ii][Dd]:)?(\d+)$/, handleSetBilibiliPush, {
	name: "kkk-推送功能-设置",
	event: "message.group",
