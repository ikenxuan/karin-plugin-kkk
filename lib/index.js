...
import { C as Config, l as logMiddleware, r as registerBilibiliRoutes, a as registerDouyinRoutes, b as registerKuaishouRoutes, c as logger$1, i as index_default, d as Common } from "./chunk/douyin-sd-LdfZC.js";
...
import { app, logger as logger$2, createNotFoundResponse, mkdirSync } from "node-karin";
...
app.get("/api/kkk/videodata/:filename", (req, res) => {
  ...
  try {
    ...
    if (range) {
      ...
      file.pipe(res);
      file.on("error", (err) => {
        logger$2.error(`读取视频文件流时出错 (Range: ${start}-${end}): ${err.message}`);
        if (!res.writableEnded) {
          res.end();
        }
      });
      ...
      file.pipe(res);
      file.on("error", (err) => {
        logger$2.error(`读取视频文件流时出错 (Full): ${err.message}`);
        if (!res.headersSent) {
          try {
            createNotFoundResponse(res, "读取视频文件时出错");
          } catch (e) {
            logger$2.error("发送读取错误响应失败:", e);
            if (!res.writableEnded) {
              res.end();
            }
          }
    ...
    if (error.code === "ENOENT") {
      ...
      logger$2.error(`处理视频数据请求时发生错误: ${error.message}`);
      if (!res.headersSent) {
