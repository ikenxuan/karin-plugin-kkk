...
import { bilibiliComments, DynamicType, genParams } from '../../platform/bilibili/index.js';
let img;
        ...
        switch (this.TYPE) {
            case 'one_video': {
                ...
                if ((Config.upload.usefilelimit && Number(videoSize) > Number(Config.upload.filelimit)) && !Config.upload.compress) {
                    ...
                    await this.getvideo(Config.bilibili.videopriority === true ? { DATA: nocdData } : OBJECT);
                break;
            }
            case 'bangumi_video_info': {
                ...
                if (Config.bilibili.autoResolution) {
                    /** 提取出视频流信息对象，并排除清晰度重复的视频流 */
                    const simplify = DATA.result.dash.video.filter((item, index, self) => {
                        return self.findIndex((t) => {
                            return t.id === item.id;
                        }) === index;
                    });
