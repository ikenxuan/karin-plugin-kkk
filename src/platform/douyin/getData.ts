...
export async function fetchDouyinData<T extends keyof DouyinDataTypes> (
  ...
  switch (type) {
    case 'one_work': {
      const VideoData = await getDouyinData('聚合解析', dyck, {
        aweme_id: (opt as DouyinDataOptionsMap['聚合解析']['opt']).aweme_id
      })
      const CommentsData = await getDouyinData('评论数据', dyck, {
        aweme_id: (opt as DouyinDataOptionsMap['评论数据']['opt']).aweme_id,
    ...
    case 'user_mix_videos': {
      ...
      const LiveImageData = await getDouyinData('合辑作品数据', dyck, {
        aweme_id: (opt as DouyinDataOptionsMap['合辑作品数据']['opt']).aweme_id
      })
    ...
    case 'user_profile': {
      const UserInfoData = await getDouyinData('用户主页数据', dyck, {
        sec_uid: (opt as unknown as DouyinDataOptionsMap['用户主页数据']['opt']).sec_uid
      })
      return UserInfoData
    }
    case 'emoji_list': {
    ...
    case 'user_dynamic': {
      const UserVideoListData = await getDouyinData('用户主页视频列表数据', dyck, {
        sec_uid: (opt as unknown as DouyinDataOptionsMap['用户主页视频列表数据']['opt']).sec_uid
      })
      return UserVideoListData
    }
    case 'suggest_words': {
      const SuggestWordsData = await getDouyinData('热点词数据', dyck, {
        query: (opt as unknown as DouyinDataOptionsMap['热点词数据']['opt']).query
      })
    ...
    case 'music_work': {
      const MusicData = await getDouyinData('音乐数据', dyck, {
        music_id: (opt as unknown as DouyinDataOptionsMap['音乐数据']['opt']).music_id
      })
      return MusicData
    }
    default:
