...
import { DouyinLoginSession } from './session'
...
const handleSecondVerify = async (
  ...
  const sent = await session.sendSmsCode(verify)
  if (!sent.ok) {
    await tracker.send(`短信验证码发送失败：${sent.message}`)
    return false
  }

  const target = sent.mobile || smsWay?.mobile || '扫码设备绑定的手机号'
  ...
  for (let attempt = 1; attempt <= CODE_MAX_ATTEMPTS; attempt++) {
    ...
    const result = await session.submitSmsCode(verify, code)
    if (result.ok) {
    ...
    if (!result.wrongCode || attempt === CODE_MAX_ATTEMPTS) {
      await tracker.send(`验证失败：${result.message}`)
      return false
    }
 ...
 * @param session 登录会话
 */
const persistCookie = async (session: DouyinLoginSession): Promise<void> => {
  const summary = session.credentialSummary
  ...
  await Config.Modify('amagi', 'cookies.douyin', session.cookie)
 ...
 * 抖音扫码登录
 *
 * 纯 HTTP 实现：走 passport 接口取码、轮询、二次验证，全程不启动浏览器。
 * @param e 消息事件
 */
export const douyinLogin = async (e: Message) => {
  const session = new DouyinLoginSession()
  const tracker = createMessageTracker(e)

  try {
    const qrcode = await session.requestQrcode()
    logger.mark(`[抖音登录] 二维码已获取，有效期 ${Math.round(qrcode.expireTime / 1000)} 秒`)

    const rendered = await Render(e, 'douyin/qrcodeImg', { share_url: qrcode.content })
