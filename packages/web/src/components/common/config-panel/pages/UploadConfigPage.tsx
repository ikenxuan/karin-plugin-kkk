      ...
      {renderSubSection(
        ...
        <>
          ...
          {renderSwitch(
            ...
            uploadVideoSendMode === 'base64'
          )}
          {renderTextField(
            ['upload', 'groupfilevalue'],
            '群文件上传阈值',
            '当文件大小超过该值时将使用群文件上传，单位：MB，「使用群文件上传」开启后才会生效。',
            { type: 'number', fallback: 100, min: 1, disabled: !uploadUseGroupFile || uploadVideoSendMode === 'base64' }
          )}
      ...
      {renderSubSection(
        ...
        <>
          ...
          {renderTextField(
            ...
            { type: 'number', fallback: 20, min: 1, disabled: !uploadUseFileLimit }
          )}
        </>
      )}
      {renderSubSection(
        ...
        <>
          ...
          {renderTextField(
            ['upload', 'compresstrigger'],
            '压缩触发阈值',
            '触发视频压缩的阈值，单位：MB。当文件大小超过该值时，才会压缩视频，「压缩视频」开启后才会生效。',
            { type: 'number', fallback: 80, min: 1, disabled: !uploadCompress }
          )}
          {renderTextField(
            ['upload', 'compressvalue'],
            '压缩后的值',
            '单位：MB，若视频文件大小大于「压缩触发阈值」的值，则会进行压缩至该值（±5%），「压缩视频」开启后才会生效。',
            { type: 'number', fallback: 30, min: 1, disabled: !uploadCompress }
          )}
        </>
      )}
      {renderSubSection(
        ...
        <>
          {renderSwitch(
            ['upload', 'downloadThrottle'],
            '下载限速',
            '开启后会限制下载速度，避免触发服务器风控导致连接被重置（ECONNRESET）。如果下载时经常报错"连接被重置"，建议开启。'
          )}
          {renderTextField(
            ['upload', 'downloadMaxSpeed'],
            '最大下载速度',
            '单位：MB/s，建议设置为 5-20 之间。设置过高可能触发风控，设置过低会影响下载体验。',
            { type: 'number', fallback: 10, min: 1, max: 1000, disabled: !uploadDownloadThrottle }
          )}
          {renderSwitch(
            ['upload', 'downloadAutoReduce'],
            '断流自动降速',
            '当检测到连接被重置时自动降低下载速度，每次断流后速度会降低到当前的 60%。',
            !uploadDownloadThrottle
          )}
          {renderTextField(['upload', 'downloadMinSpeed'], '最低下载速度', '单位：MB/s，自动降速时不会低于此值。', {
            type: 'number',
            fallback: 1,
            min: 0.1,
            max: 100,
