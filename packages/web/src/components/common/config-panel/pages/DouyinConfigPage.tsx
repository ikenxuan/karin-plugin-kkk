  ...
  return (
    <>
      ...
      {renderSubSection('评论详情设置', (
        <>
          {renderTextField(['douyin', 'numcomment'], '评论解析数量', '主评论解析数量。', { type: 'number', fallback: 5, min: 1, disabled: commentDisabled })}
          {renderTextField(['douyin', 'subCommentLimit'], '次级评论解析数量', '次级评论解析数量，当前逻辑不仅无法判断请求的来的评论的嵌套深度，而且「次级评论解析深度」会限制嵌套深度，超过深度的评论会被截断。', { type: 'number', fallback: 5, min: 1, max: 20, disabled: commentDisabled })}
          {renderSwitch(['douyin', 'commentImageCollection'], '是否收集评论区的图片', '开启后将收集评论区的图片，以合并转发的形式返回。', commentDisabled)}
        </>
      ))}
      ...
      {renderSubSection('弹幕烧录相关', (
        <>
          ...
          {renderSelectField(['douyin', 'danmakuArea'], '弹幕显示区域', '限制弹幕范围，避免遮挡视频主体。', danmakuAreaOptions, Number, danmakuDisabled)}
          {renderSelectField(['douyin', 'danmakuFontSize'], '弹幕字号', '弹幕文字大小。', fontSizeOptions, (value) => value, danmakuDisabled)}
          {renderTextField(['douyin', 'danmakuOpacity'], '弹幕透明度', '0为完全透明，100为完全不透明，推荐70。', { type: 'number', fallback: 70, min: 0, max: 100, disabled: danmakuDisabled })}
          {renderSelectField(['douyin', 'verticalMode'], '竖屏适配', '针对横屏视频，模拟手机端竖屏观看体验，视频居中显示，上下黑边区域用于展示弹幕。', verticalModeOptions, (value) => value, danmakuDisabled)}
          {renderSelectField(['douyin', 'videoCodec'], '视频编码格式', '烧录弹幕后的视频编码格式。', videoCodecOptions, (value) => value, danmakuDisabled)}
        </>
      ))}
