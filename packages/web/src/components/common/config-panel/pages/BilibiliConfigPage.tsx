  ...
  return (
    <>
      ...
      {renderSubSection('评论详情设置', (
        <>
          {renderTextField(['bilibili', 'numcomment'], '评论解析数量', '主评论解析数量。', { type: 'number', fallback: 5, min: 1, disabled: commentDisabled })}
          {renderSwitch(['bilibili', 'realCommentCount'], '显示真实评论数量', '关闭则显示解析到的评论数量。', commentDisabled)}
          {renderSwitch(['bilibili', 'commentImageCollection'], '是否收集评论区的图片', '开启后将收集评论区的图片，以合并转发的形式返回。', commentDisabled)}
        </>
      ))}
      ...
      {renderSubSection('弹幕烧录相关', (
        <>
          ...
          {renderSelectField(['bilibili', 'danmakuArea'], '弹幕区域', '限制弹幕的显示范围，避免遮挡视频主体内容。', danmakuAreaOptions, Number, danmakuDisabled)}
          {renderSelectField(['bilibili', 'danmakuFontSize'], '弹幕字号', '弹幕文字大小。', fontSizeOptions, (value) => value, danmakuDisabled)}
          {renderTextField(['bilibili', 'danmakuOpacity'], '弹幕透明度', '0为完全透明，100为完全不透明，推荐70。', { type: 'number', fallback: 70, min: 0, max: 100, disabled: danmakuDisabled })}
          {renderSelectField(['bilibili', 'verticalMode'], '竖屏适配', '模拟手机端竖屏观看体验，视频居中显示，上下黑边区域用于展示弹幕。', verticalModeOptions, (value) => value, danmakuDisabled)}
          {renderSelectField(['bilibili', 'videoCodec'], '视频编码格式', '弹幕烧录时使用的视频编码格式，会自动检测硬件加速。', videoCodecOptions, (value) => value, danmakuDisabled)}
        </>
      ))}
