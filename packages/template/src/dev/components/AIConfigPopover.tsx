import { Button, Input, Label, ListBox, Popover, Select, TextField, toast } from '@heroui/react'
...
import { addProvider, getAIConfig, removeProvider, setActiveProvider, setDefaultPrompt, updateProvider } from '../utils/aiConfig'
 ...
 * AI 配置 Popover（非模态窗）
 */
export const AIConfigPopover: React.FC<AIConfigPopoverProps> = ({ panelTheme = 'light', panelThemeStyle }) => {
    ...
    <Popover.Root onOpenChange={handleOpenChange}>
      <Popover.Trigger>
        <Button size="sm" variant="secondary">
          ...
          AI 配置
          {activeProvider && (
            <span className="ml-1 rounded-full bg-foreground px-1.5 py-0.5 text-[10px] text-background">{activeProvider.apiFormat}</span>
          )}
        </Button>
      </Popover.Trigger>
          ...
          {isEditing ? (
            <div className="flex flex-col gap-3">
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-semibold tracking-[0.18em] text-muted uppercase">API 格式</Label>
                <Select
                      ...
                      <ListBox className="p-1">
                        {API_FORMAT_TEMPLATES.map((t) => (
                          <ListBox.Item key={t.key} id={t.key} textValue={t.name}>
                            {(() => {
                              const TIcon = t.icon
                              return <TIcon size={16} className="text-muted" />
                            })()}
                            {t.name}
              ...
              {/* Base URL */}
              <TextField value={draft.baseUrl} onChange={(value: string) => setDraft((prev) => ({ ...prev, baseUrl: value }))}>
                <Label className="text-[10px] font-semibold tracking-[0.18em] text-muted uppercase">Base URL</Label>
                <Input placeholder="https://api.openai.com" />
              </TextField>

              {/* API Key */}
              <TextField value={draft.apiKey} onChange={(value: string) => setDraft((prev) => ({ ...prev, apiKey: value }))}>
                <Label className="text-[10px] font-semibold tracking-[0.18em] text-muted uppercase">API Key</Label>
                <div className="flex items-center gap-2">
                  <Input className="flex-1" placeholder="sk-xxx 或 sk-ant-xxx" type={showApiKey ? 'text' : 'password'} autoComplete="off" />
                  <Button isIconOnly onPress={() => setShowApiKey((v) => !v)} size="sm" variant="ghost">
              ...
              {/* Model ID */}
              <TextField value={draft.model} onChange={(value: string) => setDraft((prev) => ({ ...prev, model: value }))}>
              ...
              <div className="flex gap-2">
                <Button className="flex-1" onPress={() => setIsEditing(false)} size="sm" variant="tertiary">
                  取消
                </Button>
                <Button className="flex-1" isDisabled={isTesting} isPending={isTesting} onPress={handleTest} size="sm" variant="secondary">
                  {({ isPending }) => (isPending ? '测试中...' : '测试连接')}
                </Button>
                <Button className="flex-1" onPress={handleSave} size="sm" variant="primary">
                  保存
                </Button>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {providers.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border bg-surface-secondary p-4 text-center">
                  ...
                  <Button className="mt-2 w-full" onPress={handleStartAdd} size="sm" variant="primary">
                    <Plus size={16} />
                    添加供应商
                  </Button>
                </div>
                ...
                <div className="flex flex-col gap-2">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-1.5">
                              {(() => {
                                const TmplIcon = tmpl?.icon ?? Bot
                                return <TmplIcon size={14} className="text-muted" />
                              })()}
                              <span className="text-xs font-medium">{tmpl?.name ?? p.apiFormat}</span>
                              {isActive && <span className="rounded bg-foreground px-1 py-0.5 text-[9px] text-background">默认</span>}
                            </div>
                            <p className="mt-0.5 truncate text-[11px] text-muted">{p.model}</p>
                          </div>
                          <div className="flex items-center gap-1">
                            {!isActive && (
                              <Button isIconOnly onPress={() => handleSetActive(p.id)} size="sm" variant="ghost">
                                <CheckCircle2 size={14} />
                              </Button>
                            )}
                            <Button isIconOnly onPress={() => handleStartEdit(p)} size="sm" variant="ghost">
                              <Pencil size={14} />
                            </Button>
                            <Button isIconOnly onPress={() => handleDelete(p.id)} size="sm" variant="ghost">
                              <Trash2 size={14} />
                            </Button>
                          </div>
                        </div>
                  ...
                  <Button className="w-full" onPress={handleStartAdd} size="sm" variant="secondary">
                    <Plus size={16} />
                    添加供应商
                  </Button>
                </div>
              ...
              <div className="space-y-1.5 rounded-xl border border-border bg-surface-secondary p-3">
                <Label className="text-[10px] font-semibold tracking-[0.18em] text-muted uppercase">默认生成要求（可选）</Label>
                <textarea
                  value={defaultPrompt}
