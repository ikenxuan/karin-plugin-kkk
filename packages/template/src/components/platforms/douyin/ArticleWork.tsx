...
const TitleSection: React.FC<{ title: string; createTime: string; readTime: number }> = ({ title, createTime, readTime }) => {
  return (
    <div className="flex flex-col px-16 py-5">
      <h1 className="text-8xl font-bold leading-relaxed mb-6 text-foreground select-text tracking-wide wrap-break-word">{title}</h1>
...
const ContentSection: React.FC<{ markdown: string; images: DouyinArticleWorkProps['data']['images'] }> = ({ markdown, images }) => {
  ...
  const preprocessedMarkdown = markdown.replace(/!\[([^\]]*)\]\(([^\s)]+)(?:\s+width=\d+)?(?:\s+height=\d+)?\)/g, '![$1]($2)')
  ...
  return (
    <div className="flex flex-col px-16 py-5">
      <div className="prose prose-lg max-w-none text-foreground select-text">
        <ReactMarkdown
          components={{
            h1: ({ children }) => <h1 className="text-8xl font-bold mb-8 mt-12 text-foreground">{children}</h1>,
            h2: ({ children }) => <h2 className="text-7xl font-bold mb-6 mt-10 text-foreground">{children}</h2>,
            h3: ({ children }) => <h3 className="text-6xl font-bold mb-5 mt-8 text-foreground">{children}</h3>,
            h4: ({ children }) => <h4 className="text-5xl font-bold mb-4 mt-6 text-foreground">{children}</h4>,
            p: ({ children }) => <p className="text-6xl leading-relaxed mb-6 text-foreground/90 tracking-wide">{children}</p>,
            ul: ({ children }) => <ul className="list-disc list-inside mb-6 text-6xl text-foreground/90">{children}</ul>,
            ol: ({ children }) => <ol className="list-decimal list-inside mb-6 text-6xl text-foreground/90">{children}</ol>,
            li: ({ children }) => <li className="mb-3 leading-relaxed">{children}</li>,
            img: ({ src, alt }) => {
              const highQualityUrl = src ? imageMap.get(src) || src : ''
              return (
                <div className="flex flex-col items-center my-8 w-full">
                  <img src={highQualityUrl} alt={alt || '文章图片'} className="rounded-2xl shadow-large w-full h-auto" />
                  {alt && alt !== '图片描述' && <span className="text-5xl text-muted mt-4">{alt}</span>}
                </div>
              )
            },
            ...
            blockquote: ({ children }) => (
              <blockquote className="border-l-4 border-accent pl-6 my-6 text-foreground/80 italic">{children}</blockquote>
            ),
            code: ({ children }) => <code className="bg-surface px-3 py-1 rounded text-5xl text-accent font-mono">{children}</code>
          }}
        >
