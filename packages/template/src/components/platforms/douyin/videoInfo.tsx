...
export const DouyinVideoInfo: React.FC<DouyinVideoInfoProps> = React.memo((props) => {
  ...
  return (
    <DefaultLayout {...props} className="relative">
      ...
      <div className="absolute inset-0 overflow-hidden -z-10">
        <img src={props.data.image_url} alt="" className="w-full h-full object-cover scale-150 blur-[120px] saturate-[1.8] opacity-50" />
      ...
      <div className="relative w-full overflow-hidden text-foreground">
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between px-16 pt-20 pb-16 gap-12">
            ...
            <div className="flex items-center gap-10 min-w-0 shrink">
              ...
              <div className="flex flex-col gap-4 min-w-0">
                <span className="text-6xl text-foreground font-bold leading-tight line-clamp-2">{props.data.author.name}</span>
          ...
          <div className="relative -mt-16">
            <img src={props.data.image_url} alt="Cover" className="w-full h-auto" style={coverMaskStyle} />
          </div>
              ...
              {props.data.music && (
                <div className="flex items-center gap-6 p-4 rounded-3xl bg-surface-secondary/20 backdrop-blur-2xl border border-border shadow-lg overflow-hidden">
                  <div className="relative w-24 h-24">
                    ...
                    />
                    {/* 上层 */}
                    <img src={props.data.music.cover} alt="" className="relative w-full h-full rounded-2xl object-cover z-10" />
                  </div>
                  <div className="flex flex-col gap-3 pr-4">
                    <span className="text-4xl font-semibold text-foreground max-w-150 truncate">{props.data.music.title}</span>
                    <span className="text-3xl text-foreground/90">{props.data.music.author}</span>
                  </div>
                </div>
              )}
            </div>
...
const StatItem: React.FC<{ icon: React.ReactNode; value: number }> = ({ icon, value }) => (
  <div className="flex flex-col items-center gap-3">
    ...
    <span className="text-4xl text-foreground tabular-nums whitespace-nowrap">{formatNumber(value)}</span>
  </div>
)
