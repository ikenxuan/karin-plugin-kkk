import {
  SiApple, SiHonor, SiHuawei, SiOneplus, SiOppo, SiSamsung, SiVivo, SiXiaomi
...
const GooglePhotosIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg className={className} viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path d="M4,16a6,6,0,0,1,12,0Z" fill="#ffba00" /><path d="M22,10a6,6,0,0,1-6,6V4a6,6,0,0,1,6,6" fill="#ea4435" /><path d="M28,16a6,6,0,0,1-12,0Z" fill="#0066da" /><path d="M10,22a6,6,0,0,1,6-6V28a6,6,0,0,1-6-6" fill="#00ac47" /></svg>
)
...
export const LivePhotoTip: React.FC<Omit<LivePhotoTipProps, 'templateType' | 'templateName'>> = React.memo((props) => {
  ...
  const unsupportedBrands: BrandItem[] = [
    { icon: <SiApple className='w-20 h-auto' style={{ color: appleColor }} />, color: appleColor, status: 'unsupported' },
  ...
  return (
    ...
    >
      ...
      <div className='relative z-10 flex flex-col min-h-360 px-20 py-16'>
        ...
        {/* 兼容说明 */}
        <div className='flex items-start gap-4 mt-12 px-8 py-5 rounded-2xl' style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)' }}>
