...
<Callout type="info">动态照片会以独立文件的形式出现在合并转发消息中，保存到相册后即可在支持的设备上播放。</Callout>

<Callout type="warn">**重要提示**：必须保存原图才能被设备正确识别为动态照片。如果保存压缩图，会丢失视频数据，只能看到静态图片。</Callout>
...
<Callout type="info">如果你不确定选哪个系统，先用默认的 `oppo`；如果你更在意“尽量标准”，可以改回 `google`。</Callout>
...
<Accordion title="OPPO ColorOS 模式（当前默认推荐）">
  ...
  <Accordion title="华为/荣耀模式">
    使用不同的实现方式，在文件末尾添加特殊标记而非 XMP 元数据： ``` v2_f35 409:1000 LIVE_{timestamp}
    ``` **适用设备**： - 华为 / 荣耀系列（HarmonyOS / MagicOS）
    <Callout type="warn">此模式未经实测验证，可能存在兼容性问题。</Callout>
  </Accordion>
</Accordions>
