...
<Accordions>
  <Accordion title="喜欢列表推送效果">
    当监控的博主点赞了某个视频时触发，是否能抓到数据取决于对方是否公开了喜欢列表。 ![喜欢列表推送效果展示](/favorite-demo-placeholder.png)
  </Accordion>
  <Accordion title="推荐列表推送效果">
    当博主将某个视频标记为“推荐”时触发，同样依赖对方的公开设置。 ![推荐列表推送效果展示](/recommend-demo-placeholder.png)
  </Accordion>
</Accordions>
...
<Callout type="info">推送是否“能配置成功”，除了平台配置外，还取决于 `pushlist` 里是否存在正确的 `群号:机器人账号` 绑定。</Callout>
