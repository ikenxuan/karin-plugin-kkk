...
export function UIMockupsSection() {
  ...
  const renderMobileCarouselCard = (mockup: MockupItem) => {
    ...
    return (
      <div key={mockup.id} className="shrink-0 px-4">
  ...
  return (
    <section ref={sectionRef} className="relative py-20 w-full overflow-hidden">
      ...
      >
        <h2 className="mb-4 text-4xl font-bold md:text-5xl text-fd-foreground">精雕细琢</h2>
        ...
        <p className="mt-4 text-sm md:text-base text-fd-muted-foreground/80">以下是部分 UI 样式展示，更多样式请在插件内慢慢探索</p>
      </motion.div>
      ...
      {isMobile ? (
        <div className="relative w-full overflow-hidden">
          {/* 移动端横向滚动画册 */}
          <div className="flex overflow-x-auto overflow-y-hidden snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <div className="flex gap-0 pl-[calc(50vw-140px)]">
              {mockups.map((mockup) => (
                <div key={mockup.id} className="snap-center">
          ...
          {/* 左右边缘渐隐遮罩 */}
          <div className="absolute inset-y-0 left-0 w-20 pointer-events-none bg-linear-to-r from-fd-background to-transparent" />
          <div className="absolute inset-y-0 right-0 w-20 pointer-events-none bg-linear-to-l from-fd-background to-transparent" />

          {/* 移动端提示文字 */}
          <div className="mt-8 text-center">
            <p className="text-xs text-fd-muted-foreground">← 左右滑动查看更多 →</p>
          </div>
        </div>
      ) : (
          ...
          >
            {columns.map((column, columnIndex) => (
              <div key={`column-${columnIndex}`} className="flex flex-col">
                {column.map((mockup, itemIndex) => renderMockupCard(mockup, columnIndex, itemIndex))}
              </div>
            ))}
          </div>
      ...
      >
        <p className="text-sm text-fd-muted-foreground">支持多平台内容解析 · 实时动态推送 · 30+ 精美卡片组件</p>
      </motion.div>
    </section>
  )
}
