'use client'
import { Accordion, Button, Input, Label, ListBox, Select, Separator, Spinner, TextField, toast } from '@heroui/react'
...
export function Geetest3Panel({ initialGt, initialChallenge }: Geetest3PanelProps) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <TextField>
        <Label>GT</Label>
        <Input placeholder="请输入 gt 值（32位）" value={gt} onChange={(event) => setGt(event.target.value)} />
      </TextField>

      <TextField>
        <Label>CHALLENGE</Label>
        <Input placeholder="请输入 challenge 值（32位）" value={challenge} onChange={(event) => setChallenge(event.target.value)} />
      </TextField>
