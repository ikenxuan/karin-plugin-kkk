...
export function PokerCardsSection() {
  ...
  useEffect(() => {
    ...
    const TOTAL_SCROLL_SVH = CARD_DISMISS_START + stickyCardCount * CARD_DISMISS_DURATION
    ...
    const CTA_ENTER_START = CARD_DISMISS_START + (stickyCardCount - 1) * CARD_DISMISS_DURATION
    ...
    const ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        ...
        onUpdate: ({ progress }) => {
          // === 阶段 1：所有卡片整体从下方进入 ===
          const enterProgress = gsap.utils.clamp(0, 1, gsap.utils.mapRange(0, svhToProgress(CARDS_ENTER_END), 0, 1, progress))
          ...
          backs.forEach((back, i) => {
            ...
            const dismissProgress = gsap.utils.clamp(0, 1, gsap.utils.mapRange(dismissStart, dismissEnd, 0, 1, progress))

            const backY = gsap.utils.mapRange(0, 1, 0, -250, dismissProgress)

            if (dismissProgress > 0) {
              // 终止背面可能仍在进行的 rotation 补间，避免与 scroll-driven 的 set 冲突导致闪烁
              gsap.killTweensOf(back, 'rotation')

              const backRotation = gsap.utils.mapRange(0, 1, cardFlipTiltAngles[i] || 0, cardDismissTiltAngles[i] || 0, dismissProgress)
          ...
          if (cta) {
            const ctaProgress = gsap.utils.clamp(
              0,
              1,
              gsap.utils.mapRange(svhToProgress(CTA_ENTER_START), svhToProgress(CTA_ENTER_END), 0, 1, progress)
            )
  ...
  return (
    <section ref={sectionRef} className="relative w-full overflow-hidden" style={{ height: '100vh' }}>
      {/* 卡片容器 */}
      <div className="absolute top-0 left-0 w-full h-full" style={{ perspective: 1000 }}>
        {CARDS_DATA.map((card, i) => (
          ...
          >
            ...
            >
              {/* 左上：中文标题 */}
              <div className="font-mono text-sm tracking-widest">{card.titleCN}</div>
              ...
              {/* 右下：英文标题 */}
              <div className="font-mono text-sm tracking-wider text-right">{card.titleEN}</div>

              {/* 右上编号 */}
              <div className="absolute top-6 right-6 font-mono text-2xl font-bold">{card.num}</div>

              {/* 左下编号 */}
              <div className="absolute bottom-6 left-6 font-mono text-2xl font-bold">{card.num}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div ref={ctaRef} className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
        <div className="text-center space-y-6 pointer-events-auto">
          <h2 className="text-3xl font-bold text-fd-foreground">开始使用 karin-plugin-kkk</h2>
          <p className="text-lg text-fd-muted-foreground leading-relaxed max-w-xl mx-auto">
            基于 Karin 框架的短视频解析与动态推送插件，支持抖音、B站等多平台内容解析， 提供精美的卡片渲染与实时动态推送功能。
          </p>
