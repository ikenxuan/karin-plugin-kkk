...
async function main() {
	...
	for (const target of targets) {
		...
		if (result.success) console.log(pc.bold(pc.green(`  ✓ ${target.padEnd(12)}`)) + pc.green(`${formatTime(result.duration)}`));
		else {
...
function getCompressedSize(dir) {
	try {
		...
		const walk = (dirPath) => {
			...
			for (const item of items) {
				...
				else if (item.isFile()) buffers.push(readFileSync(fullPath));
			}
		};
		walk(dir);
...
async function printCorePackageSize() {
	...
	for (const file of files) {
		const fullPath = join(coreDir, file);
		if (file.endsWith("/")) compressedSize += getCompressedSize(fullPath);
		else try {
