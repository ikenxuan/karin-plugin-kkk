...
- SSR 渲染的 ctx 组装在 `packages/core/src/module/utils/Render/index.ts`（scale/version/ambientCover/theme）；封面智能主题决策在同目录 `coverTheme.ts`（按封面明暗算出布尔值，由 Render 写入 `ctx.theme.mode`）。
